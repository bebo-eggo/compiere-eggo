...
/**
 *  Copie du programme InventoryValue.java en ajoutant les restrictions sur AD_Org_ID �galement
 *
 *  @author     Jorg Janke
 *  @version    $Id$
 */
public class InventoryValueByOrg extends SvrProcess
{
	/** Price List Used         */
	...
	private int p_AD_Org_ID; //ZCOM999
	...
	protected String doIt() throws Exception
	{
		...
		//ZCOM999 MWarehouse wh = MWarehouse.get(getCtx(), p_M_Warehouse_ID);
		//ZCOM999 MClient c = MClient.get(getCtx(), wh.getAD_Client_ID());
		MClient c = MClient.get(getCtx(), getCtx().getAD_Client_ID()); //ZCOM999
		...
		sql = new StringBuffer ("INSERT INTO T_InventoryValue "
			...
			//ZCOM999 + "WHERE w.M_Warehouse_ID=").append(p_M_Warehouse_ID);
		if(p_AD_Org_ID > 0)
			sql.append("WHERE w.AD_Org_ID=").append(p_AD_Org_ID); //ZCOM999
		//ZCOM999
		if (p_M_Warehouse_ID != 0) {
			sql.append(" AND w.M_Warehouse_ID=").append(p_M_Warehouse_ID);
		}
		//ZCOM999
		...
		if (p_M_CostElement_ID != 0)
		{
			sql = new StringBuffer ("INSERT INTO T_InventoryValue "
				...
				+ " INNER JOIN AD_ClientInfo ci ON (w.AD_Client_ID=ci.AD_Client_ID)"
				+ " INNER JOIN C_AcctSchema acs ON (ci.C_AcctSchema1_ID=acs.C_AcctSchema_ID)"
				//ZCOM999 + " INNER JOIN M_Cost c ON (acs.C_AcctSchema_ID=c.C_AcctSchema_ID AND acs.M_CostType_ID=c.M_CostType_ID AND c.AD_Org_ID IN (0, w.AD_Org_ID)) "
				// !! Ne marche en cout multi site
				/*ZCOM999*/ + " LEFT OUTER JOIN M_Cost c ON (acs.C_AcctSchema_ID=c.C_AcctSchema_ID AND acs.M_CostType_ID=c.M_CostType_ID AND c.AD_Org_ID = 0) "
				+ " INNER JOIN M_Product pr on (pr.M_Product_ID=c.M_Product_ID) "); 
				//ZCOM999 + "WHERE w.M_Warehouse_ID=").append(p_M_Warehouse_ID)
				if(p_AD_Org_ID > 0)
					sql.append("WHERE w.AD_Org_ID=").append(p_AD_Org_ID);
//				+ "WHERE w.AD_Org_ID=").append(p_AD_Org_ID)
