 ...
 */
public class InventoryCountCreate extends SvrProcessExtended
{
	...
	/*ZCOM478*/private int p_C_Charge_ID = 0;
	
	/*ZCOM478*/private String p_inventoryType = MInventoryLine.INVENTORYTYPE_InventoryDifference;
	//Smile-2632
	private int			p_Z_S_Famille_ID = 0;
	private boolean		p_IncludeAll = false;
	...
	protected void prepare()
	{
		...
		for (ProcessInfoParameter element : para) 
		{
			...
			else if (name.equals("InventoryType"))
				p_inventoryType = (String)element.getParameter();
			/*END OF ZCOM478*/
			//Smile-2632
			else if (name.equals("Z_S_Famille_ID"))
				p_Z_S_Famille_ID = element.getParameterAsInt();
			else if (name.equals("IncludeAll"))
				p_IncludeAll = "Y".equals(element.getParameter());
	...
	protected String doIt () throws Exception
	{
		...
		/*ZCOM478*/
		if (p_inventoryType.equalsIgnoreCase(MInventoryLine.INVENTORYTYPE_ChargeAccount)) {
			if (p_C_Charge_ID == 0) {
				throw new CompiereSystemException("@FillMandatory@ @C_Charge_ID@");
			...
			for (X_Ref_Quantity_Type qtyType : X_Ref_Quantity_Type.values())
			{
				...
				sql += " AND l.IsDefault='Y'"
					...
					+ " AND NOT EXISTS (SELECT * FROM M_StorageDetail s"
						...
						+ " AND s.QtyType='" + qtyType.getValue() + "')"; // #16649
		...
		if (p_Z_S_Famille_ID != 0)
			sql.append(" AND p.Z_S_FAMILLE_ID=?");
		...
		try
		{
			...
			if (p_Z_S_Famille_ID != 0)
				pstmt.setInt(index++, p_Z_S_Famille_ID);
			if (!p_DeleteOld)
		...
		if (p_InventoryCountSetZero)
		{
			...
			int no = DB.executeUpdate(get_Trx(), sql1);
			log.info("Set Cont to Zero=" + no);
		}
		
		//2632 IncludeAll=Y => ajouter toute la base article dans la liste de comptage
		if(p_IncludeAll)
		{
			String SQL = "insert into ZT_SEQNO (seqNo, AD_PINSTANCE_ID)  (Select NVL(MAX(LINE),10)+10, max("+getAD_PInstance_ID()+") FROM M_InventoryLine where M_InVentory_ID = ?)";
			Trx xxx = Trx.get("SEQNO");
			int no = DB.executeUpdate(SQL,m_inventory.getM_Inventory_ID(),xxx);
			xxx.commit();
			
			SQL = "INSERT INTO M_INVENTORYLINE (AD_CLIENT_ID , AD_ORG_ID , AD_ORGTRX_ID , C_ACTIVITY_ID , C_CHARGE_ID , CREATED , CREATEDBY , DESCRIPTION , INVENTORYTYPE ,"
					+ "ISACTIVE , ISINTERNALUSE , LINE , M_ABCANALYSISGROUP_ID , M_ATTRIBUTESETINSTANCE_ID , M_INVENTORY_ID , M_INVENTORYLINE_ID , M_LOCATOR_ID , "
					+ "M_PRODUCT_ID , PROCESSED , QTYBOOK , QTYCOUNT , QTYINTERNALUSE , UPDATED , UPDATEDBY ) "
					...
					+ "?,"
					+ "'Y','N',"
					+ "AD_Sequence_NextSeqNo(?),"
					+ "null,0,"
					+ "?,"
					+ "AD_Sequence_NextNo ('M_InventoryLine'),"
					+ "(Select MAX(M_LOcator_ID) FROM M_LOCATOR WHERE IsDefault='Y' and M_WAREHOUSE_ID = ?),"
					+ "p.M_Product_ID,'N',0,0,0,Sysdate, "
					+ "? "
					+ "from M_Product p where Not Exists (Select 1 from M_InventoryLine where M_Inventory_ID = ? "
					+ "and p.M_PRODUCT_ID = M_InventoryLine.M_PRODUCT_ID and p.AD_CLIENT_ID = M_InventoryLine.AD_CLIENT_ID)   "
					+ "and p.IsActive='Y' AND p.IsStocked='Y' AND p.IsSummary = 'N' AND p.ProductType='I' and p.AD_CLIENT_ID = ?";
			ArrayList<Object> params = new ArrayList<Object>();
			params.add(m_inventory.getAD_Org_ID());
			params.add(getAD_User_ID());
			params.add(p_inventoryType);
			params.add(getAD_PInstance_ID());
			params.add(m_inventory.getM_Inventory_ID());
			params.add(m_inventory.getM_Warehouse_ID());
			params.add(getAD_User_ID());
			params.add(m_inventory.getM_Inventory_ID());
			params.add(getAD_Client_ID());
			PreparedStatement ps = null;
			no = DB.executeUpdate(SQL, params, get_Trx());
			log.info("INSERT ALL=" + no);
		}
		...
		int M_AttributeSetInstance_ID, BigDecimal QtyOnHand, int M_AttributeSet_ID)
	{
		...
		if (oneLinePerASI)
		{
			...
			if (p_inventoryType.equalsIgnoreCase(MInventoryLine.INVENTORYTYPE_ChargeAccount)) {
				if (line != null) {
					line.setInventoryType(p_inventoryType);
		...
		if (p_inventoryType.equalsIgnoreCase(MInventoryLine.INVENTORYTYPE_ChargeAccount)) {
			if (m_line != null) {
			m_line.setInventoryType(p_inventoryType);
			m_line.setC_Charge_ID(p_C_Charge_ID);
			}
		}
		/*ZCOM478********************/
