 ...
 */
public class AllocationAutoEC extends SvrProcess
{
	/**	BP Group					*/
...
//	private String		p_APAR = "A";
	...
	private Boolean p_IsCustomer=true;
...
//				p_APAR = (String)element.getParameter();
				...
				p_IsCustomer = "Y".equals(element.getParameter());
			else
	...
	protected String doIt() throws Exception
	{
		log.info ("C_BP_Group_ID=" + p_C_BP_Group_ID 
			+ ", C_BPartner_ID=" + p_C_BPartner_ID + ",p_IsCustomer" +p_IsCustomer);
...
//			+ ", AP/AR=" + p_APAR);
		...
		{
			 MBPartner bp=new MBPartner(getCtx(), p_C_BPartner_ID, get_Trx());
		...
		else if (p_C_BP_Group_ID != 0)
		{
              ...
              if(p_IsCustomer)
				sql = "SELECT C_BPartner_ID FROM C_BPartner WHERE C_BP_Group_ID=? and IsCustomer ='Y'  ORDER BY Value";
				else
				sql = "SELECT C_BPartner_ID FROM C_BPartner WHERE C_BP_Group_ID=? and IsVendor ='Y'  ORDER BY Value";
		...
		else
		{
			...
			if(p_IsCustomer)
			sql = "SELECT C_BPartner_ID FROM C_BPartner WHERE AD_Client_ID=? and IsCustomer ='Y'  ORDER BY Value";
			else
			sql = "SELECT C_BPartner_ID FROM C_BPartner WHERE AD_Client_ID=? and IsVendor ='Y'  ORDER BY Value";
...
//		else if (ONLY_AR.equals(p_APAR))
	...
	private int allocateBPOldestFirst(boolean exactMatchOnly) throws Exception
	{
		...
		if (difference.signum() == 0 || !exactMatchOnly)
		{
			...
			//Evolution #74558
			if(difference.signum() == 0 && m_allocation == null)
				return 0;
