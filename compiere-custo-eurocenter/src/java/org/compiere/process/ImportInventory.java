...
public class ImportInventory extends SvrProcess
{
	...
	private ArrayList<MInventory> imps = null;
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuffer("Update I_Inventory i set AD_Org_ID =(SELECT MAX(AD_Org_ID) FROM AD_Org w"
				+ " WHERE i.Org_Value=w.Value AND i.AD_Client_ID=w.AD_Client_ID) where i.AD_Org_ID = 0 or i.AD_Org_ID is null");
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		log.info ("Set Org=" + no);
		...
		sql = new StringBuffer ("update I_Inventory i set i.ProductValue = replace(i.ProductValue,'\\\\','\\') where i.IsActive='Y' AND i.I_IsImported<>'Y' ").append (clientCheck);
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		...
		try
		{
			...
			while (rs.next())
			{
					...
					|| !MovementDate.equals(x_MovementDate))
				{
					...
					inventory.setC_DocType_ID(imp.get_ValueAsInt("C_DocType_ID"));
					...
					if(MInventory.DOCACTION_None.equals(docAction))
						docAction = MInventory.DOCACTION_Complete;
