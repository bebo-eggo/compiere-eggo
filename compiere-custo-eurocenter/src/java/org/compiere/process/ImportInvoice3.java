 ...
 */
public class ImportInvoice3 extends SvrProcess
{
	...
	List<MInvoice> invoicesToDelete = new ArrayList<MInvoice>();
	List<X_I_Invoice> ImpToReup = new ArrayList<X_I_Invoice>();
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = "UPDATE I_Invoice "
				...
				+ " DocumentNo = NVL (DocumentNo, TO_CHAR('<TOCREATE>')),"
				+ " I_ErrorMsg = NULL,"
				+ " I_IsImported = 'N' "
		...
		sql =  "UPDATE I_Invoice o "
				+ "SET (C_TAX_ID)=(SELECT max(C_TAX_ID) "
				+ "FROM C_TAX  where C_TAX.NAME = 'AMEUBN' and C_TAX.AD_Client_ID = o.AD_Client_ID) where o.TaxID like 'BE%' and o.AD_client_ID = 1000024 " 
				+ STD_CLIENT_CHECK;
		no = DB.executeUpdate(sql.toString(), new Object[]{m_AD_Client_ID}, get_Trx());
		log.fine("Set BP from updated Value=" + no);
		...
		/*
		sql = "UPDATE I_Invoice "
				+ "SET I_IsImported='E', I_ErrorMsg="+ts +"||'ERR=Pourcentage non cohérent XML <> parametrage, ' "
		...
		*/
		...
		log.fine("Set Product from Value=" + no);
		sql = "UPDATE I_Invoice "
				+ "SET XX_ErrorType = '01', I_IsImported='E', I_ErrorMsg="+ts +"||'ERR=Article inconnu, ' "
				+ "WHERE M_Product_ID IS NULL AND (ProductValue IS NOT NULL)"
				+ " AND I_IsImported <>'Y'" 
				+ STD_CLIENT_CHECK;
		no = DB.executeUpdate(sql, new Object[] {m_AD_Client_ID}, get_Trx());
	...
	////Evolution #86715 Ordre ACHAT
			...
			sql = "UPDATE I_Invoice o "
					+ " SET ZZ_ORDERLINE_ID=(SELECT MAX(C_ORDERLINE_ID) FROM C_OrderLine ord"
					+ " WHERE ord.C_ORDER_ID = o.C_OrderPO_ID AND o.AD_Client_ID=ord.AD_Client_ID and ord.C_BPartner_ID = o.C_BPartner_ID and ord.QtyOrdered = o.QtyOrdered and ord.M_Product_ID = o.M_Product_ID) "
					+ " WHERE ZZ_ORDERLINE_ID IS NULL AND C_OrderPO_ID IS NOT NULL "
					+ " AND I_IsImported <>'Y'" 
					+ STD_CLIENT_CHECK;
			no = DB.executeUpdate(sql, new Object[] {m_AD_Client_ID}, get_Trx());	
			...
			sql = "UPDATE I_Invoice o "
					+ " SET ZZ_ORDERLINE_ID=(SELECT MAX(C_ORDERLINE_ID) FROM C_OrderLine ord"
					+ " WHERE ord.C_ORDER_ID = o.C_OrderPO_ID AND o.AD_Client_ID=ord.AD_Client_ID and ord.C_BPartner_ID = o.C_BPartner_ID  and ord.M_Product_ID = o.M_Product_ID) "
					+ " WHERE ZZ_ORDERLINE_ID IS NULL AND C_OrderPO_ID IS NOT  NULL "
					+ " AND I_IsImported <>'Y'" 
					+ STD_CLIENT_CHECK;
			no = DB.executeUpdate(sql, new Object[] {m_AD_Client_ID}, get_Trx());	
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if(imp.get_Value("Z_ProductTax")!=null ){
					...
					{
						String str = items.get(x);
						//Vérifier que le code est dans les taxes
						if(!listTaxesProduct.contains(str.substring(0,3).toUpperCase()) && id!=imp.getI_Invoice_ID())
						{
							imp.setI_ErrorMsg(imp.getI_ErrorMsg()+"ERR=Code taxe article inconnu,"+str.substring(0,3));
		...
		sql = "UPDATE I_Invoice i "
				+ "SET I_IsImported='E',XX_ErrorType = '15', I_ErrorMsg="+ts +"||'ERR="+msg+", ' "
		...
		msg = ", Ref fournisseur en doublon dans les imports pour fichiers différents";
		sql = "UPDATE I_Invoice i "
				+ "SET I_IsImported='E', I_ErrorMsg="+ts +"||'ERR=ERR||"+msg+", ' "
				+ "WHERE (I_IsImported<>'Y' OR I_IsImported IS NULL) and exists (select 1 from I_Invoice ii WHERE  " 
				+ " ii.POReference = i.POReference AND ii.POReference IS NOT NULL and ii.ZZ_FileName != i.ZZ_FileName "
        		+ "AND ii.C_BPartner_ID = i.C_BPartner_ID and ii.AD_CLIENT_ID = i.AD_CLIENT_ID) "
        		+ STD_CLIENT_CHECK;
		no = DB.executeUpdate(sql.toString(), new Object[] {m_AD_Client_ID}, get_Trx());
		if (no > 0)
			log.warning("EC_DUPLICAT_POREFERENCE=" + no);
		...
		try
		{
			...
			while (rs.next ())
			{
						...
						|| !oldPOReference.equals(cmpPOReference))
				{
					...
					if(!oldDocumentNo.equals("") && !oldDocumentNo.equals("<TOCREATE>"))
						invoice.setDocumentNo(oldDocumentNo);
				...
				String TaxID = imp.get_ValueAsString("TaxID");
				int M_Warehouse_ID = 0;
				if(TaxID != null && TaxID.length()>0 && TaxID.startsWith("BE"))
				{
					//int Org_ID = InvoiceUtil.getOrgByGLN(getCtx(),getAD_Client_ID(),imp.get_ValueAsString("ZZ_ShipToGnl"), get_Trx());
					//M_Warehouse_ID = InvoiceUtil.getWarehouseIDByCountry(getCtx(),Org_ID>0?Org_ID: invoice.getAD_Org_ID(), "BE", get_Trx());
				...
				line.setZZ_ProductTax(imp.get_ValueAsString("Z_ProductTax"));
				
				//MatchOA
				if(imp.get_ValueAsInt("ZZ_ORDERLINE_ID")>0)
				{
					line.setC_OrderLine_ID(imp.get_ValueAsInt("ZZ_ORDERLINE_ID"));
				}else
					if(imp.get_ValueAsInt("C_OrderPO_ID")>0)
					{
						MOrder mo = new MOrder(getCtx(), imp.get_ValueAsInt("C_OrderPO_ID"), get_Trx());
						int OrderLine_ID = mo.addPartFromInv(line.getM_Product_ID(), Env.ZERO, line.getPriceEntered(), "Import Facture "+imp.getPOReference());
						line.setDescription("Import Facture "+imp.getPOReference());
						line.setC_OrderLine_ID(OrderLine_ID);
						imp.set_ValueNoCheck("C_OrderLinePO_ID", OrderLine_ID);
	...
	public void saveInvoices (	Map<MInvoice, List<MInvoiceLine>> invoiceMap,
		...
		CopyOnWriteArrayList<InvoiceTaxProduct> lInvProductTaxes = new CopyOnWriteArrayList<InvoiceTaxProduct> ();
		int invoice_ID  = 0;
		...
		for(Map.Entry<MInvoice, List<MInvoiceLine>> entry : invoiceMap.entrySet()) {
			...
			for(MInvoiceLine invoiceLine : entry.getValue()) {
				...
				invoiceLine.setImportInvoiceElectro(true);
				invoiceLine.setXxx_partner_ID(invoice.getC_BPartner_ID());
				invoiceLinesToSave.add(invoiceLine);
				if(invoice_ID!=invoice.getC_Invoice_ID()){
					lInvProductTaxes.add(new InvoiceTaxProduct(invoice.getC_Invoice_ID(), invoice.getC_BPartner_ID()));
					invoice_ID = invoice.getC_Invoice_ID();
				}
			}
		}
		...
		List<Integer> importInvoices_IDs = new ArrayList<Integer>();
		
		//Create Line Tax
		HashMap<String, Double> litsTaxPorduct = new HashMap<String, Double>();
		
		
		
		InvoiceTaxProduct tx = null;
		invoice_ID = 0;
		for (int i = 0; i < lInvProductTaxes.size(); i++) {
			for(MInvoiceLine line :invoiceLinesToSave) {
				X_I_Invoice imp = importInvoiceMap.get(line.getI_Invoice_ID());
				if(lInvProductTaxes.get(i).getInvoice_ID()!=line.getC_Invoice_ID())
					continue;
				if(imp != null && !importInvoices_IDs.contains(imp.getI_Invoice_ID())) {
					imp.setC_InvoiceLine_ID(line.getC_InvoiceLine_ID());
					imp.setC_Invoice_ID(line.getC_Invoice_ID());
					importInvoicesToSave.add(imp);
					importInvoices_IDs.add(imp.getI_Invoice_ID());
				}
				if(invoice_ID!=line.getC_Invoice_ID()){
					litsTaxPorduct = new HashMap<String, Double>();
					lInvProductTaxes.get(i).setLitsTaxPorduct(litsTaxPorduct);
				}
				if(line.getZZ_ProductTax()!=null && line.getZZ_ProductTax().length()>0){
					List<String> items = Arrays.asList(line.getZZ_ProductTax().split(";"));
					for(String str : items) 
					{
						List<String> str1 = Arrays.asList(str.split(","));
						if(litsTaxPorduct.size()>0 && litsTaxPorduct.containsKey(str1.get(0)))
							litsTaxPorduct.put(str1.get(0), litsTaxPorduct.get(str1.get(0))+((double)Double.parseDouble(str1.get(1))));
						else
							litsTaxPorduct.put(str1.get(0), (double) Double.parseDouble(str1.get(1)));
					}
				}
		...
		for (int i = 0; i < lInvProductTaxes.size(); i++) {
			InvoiceTaxProduct nn = lInvProductTaxes.get(i);
			for(Entry<String, Double> entry : nn.getLitsTaxPorduct().entrySet()) {
				InvoiceUtil.addModifyRecupelLine(getCtx(),nn.getInvoice_ID(), nn.getBPartner_ID(), entry.getValue(), entry.getKey(), get_Trx());
			}
		...
		for(X_I_Invoice imp : importInvoicesToSave) {
			if(imp.get_ValueAsInt("C_OrderLinePO_ID")>0)
				DB.executeUpdate("Update C_OrderLine set QtyOrdered = 0 where C_OrderLine_ID = ? ", imp.get_ValueAsInt("C_OrderLinePO_ID"), get_Trx());
		}
		...
		for(MInvoice invoice : invoicesToSave) {
			...
			for(X_I_Invoice imp : importInvoicesToSave) {
				...
				if(C_Invoice_id == imp.getC_Invoice_ID())
				{
					...
					BigDecimal diff  =new BigDecimal( (imp.get_ValueAsBigDecimal("InvoiceTotalAmt").doubleValue())- invoice.getGrandTotal().doubleValue()).setScale(2, RoundingMode.HALF_UP);
					if(diff.abs().equals((new BigDecimal(0.01)).setScale(2,RoundingMode.HALF_UP) ))
					{
						MInvoiceLine[] mils = invoice.getLines();
						if(mils.length>0)
						{
							MInvoiceLine mil = mils[0];
							mil.setTaxAmt(mil.getTaxAmt().add(diff));
							mil.save();invoice.calculateTaxTotal();
							invoice.save();
						}
					...
					if(invoice.getGrandTotal().doubleValue()==(imp.get_ValueAsBigDecimal("InvoiceTotalAmt").doubleValue()))
					{
						if(BPartner_ValidateInvoice.get(invoice.getC_BPartner_ID()).equals("Y"))
						{
							...
							if(!DocumentEngine.processIt(invoice, DocActionConstants.ACTION_Complete)) {
								...
								imp.setI_ErrorMsg("Impossible de traiter la facture, traitement manuel : "+invoice.getProcessMsg());
								imp.setDocumentNo(invoice.getDocumentNo());
								imp.setC_Invoice_ID(0);
								imp.setC_InvoiceLine_ID(0);
								invoicesToDelete.add(invoice);
								ImpToReup.add(imp);
								imp.setI_IsImported(X_I_Invoice.I_ISIMPORTED_Error);
								imp.setI_IsImported("E");imp.setProcessed(false);
								imp.set_ValueNoCheck("XX_ErrorType", "10");
								countErrorMntTot++;
								imp.save(get_Trx());
							}else
							{/*
								String sql =  "UPDATE C_Invoice o "
										+ "SET (C_BPartner_Location_ID)=(SELECT MAX(C_BPartner_Location_ID) "
										+ "FROM C_BPartner_Location bpl WHERE bpl.C_BPartner_ID = o.C_BPartner_ID AND o.AD_Client_ID=bpl.AD_Client_ID " 
										+ "AND exists (select 1 from C_BPartner where C_BPartner.C_BPartner_ID = bpl.C_BPartner_ID and bpl.C_BPartner_Location_ID = C_BPartner.Z_DEFAULT_ADR_ID) ) "
										+ "WHERE C_Invoice_ID = ? and EXISTS (select 1 from I_Invoice where I_Invoice.C_Invoice_ID = o.C_Invoice_ID and I_Invoice.TaxID like 'BE%' and I_Invoice.AD_client_ID = 1000024 )" 
										+ STD_CLIENT_CHECK;
								int no = DB.executeUpdate(sql.toString(), new Object[]{invoice.getC_Invoice_ID(),m_AD_Client_ID}, get_Trx());
								log.fine("Set BP from updated Value=" + no);
								*/
							}
						}else
					...
					else
					{
						...
						invoicesToDelete.add(invoice);
						ImpToReup.add(imp);
						imp.set_ValueNoCheck("XX_ErrorType", "05");
						imp.setI_IsImported(X_I_Invoice.I_ISIMPORTED_Error);
						imp.setI_ErrorMsg("Montants totaux incohérents, traiter la facture manuellement, "+invoice.getGrandTotal());imp.setI_IsImported("E");imp.setProcessed(false);
						countErrorMntTot++;
						imp.save(get_Trx());
					}
		...
		for(MInvoice invoice : invoicesToDelete) {
			DB.executeUpdate("DELETE FROM C_INVOICE WHERE C_INVOICE_ID = ? ",invoice.getC_Invoice_ID(), get_Trx());
		}
		
		for(X_I_Invoice imp : ImpToReup) {
			DB.executeUpdate(get_Trx(), "Update I_Invoice set Processed = 'N', I_IsImported = 'E' WHERE AD_Client_ID = ? and POReference = ?", imp.getAD_Client_ID(),imp.getPOReference());
		}
	...
	public static void main(String[] args)
	{
		...
		ImportInvoice3 test = new ImportInvoice3();
	...
	private class InvoiceTaxProduct{

		private int Invoice_ID = 0;
		private int BPartner_ID = 0;
		private Map<String, Double> LitsTaxPorduct = new HashMap<String, Double>();
		
		public InvoiceTaxProduct(){
			super();
		}
		
		public InvoiceTaxProduct(int invoice_ID, int bPartner_ID, HashMap<String, Double> litsTaxPorduct) {
			super();
			Invoice_ID = invoice_ID;
			BPartner_ID = bPartner_ID;
			LitsTaxPorduct = litsTaxPorduct;
		}

		public InvoiceTaxProduct(int invoice_ID, int bPartner_ID) {
			super();
			Invoice_ID = invoice_ID;
			BPartner_ID = bPartner_ID;
		}
		...
		public Map<String, Double> getLitsTaxPorduct() {
			return LitsTaxPorduct;
		}

		public void setLitsTaxPorduct(HashMap<String, Double> litsTaxPorduct) {
			LitsTaxPorduct = litsTaxPorduct;
		}
