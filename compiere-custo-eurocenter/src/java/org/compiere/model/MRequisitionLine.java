...
public class MRequisitionLine extends X_M_RequisitionLine
{
	...
	public void setPrice()
	{
		...
		if (getM_Product_ID() == 0)
			return;
		MProductPO mpo[] = MProductPO.getOfProduct(getCtx(), getM_Product_ID(), get_Trx());
		if(mpo.length>0)
		{
			int pricelis = DB.getSQLValue(get_Trx(), "Select PO_PriceList_ID from C_BPartner where C_BPartner_ID = ?", mpo[0].getC_BPartner_ID());
			if(pricelis>0)
			{
				setPrice (pricelis);
				return;
		...
		if (columnName.equals("QtyEntered"))
		{
			...
			if(getM_Product_ID()!=0)
			{
				...
				for (int i = 0; i < mpo.length; i++) {
					if(mpo[i].isCurrentVendor() || mpo.length==0)
					{
						C_UOM_To_ID = mpo[i].getC_UOM_ID();
						break;
					}
					else 
						continue;
				...
				if(C_UOM_To_ID !=0)
				{
					...
					Qty = MUOMConversion.convertProductFrom (getCtx(),
							getM_Product_ID(), C_UOM_To_ID, QtyEntered);
					if (Qty == null)
						Qty = QtyEntered;
	 ...
	 *	@throws Exception
	 */
	@UICallout public void setQtyEntered (String oldQty, 
			String newQty, int windowNo) throws Exception
	{
		if (newQty == null || newQty.length() == 0)
			return;
		BigDecimal QtyEntered = new BigDecimal(newQty);
		super.set_Value("QtyEntered", QtyEntered);
		setQty(windowNo, "QtyEntered");
		setAmt(windowNo, "Qty");		
	}	//	setQty

	private java.math.BigDecimal getQtyEntered() {
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getPriceActual().compareTo(Env.ZERO) == 0)
			setPrice();
		setLineNetAmt();
		if(!getQty().equals(getQtyEntered()) && !is_new())
		{
			super.set_Value("QtyEntered", getQtyEntered());
			setQty(getQtyEntered());
			setLineNetAmt();		
		}
		return true;
	...
	public int getTypeOA_ID() {
		return TypeOA_ID;
	}

	public void setTypeOA_ID(int typeOA_ID) {
		TypeOA_ID = typeOA_ID;
	}
