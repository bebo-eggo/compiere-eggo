...
public class MInOutConfirm extends X_M_InOutConfirm implements DocAction
{
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
    	...
    	if (isProcessed ())
		{
			MInOut mi= new MInOut (getCtx(), getM_InOut_ID(), get_Trx());
			if (DocActionConstants.STATUS_InProgress.equalsIgnoreCase(mi.getDocStatus())) {
				//MInOutLine[] moveLines = mi.getLines(false);
			
				success = DocumentEngine.processIt(mi, DocActionConstants.ACTION_Complete);
				mi.save(get_Trx());
				if(success)
				{
					MOrder mo = new MOrder(getCtx(), mi.getC_Order_ID(),get_Trx() ); 
					String msg = OrderUtil.genInvOderShip(getCtx(),mo, get_Trx()) ;
				}
				}
			}	
