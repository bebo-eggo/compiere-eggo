...
/**
 *  Recopier ici tant que pas en 361.7 Voir Patch #16942
 */
public class MProductPricing
{
	...
	protected final int 		m_AD_Client_ID;
	protected final int 		m_AD_Org_ID;
	protected final int 		m_M_Product_ID;
	protected final int 		m_C_BPartner_ID;
	...
	protected boolean     m_isBOM = false;

	protected Boolean		m_userPricing = null;
	protected UserPricingInterface 	m_api = null;
	...
	private boolean calculateUser()
	{
		...
		if ((vo != null) && vo.isValid())
		{
			...
			if ((m_C_UOM_ID == 0) || (m_C_Currency_ID == 0))
				setBaseInfo();
			return true; // ZCOM #16942 SINON JAMAIS PRIS EN COMPTE !
		}
		return false;
	}	//	calculateUser
...
//		EggoCtx.setConstants();
		...
		String sql = "SELECT pv.M_PriceList_Version_ID,"	//	1
			+ " p.C_UOM_ID,pv.ValidFrom,pl.C_Currency_ID,p.M_Product_Category_ID,pl.EnforcePriceLimit,pv.ValidTo,pv.AD_ORG_ID "	// 2..6
			+ "FROM M_Product p"
			+ " INNER JOIN M_ProductPrice pp ON (p.M_Product_ID=pp.M_Product_ID)"
			+ " INNER JOIN  M_PriceList_Version pv ON (pp.M_PriceList_Version_ID=pv.M_PriceList_Version_ID)"
			+ " inner join AD_Org ao on ao.AD_ORG_ID = pv.AD_ORG_ID"
			+ " INNER JOIN M_PriceList pl ON (pv.M_PriceList_ID=pl.M_PriceList_ID) "
			+ "WHERE pv.IsActive='Y'"
			+ " AND pp.IsActive='Y'"
			+ " AND pl.IsActive='Y'"
			...
			+ " ORDER BY ao.IsMag DESC, pv.ValidFrom DESC";
		m_calculated = false;
		...
		try
		{
			...
			{
				if(rs.getInt(8) !=0)
					oip = new MOrgInfoProxy(MOrgInfo.get(Env.getCtx(), rs.getInt(8), (Trx) null));
				Timestamp plDate = rs.getTimestamp(3);
				Timestamp plDateTo = rs.getTimestamp(7);
				//	we have the price list
				//	if order date is after or equal PriceList validFrom
				if(oip != null && oip.isPOS())//MERGE
					if(m_AD_Org_ID != rs.getInt(8) && rs.getInt(8) !=0)
						continue;
					if ((plDate == null) || (!m_PriceDate.before(plDate) && ((plDateTo!=null && !m_PriceDate.after(plDateTo)) || plDateTo ==null)))
					{
