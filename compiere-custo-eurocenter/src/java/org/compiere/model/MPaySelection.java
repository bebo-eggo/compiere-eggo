...
public class MPaySelection extends X_C_PaySelection
{
	 ...
	 *	@return lines
	 */
	public MPaySelectionLine[] getLines(boolean requery, boolean OnlySepa)
	{
		...
		//#67460
		String sql = "SELECT * FROM C_PaySelectionLine pl INNER JOIN C_Invoice i ON i.C_Invoice_ID = pl.C_Invoice_ID " +
				"WHERE C_PaySelection_ID=? ";
		if(OnlySepa)
			sql = sql +" AND exists (select 1 from ZSUBPAYMENTRULE "
					+ "where i.ZSUBPAYMENTRULE_ID = ZSUBPAYMENTRULE.ZSUBPAYMENTRULE_ID and ZSUBPAYMENTRULE.ISEXPORTSEPA = 'Y' ) ";
				
			sql = sql +" ORDER BY i.C_BPartner_ID, pl.PayAmt, i.DueDate, i.DocumentNO";
