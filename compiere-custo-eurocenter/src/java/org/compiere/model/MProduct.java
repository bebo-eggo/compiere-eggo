...
public class MProduct extends X_M_Product
{
			...
			String newM_Product_Category_ID, int windowNo) throws Exception
	{
		...
		// setM_AttributeSet_ID((Integer)pc.get_Value("M_AttributeSet_ID"));//EC001
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
		 	//Set Z_IMEI
			int Z_SSS_F = get_ValueAsInt("Z_S_S_S_Famille_ID");
			if(Z_SSS_F!=0)
			{
				String Z_IMEI = DB.getSQLValueString(get_Trx(), "Select Z_IMEI from Z_S_S_S_Famille where Z_S_S_S_Famille_ID = ?", Z_SSS_F);
				set_Value("Z_IMEI", Z_IMEI.equals("Y"));
			}
		}
		//	Check Storage
		if(getValue()!=null)
		...
		if(get_ValueOld("Z_Gamme_ID")==null && is_ValueChanged("Z_Gamme_ID") && get_Value("Z_Gamme_ID")!=null) 
		{
			Calendar cal = Calendar.getInstance();
			set_ValueNoCheck("Z_GammeChange", new Timestamp(cal.getInstance().getTimeInMillis()));
		}
		if(isPurchasedToOrder())
			setIsPurchased(isPurchasedToOrder());
		return true;
	}	//	beforeSave
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (!newRecord && (is_ValueChanged("Value") || is_ValueChanged("Name")))
			//3.8.0
			MAccount.updateValueDescription(getCtx(), "M_Product_ID=" + getM_Product_ID(), get_Trx()/*, null*/);
		...
		WSUtil.updateProduct(getCtx(), this, newRecord, get_Trx());
    ...
    //REDMINE #28701
  	@UICallout public void setM_AttributeSet_ID (String oldM_AttributeSet_ID, 
  			String newM_AttributeSet_ID, int windowNo) throws Exception{
