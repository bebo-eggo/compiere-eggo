...
public class MOrder extends X_C_Order implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//Si IsOVReadOnly change de Y à N => 
		if(dti != null && is_ValueChanged("IsOVReadOnly") && !get_ValueAsBoolean("IsOVReadOnly") && dti.isOpenOV() 
				...
				&& dti.isCuisine())
		{
			//Mettre le Processed = Y à N pour les lignes dont QtyOrdered <> QtyDelivered
			//int no = 
		...
		if (getBill_Location_ID() == 0)
		{
			...
			{
				//Reprendre la plus récente adresse de facturation du tiers
		...
		//Forecer le docaction à annuler pour les doc exclu !
		//MERGE
		...
		{
			log.saveError("Error", " L'action logistique ne peut plus être modifiée ");
			return false;
		}
				...
				&& Msg.getMsg(getCtx(), "KM_NEC24_ACTIONTRFT").contains(get_Value("Z_ActionLogList").toString())  )
		{
			...
			if (count != 0)
			{
				...
				mvt.setDescription("Trft Auto, " + ("01".equalsIgnoreCase(get_Value("Z_ActionLogList").toString()) ? "Livraison chez le client " :
					"02".equalsIgnoreCase(get_Value("Z_ActionLogList").toString()) ? "Livraison par le magasin" : "Enlevé à Rhisnes") );
				mvt.setDocAction("CO");
				...
				log.saveWarning("Warning", "Transfert généré : "+ DocNo);
			}
...
//		EggoCtx.setConstants();
		...
		{
			//Mettre le devis lié au Contrat
...
//		PosCtx.setConstants(getCtx(),get_Trx());
		...
		if (oif.isPOS() &&  orgPOS != null && orgPOS.getDocType_WebSales_ID() == getC_DocTypeTarget_ID()){
			...
			if (qtyOL != qtyAction){
				m_processMsg = "La quantité des lignes d'action ne correspond à la quantité des lignes de commande";
				return DocActionConstants.STATUS_Invalid;
			}	
		}
...
//		}
	...
	public String generateConsignMvt(MOrder ord, boolean isReturn) {
		...
		if (consID <1) throw new CompiereException("Pas d'article CONSIGNATION défini pour la société ");
		int docTypeConsID = dti.getC_DocType_MVT_ID();
		if (docTypeConsID <1) throw new CompiereException("Pas de type de document CONSIGNATION défini ");
			...
			if (typeEchID > 0 ){
				...
				if (exist > 0){
					if (this.getGrandTotal().compareTo(this.getTotalEch()) != 0)
					{
						...
						{
							//Savoir si une de réajustement n'existe pas !
							typeEchAdjID = DB.getSQLValue(get_Trx(), "select max(z_typeecheance_id) from z_typeecheance where IsAjust = 'Y'  and AD_Client_ID = ? ", getAD_Client_ID());
