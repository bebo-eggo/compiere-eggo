...
public class MInvoiceLine extends X_C_InvoiceLine
{
	...
	private boolean isImportInvoiceElectro = false;
	
	private int xxx_partner_ID = 0;
	
	private String ZZ_ProductTax = null;
	...
	public void setOrderLine (MOrderLine oLine)
	{
		...
		set_Value("IsCocontractant", oLine.get_ValueAsBoolean("IsCocontractant"));
	}	//	setOrderLine
	...
	//	setS_ResourceAssignment_ID(sLine.getS_ResourceAssignment_ID());
		...
		if (C_OrderLine_ID != 0)
		{
			...
			set_Value("IsCocontractant", m_oLine.get_ValueAsBoolean("IsCocontractant"));
		}
		else
		{
			setPrice();
	...
	public boolean setTax()
	{
		...
		int C_Tax_ID = Tax.getTaxWithCocontractant(getCtx(),parent.getC_BPartner_Location_ID(),parent.getC_BPartner_Location_ID(), getAD_Org_ID(),getM_Product_ID(),getC_Charge_ID(),
				parent.getDateInvoiced(),parent.getDateInvoiced(),parent.getC_DocTypeTarget_ID(),parent.isSOTrx(),get_ValueAsBoolean("IsCocontractant")?true:false,M_Warehouse_ID );
		
		//71122
		MBPartner billBPartner = new MBPartner(getCtx(), getParent().getC_BPartner_ID(), get_Trx());
		...
		{
			C_Tax_ID = getIncyTVA(parent.getC_DocTypeTarget_ID(),getM_Product_ID(), getC_Charge_ID());
	...
	//	setLineNetAmt();
		...
		if (tax.isDocumentLevel() && m_IsSOTrx)		//	AR Inv Tax
			return;
		//
		if (!getParent().isSOTrx() && getM_Product_ID() > 0) 
			TaxAmt = tax.calculateTax(getWoDiscount(getLineNetAmt(),getParent().getC_PaymentTerm_ID()), isTaxIncluded(), getPrecision());
		else
	...
	private BigDecimal getWoDiscount(BigDecimal lineNetAmt, int paymentterm_ID) {
		BigDecimal discount = Env.ZERO;
		if(paymentterm_ID >0)
		{
			discount = DB.getSQLValueBD(get_Trx(), "select pt.discount from c_paymentterm pt where pt.c_paymentterm_id = ?",paymentterm_ID);
		}
		else
		{
			discount =DB.getSQLValueBD(get_Trx(), "select pt.discount from c_paymentterm pt where pt.c_paymentterm_id = (select bp.PO_Paymentterm_Id from c_bpartner bp where bp.c_bpartner_id =( select c_bpartner_id from c_invoice where c_invoice_id=?))",getC_Invoice_ID());
		}
		String isDiscountable = null;
		BigDecimal lna = BigDecimal.ZERO;
		BigDecimal esc = BigDecimal.ZERO;
		isDiscountable = DB.getSQLValueString(get_Trx(), "select IsDiscountable from c_bpartner where C_BPartner_ID in ( select c_bpartner_id from c_invoice where c_invoice_id=?)",getC_Invoice_ID());
	...
	public void setLineNetAmt ()
	{
		...
		//TAM
		if (getM_Product_ID() > 0 && !DocActionConstants.ACTION_Complete.equals(getParent().getDocStatus())){
			setPriceRecupel(computeRecupel(getM_Product_ID(), getParent().getC_BPartner_ID()) );
			net = getPriceRecupel().multiply(getQtyInvoiced());
			if (net.scale() > getPrecision())
				net = net.setScale(getPrecision(), BigDecimal.ROUND_HALF_UP);
			super.setLineRecupelAmt (net);
		}
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(getM_Product_ID()==0 && getC_Charge_ID()==0 && !isDescription() ) //36501  
		{
			log.saveError("Error", Msg.getMsg(getCtx(), "NoProductOrNoCharge"));
			return false;			
		}//36501
...
//				else
				...
				if (!m_priceSet && !m_priceFound)
				{
					if(((Env.ZERO.compareTo(getPriceActual()) == 0)
							...
							getParent().isSOTrx())
					{
						log.saveError("Error", Msg.getMsg(getCtx(), "ProductNotOnPriceList")+" , ProductID "+getM_Product_ID());
						return false;
					}
				}
			}
		...
		if(get_Value("POReference")==null)
		{
			if(getC_OrderLine_ID()>0)
			{
				MOrderLine mol = new MOrderLine(getCtx(), getC_OrderLine_ID(), get_Trx());
				MOrder mo = new MOrder(getCtx(), mol.getC_Order_ID(), get_Trx());
				if(mo.getPOReference()!=null)
					set_ValueNoCheck("POReference", mo.getPOReference());
			}
		}
		//Redmine 40276
		//setLineStatuts(); //Redmine 68438
		//
		return true;
	}	//	beforeSave
	
	//Redmine 40276
	private boolean setLineStatuts(){
		MOrderLine ol = new MOrderLine(getCtx(), getC_OrderLine_ID(), get_Trx());
		if (ol.getQtyEntered().compareTo(getQtyEntered())!=0 && ol.getPriceEntered().compareTo(getPriceEntered())!=0)
			set_Value("Z_LineStatus", "03");
		else if (ol.getQtyEntered().compareTo(getQtyEntered())!=0)
			set_Value("Z_LineStatus", "01");
		else if (ol.getPriceEntered().compareTo(getPriceEntered())!=0)
			set_Value("Z_LineStatus", "02");
		if ((get_ValueAsBigDecimal("Z_CutOffAmt")).equals(new BigDecimal(0)))
			set_Value("Z_LineStatus", "04");
		return true;
	}
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if(getC_OrderLine_ID()>0)
		{
			String sql = "UPDATE C_Invoice i "
					+ "SET i.C_order_ID=(Select C_order_ID from C_orderLine where C_OrderLine_ID = ?)"
						+ "WHERE C_Invoice_ID=?";
			int no = DB.executeUpdate(sql, new Object[]{ getC_OrderLine_ID(),getC_Invoice_ID()}, get_Trx());
		}
	...
	protected boolean afterDelete (boolean success)
	{
		if (!success)
			return success;
		if (!getParent().isSOTrx() && getM_Product_ID() > 0 && getC_Charge_ID() == 0 ) 
		{
			//Reclaculer les charges avec un ListProductTax not null et présent sur la facture
			String sql = "Select * from C_invoiceline where C_Charge_ID is not null and exists (Select 1 from C_Charge cc where cc.C_Charge_ID = C_invoiceline.C_Charge_ID and ListProductTax is not null) and C_invoice_ID = ?";
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
				...
				while (rs.next())
				{
					MInvoiceLine mil = new MInvoiceLine(getCtx(), rs, get_Trx());
					MCharge mc = new MCharge(getCtx(), mil.getC_Charge_ID(), get_Trx());
					if(mc.get_Value("ListProductTax")==null)
						return updateHeaderTax();
					int ref_ID  = DB.getSQLValue(get_Trx(), "Select AD_Reference_ID from AD_Reference where Name = 'ListProductTax' ");
					String NameTax = "Select Name from AD_Ref_List where AD_Reference_ID = ? and Value = '"+mc.get_ValueAsString("ListProductTax")+"'";
					NameTax = DB.getSQLValueString(get_Trx(), NameTax, ref_ID);
					
					//Reclculer le montant 
					StringBuffer sqlX =new StringBuffer("Select Sum(QtyInvoiced*zpt.PriceStd) from c_invoiceline "
							+ "inner join Z_ProductTax zpt on zpt.M_Product_ID = c_invoiceline.M_Product_ID and UPPER(zpt.Name) = UPPER('"+NameTax+"')"
							+ " where C_invoice_ID =?");
					BigDecimal sum = DB.getSQLValueBD(get_Trx(), sqlX.toString(), getC_Invoice_ID());
					if(sum == null)
					{
						mil.delete(false, get_Trx());
						continue;
					}
					double sumtax = sum==null?0:sum.doubleValue();
					mil.setPrice(new BigDecimal(sumtax));
					mil.setLineNetAmt(new BigDecimal(sumtax)); // // qty always 1 ( for compute Tax)
					mil.setTaxAmt();
					mil.save(get_Trx());
				}
				rs.close();
	 ...
	 */
	public boolean updateHeaderTax()      // Make It Public for BatchMode
	{
		// Check/Add Recupel 
		if (!getParent().isSOTrx() && getM_Product_ID() > 0 && getC_Charge_ID() == 0 && !isImportInvoiceElectro)
				//&& getCtx().isBatchMode()) 
			InvoiceUtil.addModifyRecupelLine(getCtx(),getC_Invoice_ID(),getM_Product_ID(),getParent().getC_BPartner_ID(),0,null,true, get_Trx());	
...
//		if (getParent().get_ValueAsBigDecimal("escompte2")!=BigDecimal.ZERO)
//		{
//			BigDecimal escompte2=getParent().get_ValueAsBigDecimal("escompte2");
//			if(TotalLines!=Env.ZERO)TotalLines=TotalLines.subtract(escompte2);
//		String sql = "UPDATE C_Invoice i"
//			+ " SET TotalLines="+TotalLines
//			+ "WHERE C_Invoice_ID=?";
//		int no = DB.executeUpdate(sql, new Object[]{getC_Invoice_ID()}, get_Trx());
//		if (no != 1)
//			log.warning("(1) #" + no);
//		
//		}
//		else
//		{
//			BigDecimal discount =DB.getSQLValueBD(get_Trx(), "select pt.discount from c_paymentterm pt where pt.c_paymentterm_id = (select bp.PO_Paymentterm_Id from c_bpartner bp where bp.c_bpartner_id =( select c_bpartner_id from c_invoice where c_invoice_id=?))",getC_Invoice_ID());
//			if (discount!=null)
//			{
//				String sqlD = "UPDATE C_Invoice i"
//					+ " SET escompte2="+discount.multiply(TotalLines)
//					+ "WHERE C_Invoice_ID=?";
//				int no = DB.executeUpdate(sqlD, new Object[]{getC_Invoice_ID()}, get_Trx());
//				if (no != 1)
//					log.warning("(1) #" + no);
//				TotalLines=TotalLines.subtract(discount.multiply(TotalLines))	;
//			}
//			String sql = "UPDATE C_Invoice i"
//				+ " SET TotalLines="+TotalLines
//				+ "WHERE C_Invoice_ID=?";
      ...
      if (isTaxIncluded())
			...
			sql = "UPDATE C_Invoice i "
				+ "SET GrandTotal=TotalLines+"
					...
					+ "WHERE C_Invoice_ID=?";
		no = DB.executeUpdate(sql, new Object[]{getC_Invoice_ID()}, get_Trx());
		if (no != 1)
			log.warning("(2) #" + no);
		m_parent = null;
...
// JBR					TaxAmt = tax.calculateTax(LineNetAmt, isTaxIncluded(), StdPrecision);
					if (!getParent().isSOTrx() && getM_Product_ID() > 0) 
						TaxAmt = tax.calculateTax(getWoDiscount(LineNetAmt, getParent().getC_PaymentTerm_ID()), isTaxIncluded(), StdPrecision);
					else
			...
			String newM_Product_ID, int WindowNo) throws Exception
	{
		...
		if (!pp.isCalculated())
		{
			...
			p_changeVO.addError(Msg.getMsg(getCtx(), "ProductNotOnPriceList")+", ProductID : "+M_Product_ID);
			return;
	...
	//	mTab.setValue("Discount", pp.getDiscount());
		...
		setContext( WindowNo, "DiscountSchema", pp.isDiscountSchema() ? "Y" : "N");
		//
		//String IsCocontractant = DB.getSQLValueString(get_Trx(), "Select IsCocontractant from C_BPartner where C_BPartner_ID = ?", C_BPartner_ID);
		//if(IsCocontractant!=null)
			//set_Value("IsCocontractant", IsCocontractant);
	...
	private boolean setTax (int WindowNo, String columnName )
	{
		...
		//DEBUT Redmine #21247
		//ZCOM244 int M_Warehouse_ID = ctx.getContextAsInt( "#M_Warehouse_ID");
		int M_Warehouse_ID = MOrg.get(getCtx(), AD_Org_ID).getInfo().getM_Warehouse_ID(); //ZCOM244
		//FIN Redmine #21247

		//
		
		int C_Tax_ID = Tax.getTaxWithCocontractant(getCtx(),billC_BPartner_Location_ID,shipC_BPartner_Location_ID,AD_Org_ID,M_Product_ID,C_Charge_ID,
				billDate,shipDate,getParent().getC_DocTypeTarget_ID(),getCtx().isSOTrx(WindowNo),get_ValueAsBoolean("IsCocontractant"),M_Warehouse_ID);
		
//		int C_Tax_ID = Tax.get(getCtx(), M_Product_ID, C_Charge_ID, billDate, shipDate,
//			AD_Org_ID, M_Warehouse_ID, billC_BPartner_Location_ID, shipC_BPartner_Location_ID,
//			getCtx().isSOTrx( WindowNo )
//			/*ZCOMP35*/ 	, getParent().getC_DocTypeTarget_ID()		
//		);
	...
	@UICallout public void setAD_Client_ID (String oldAD_Client_ID,
			String newAD_Client_ID, int windowNo) throws Exception
	{
		MOrg org = MOrg.get(getCtx(),
				getParent().getAD_Org_ID());
		if (org.get_ValueAsBoolean("IsMag"))
			p_changeVO.setContext(getCtx(), windowNo,"IsMag", "Y");
		else
			p_changeVO.setContext(getCtx(), windowNo,"IsMag", "N");
	}
			...
			String newAD_Org_ID, int windowNo) throws Exception
	{
		...
		if( (newAD_Org_ID == null) || (newAD_Org_ID.length() == 0) || getM_Product_ID() <= 0)
			return;
		int AD_Org_ID = Integer.parseInt( newAD_Org_ID );
		if( AD_Org_ID == 0 )
			return;
		calculateTax(AD_Org_ID);
	}
	
	


	private void calculateTax(int AD_Org_ID) {
		
		MOrg org = MOrg.get(getCtx(), AD_Org_ID);//#43039
		if(getParent().isSOTrx()){
			int belgium_Coutry_ID = 103;
			...
			int C_Tax_ID = Tax.get(getCtx(), getM_Product_ID(), getC_Charge_ID() , getParent().getDateInvoiced(), getParent().getDateInvoiced(),
		...
		//#43039
			...
			System.out.println("*************************************************************InvoiceVendor");
...
//		if(!getParent().isSOTrx()){
		int M_Warehouse_ID = MOrg.get(getCtx(), getAD_Org_ID()).getInfo().getM_Warehouse_ID(); 

			int C_Tax_ID = Tax.getTaxWithCocontractant(getCtx(),getParent().getC_BPartner_Location_ID(),0, getAD_Org_ID(), getM_Product_ID(), getC_Charge_ID(),getParent().getDateInvoiced(),null,getParent().getC_DocTypeTarget_ID(), 
					getParent().isSOTrx(),newIsCocontractant.equals("Y")?true:false,M_Warehouse_ID);
...
//		}
	...
	private BigDecimal computeRecupel(int productID, int partnerID)
	{
		if (productID > 0 && partnerID > 0){
			...
			String ctry = DB.getSQLValueString(get_Trx(), sql.toString(), partnerID);
			if ("BE".equalsIgnoreCase(ctry) || "LU".equalsIgnoreCase(ctry)){
				//Boucle sur les taxes de l'article
				int ids[] = X_Z_ProductTax.getAllIDs("Z_ProductTax", " M_Product_ID = "+productID, get_Trx());
				for (int i = 0; i < ids.length; i++) {
					
				}
				sql = new StringBuffer("select pricestd from z_producttax where upper(value) = 'RECUPEL' and m_product_id = ? ");
				BigDecimal recupel = DB.getSQLValueBD(get_Trx(), sql.toString(), productID);
				if (recupel != null && recupel.compareTo(BigDecimal.ZERO) != 0) return recupel;
			}
		}
		return BigDecimal.ZERO;
	}

	public boolean isImportInvoiceElectro() {
		return isImportInvoiceElectro;
	}

	public void setImportInvoiceElectro(boolean isImportInvoiceElectro) {
		this.isImportInvoiceElectro = isImportInvoiceElectro;
	}

	public int getXxx_partner_ID() {
		return xxx_partner_ID;
	}

	public void setXxx_partner_ID(int xxx_partner_ID) {
		this.xxx_partner_ID = xxx_partner_ID;
	}

	public String getZZ_ProductTax() {
		return ZZ_ProductTax;
	}

	public void setZZ_ProductTax(String zZ_ProductTax) {
		ZZ_ProductTax = zZ_ProductTax;
	}
