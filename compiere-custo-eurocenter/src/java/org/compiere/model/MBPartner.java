...
public class MBPartner extends X_C_BPartner
{
	...
	public MUser[] getContacts (boolean reload)
	{
		...
		String sql = "SELECT * FROM AD_User WHERE C_BPartner_ID=? AND IsActive='Y' ORDER by IS_ZDEFAULT DESC, AD_User_ID DESC";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql, (Trx) null);
	 ...
	 */
	@UICallout public void setName (String oldName,
			String newName, int windowNo) throws Exception
	{
		if ((MInvoice.getOfBPartner(getCtx(), getC_BPartner_ID(), get_Trx())).length != 0 && !isVendor() )
			setName(oldName);  //  Never change this Value
	}
	...
	public MBPartnerLocation[] getLocations (boolean reload)
	{
		...
		String sql = "SELECT * FROM C_BPartner_Location WHERE C_BPartner_ID=? AND ISACtive='Y' ORDER BY C_BPartner_Location_ID DESC";
		PreparedStatement pstmt = null;
		try
		{
	...
	public void setTotalOpenBalance (
			boolean force  //ZCOM656		
	)
	{
		
		/*ZCOM656*/
		MMessage msg = MMessage.get(getCtx(),"AUDAXIS_BYPASS_BP_BALANCE_CALCUL");
		String txtMsg = null;
		if (! force && msg != null)
		{
			txtMsg = msg.getMsgText();
			if (! txtMsg.equalsIgnoreCase("FALSE")){
				return;
			}
		}
		/*ZCOM656*/
		...
		String sql = "SELECT "
			...
			+ "+(SELECT COALESCE(SUM(currencyBaseSpecific(a.Amount+a.DiscountAmt+a.WriteOffAmt,h.C_Currency_ID,h.DateAcct,p.C_ConversionType_ID,a.AD_Client_ID,a.AD_Org_ID)),0) "
 			...
 			// 88795 
 			+ "+COALESCE((SELECT SUM(currencyBaseSpecific(o.GrandTotal,o.C_Currency_ID,o.DateOrdered,COALESCE(o.C_ConversionType_ID, 0),o.AD_Client_ID,o.AD_Org_ID)) "
 				+ "FROM C_order o "
 				+ "inner join C_DOCTYPE cd on Cd.C_Doctype_ID = o.C_DocTypeTarget_ID " 
 				+ "WHERE o.C_BPartner_ID=bp.C_BPartner_ID "
 				+ "AND o.IsSOTrx  ='Y' "
 				+ "AND NOT EXISTS "
 				+ "(SELECT 1 FROM c_invoice i "
 				+ "WHERE o.AD_Client_ID  = i.AD_Client_ID "
 				+ "AND upper(o.poreference) = upper(i.poreference) "
 				+ "AND issotrx  = 'Y') "
 				+ "AND o.DOCSTATUS in ('IP','CO','CL') AND cd.DOCSUBTYPESO = 'SO' ),0), "
			
			//	debut SC1261: Rredmine 17961 Open Balance			= All Invoices
				+ "(COALESCE((SELECT SUM(Z_Invoice_Open_AsOf_Base(i.C_Invoice_ID,0,trunc(SYSDATE))) "
				+ "FROM C_Invoice_v i "
				+ "INNER JOIN C_Invoice ii ON (i.C_Invoice_ID = ii.C_Invoice_ID AND ii.Posted='Y')"
				+ "WHERE i.C_BPartner_ID=bp.C_BPartner_ID"
				+ " AND i.DocStatus IN('CO','CL', 'RE','VO') AND i.IsSoTrx='Y'),0) "
			+ "- COALESCE((SELECT SUM(Z_Invoice_Open_AsOf_Base(i.C_Invoice_ID,0,trunc(SYSDATE))) "
				+ "FROM C_Invoice_v i "
				+ "INNER JOIN C_Invoice ii ON (i.C_Invoice_ID = ii.C_Invoice_ID AND ii.Posted='Y')"
				+ "WHERE i.C_BPartner_ID=bp.C_BPartner_ID"
				+ " AND i.DocStatus IN('CO','CL', 'RE','VO') AND i.IsSoTrx='N'),0)) "
			//					- All Allocations
//			+ "-COALESCE((SELECT SUM(currencyBase(a.Amount+a.DiscountAmt+a.WriteOffAmt,i.C_Currency_ID,i.DateInvoiced,a.AD_Client_ID,a.AD_Org_ID)) "
//				+ "FROM C_AllocationLine a INNER JOIN C_Invoice i ON (a.C_Invoice_ID=i.C_Invoice_ID) "
//				+ "INNER JOIN C_AllocationHdr h ON (a.C_AllocationHdr_ID = h.C_AllocationHdr_ID) "
//				+ "WHERE a.C_BPartner_ID=bp.C_BPartner_ID AND a.IsActive='Y' AND h.DocStatus IN('CO','CL')),0) "
//			//					- Unallocated Receipts	= (All Receipts
				
			+ "- ((SELECT COALESCE(SUM(currencyConvert(zpaymentAvailable(p.C_Payment_ID,trunc(SYSDATE)),p.C_Currency_ID,(select a.C_Currency_ID from c_acctschema a where a.ad_client_id = p.ad_client_id),pp.DateAcct,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID)),0) " 
				+ "FROM C_Payment_v p "
				+ "INNER JOIN C_Payment pp ON (p.C_Payment_Id = pp.C_Payment_ID AND pp.Posted = 'Y')"
				+ "WHERE p.C_BPartner_ID=bp.C_BPartner_ID"
				+ " AND p.DocStatus IN('CO','CL', 'RE', 'VO') AND p.IsReceipt = 'Y'"
				+ " AND p.C_Charge_ID IS NULL)" 
			+ "- (SELECT COALESCE(SUM(currencyConvert(zpaymentAvailable(p.C_Payment_ID,trunc(SYSDATE)),p.C_Currency_ID,(select a.C_Currency_ID from c_acctschema a where a.ad_client_id = p.ad_client_id),pp.DateAcct,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID)),0) " 
				+ "FROM C_Payment_v p "
				+ "INNER JOIN C_Payment pp ON (p.C_Payment_Id = pp.C_Payment_ID AND pp.Posted = 'Y')"
				+ "WHERE p.C_BPartner_ID=bp.C_BPartner_ID"
				+ " AND p.DocStatus IN('CO','CL', 'RE', 'VO') AND p.IsReceipt = 'N'"
				+ " AND p.C_Charge_ID IS NULL))" 
			//											- All Allocations
//			+ "+(SELECT COALESCE(SUM(currencyBase(a.Amount+a.DiscountAmt+a.WriteOffAmt,i.C_Currency_ID,i.DateInvoiced,a.AD_Client_ID,a.AD_Org_ID)),0) "
//				+ "FROM C_AllocationLine a INNER JOIN C_Invoice i ON (a.C_Invoice_ID=i.C_Invoice_ID) "
//				+ "INNER JOIN C_AllocationHdr h ON (a.C_AllocationHdr_ID = h.C_AllocationHdr_ID) "
//				+ "WHERE a.C_BPartner_ID=bp.C_BPartner_ID"
//				+ " AND a.IsActive='Y' AND a.C_Payment_ID IS NOT NULL AND h.DocStatus IN('CO','CL')) "
			//FIN SC1261: Rredmine 17961
			+ "FROM C_BPartner bp "
			+ "WHERE C_BPartner_ID=?";
		PreparedStatement pstmt = null;
		try
	...
	{
		//85220
		if(get_Value("Z_GLN")!=null){
		 String sql = "select count(*) from C_BPartner where Z_GLN=? and AD_Client_ID=? and c_bpartner_id !=?";
		 String gln = get_Value("Z_GLN").toString();
		 ArrayList params = new ArrayList();
		 params.add(gln);
		 params.add(getAD_Client_ID());
		 params.add(getC_BPartner_ID());
		 int count = DB.getSQLValue(get_Trx(), sql, params);
		 if(count > 0){
			 String value = DB.getSQLValueString(get_Trx(), "SELECT VALUE FROM C_BPartner WHERE Z_GLN=? and AD_Client_ID="+getAD_Client_ID(), gln);
			 throw new CompiereException(Msg.getMsg(getCtx(), "Z_GLNExist")+value);
		 }
		}
		if(newRecord && get_ValueAsInt("I_PosOrderPartner_ID") <=0){
			set_Value("Last_AD_Org_ID",0);
		}
		...
		MPaymentTerm payterm=new MPaymentTerm (getCtx(),getPO_PaymentTerm_ID(),get_Trx());
		if (payterm!=null)
		{
			set_ValueNoCheck("NetDays", payterm.getNetDays());
			set_ValueNoCheck("Discount", payterm.getDiscount());
			set_ValueNoCheck("DiscountDays", payterm.getDiscountDays());
			set_ValueNoCheck("DiscountDays2", payterm.getDiscountDays2());
		
		}
		/*
		if(is_ValueChanged("Name")){
			setName(getName().toUpperCase().trim());
		}
		if(is_ValueChanged("Name2")){
			System.out.println("Name2 value changed");
			String name2New = "";
			String name2 = getName2().toLowerCase().trim();
			int Sbef = -1;
			int x = 0;
			if(name2!=null)
			{
				...
				if(!name2New.equals(name2))
					set_ValueNoCheck("Name2",name2New);
			}
		}
		*/
		return true;
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		if(!getCtx().isBatchMode())
		{
			...
			if (success && !newRecord  && is_ValueChanged("C_BP_Group_ID"))
			{
				...
				//104239 
				DB.executeUpdate(get_Trx(), "UPDATE C_Invoice set C_BP_GROUP_ID = ? where C_BPartner_ID = ? ", getC_BP_Group_ID(), getC_BPartner_ID());
				DB.executeUpdate(get_Trx(), "UPDATE C_Order set C_BP_GROUP_ID = ? where C_BPartner_ID = ? ", getC_BP_Group_ID(), getC_BPartner_ID());
			}
		}
		//	Value/Name change
		if (success && !newRecord 
		...
		if(success)
		{
			...
			{
				MGreeting mg = new MGreeting(getCtx(), getC_Greeting_ID(), get_Trx());
				if(mg.is2Pers())
				{
					log.saveWarning("Attention", Msg.getMsg(getCtx(), "CREATE_2_CONTACTS"));
				}
			}
			
			WSUtil.updateBPartner(getCtx(),this,newRecord,get_Trx());
		}
