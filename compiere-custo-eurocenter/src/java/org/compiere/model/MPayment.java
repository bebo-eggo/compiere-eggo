...
import com.audaxis.compiere.model.*;
...
//					|| ((getC_Project_ID() != 0) && (getC_Invoice_ID() == 0))));
		...
		if (is_ValueChanged("ZSubPaymentRule_ID") && (
						DOCSTATUS_Completed.equalsIgnoreCase(getDocStatus())
						|| DOCSTATUS_Closed.equalsIgnoreCase(getDocStatus())
						|| DOCSTATUS_Reversed.equalsIgnoreCase(getDocStatus())
						|| DOCSTATUS_Voided.equalsIgnoreCase(getDocStatus())
					)
				)
		{
			if (DocumentEngine.isPeriodOpen(this) != null)
				throw new CompiereException("@PeriodClosed@");

			String sql2 = "SELECT COUNT(*) FROM C_BankStatementLine WHERE C_Payment_ID=?";
			int no = DB.getSQLValue(get_Trx(), sql2, getC_Payment_ID());
			if (no>0)
				throw new CompiereException("Payment is on Bank Statement Line");
			
			if (isPosted())
			{
				String sql3 = "DELETE FROM FACT_ACCT WHERE Record_ID=? AND AD_Table_ID=?";
				DB.executeUpdate(get_Trx(), sql3, getC_Payment_ID(), MPayment.Table_ID);
				setPosted(false);
			}
		}
		
		boolean payAmtChange = false;
		MZSubPaymentRule msz = new MZSubPaymentRule(getCtx(),get_ValueAsInt("ZSubPaymentRule_ID"), get_Trx());
		if(msz.IsRounded() && !getPayAmt().equals(Env.ZERO) && !isFromEch())
		{
			log.warning("668, AMT : "+getPayAmt()); 
			BigDecimal PriceRounded = PaymentUtil.roundAmt(getPayAmt());
			log.warning("MPayment, 670, AMT : "+PriceRounded); 
	        if(getPayAmt().doubleValue()!=(PriceRounded.doubleValue()))
	        	payAmtChange = true;
	        setPayAmt(PriceRounded);
		}
		if(payAmtChange && !isFromEch() && p_changeVO!= null)
		{
	...
	private void setDocumentNo()
	{
		...
		//	If external number exists - enforce it   //#20840
...
//		if (documentNo.length() > 0)
//			setDocumentNo(documentNo); //#20840
	}	//	setDocumentNo
...
//				}
			...
			String newC_Order_ID, int windowNo) throws Exception
	{
		...
		//
//		setC_Invoice_ID(0);
		int C_Invoice_id = DB.getSQLValue(get_Trx(),"Select max(C_Invoice_ID) from C_Invoice where C_Order_id=?",C_Order_ID);
		if(C_Invoice_id > 0)
			setC_Invoice_ID(C_Invoice_id);
		else
			setC_Invoice_ID(0);
...
//		BigDecimal WriteOffAmt = super.getWriteOffAmt();
//		BigDecimal OverUnderAmt = super.getOverUnderAmt();
//		BigDecimal EnteredDiscountAmt = super.getDiscountAmt();
//		log.fine("Pay=" + PayAmt + ", Discount=" + EnteredDiscountAmt + ", WriteOff=" + WriteOffAmt + ", OverUnderAmt=" + OverUnderAmt);
//		//	Get Currency Info
...
//				setOverUnderAmt(Env.ZERO);
//		}
//		//  PayAmt - calculate write off or over under
//		else if (columnName.equals("PayAmt"))
//		{
//			if (isOverUnderPayment())
//			{
//				OverUnderAmt = InvoiceOpenAmt.subtract(PayAmt).subtract(EnteredDiscountAmt).subtract(WriteOffAmt);
//				setOverUnderAmt(OverUnderAmt);
//			}
//			else
//			{
...
//				setOverUnderAmt(OverUnderAmt);
//				WriteOffAmt = InvoiceOpenAmt.subtract(PayAmt).subtract(EnteredDiscountAmt);
//				setWriteOffAmt(WriteOffAmt);
//			}
//		}
//		else    //  calculate Amount
//		{
//			PayAmt = InvoiceOpenAmt.subtract(EnteredDiscountAmt).subtract(WriteOffAmt).subtract(OverUnderAmt);
//			setPayAmt(PayAmt);
//		}
//	}	//	checkAmt
	 ...
	 *	@param columnName columnName
	 */
	private void checkAmt (int windowNo, String columnName) throws Exception
	{
		if(CThreadUtil.isCalloutActive())
			return;

		if(get_ValueAsInt("C_BankAccount_ID") > 0)
		{
			MBankAccount bankAccount = new MBankAccount (getCtx(), get_ValueAsInt("C_BankAccount_ID"), null);
			if(bankAccount != null && getC_Currency_ID() > 0)
			{
		...
		if (getC_Invoice_ID() == 0)
		{
			checkAmtNoInvoice(windowNo, columnName);
			return;
		}
		
		if (columnName.equals("IsOverUnderPayment"))
		{
			if (! isOverUnderPayment())
				setOverUnderAmt(Env.ZERO);
		}
		...
		int C_InvoicePaySchedule_ID = 0;
		if ((getCtx().getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_Invoice_ID") == getC_Invoice_ID())
			&& (getCtx().getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_InvoicePaySchedule_ID") != 0))
			C_InvoicePaySchedule_ID = getCtx().getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_InvoicePaySchedule_ID");
		...
		String sql = "SELECT i.*, invoiceOpen(i.C_Invoice_ID,?) InvoiceOpenAmt, " +
				" DECODE(dt.IsReturnTrx, 'Y', -1*invoiceDiscount(i.C_Invoice_ID,?,?), invoiceDiscount(i.C_Invoice_ID,?,?)) DiscountAmt " +
				" FROM C_Invoice i INNER JOIN C_DocType dt ON (i.C_DocType_ID=dt.C_DocType_ID) WHERE C_Invoice_ID=?";
		try
		{
			ps = DB.prepareStatement(sql, (Trx) null);
			ps.setInt(1, C_InvoicePaySchedule_ID);
			ps.setTimestamp(2, getDateAcct());
			ps.setInt(3, C_InvoicePaySchedule_ID);
			ps.setTimestamp(4, getDateAcct());
			ps.setInt(5, C_InvoicePaySchedule_ID);
			ps.setInt(6, getC_Invoice_ID());
			rs = ps.executeQuery();
			if (rs.next())
			{
				invoice = new MInvoice(getCtx(), rs, null);
				
				InvoiceOpenAmt = rs.getBigDecimal("InvoiceOpenAmt");		//	Set Invoice Open Amount
				if (InvoiceOpenAmt == null)
					InvoiceOpenAmt = Env.ZERO;
				DiscountAmt = rs.getBigDecimal("DiscountAmt");
				if (DiscountAmt == null)
					DiscountAmt = Env.ZERO;
				boolean isReturnTrx = "Y".equalsIgnoreCase(rs.getString("IsReturnTrx"));
				if (isReturnTrx) // Adjust discount amount for credit memos. Invoice Open Amt is already adjusted.						
					DiscountAmt = DiscountAmt.negate();
			}
		}
		catch (SQLException e)
		...
		{
			com.audaxis.compiere.db.Util.closeCursor(ps, rs);
		}
		
		if (invoice == null)
		{
			p_changeVO.addError(Msg.getMsg(getCtx(), "NoInvoiceFound"));
		}
		else if (invoice.getC_Currency_ID() != getC_Currency_ID())
		{
			checkAmtMultiCurrency(windowNo, invoice, columnName, InvoiceOpenAmt, DiscountAmt);
		}
		else
		{
			checkAmtBase(windowNo, invoice, columnName, InvoiceOpenAmt, DiscountAmt);
		}
		
		return;
	}
	
	private void checkAmtBase(int windowNo, MInvoice invoice, String columnName, BigDecimal InvoiceOpenAmt, BigDecimal DiscountAmt) throws Exception
	{
		
		if (columnName.equals("C_Invoice_ID"))
		{
			resetAmt();
			setPayAmt(InvoiceOpenAmt);
			setDiscountAmt(DiscountAmt);
		}
		else if (columnName.equals("PayAmt") || columnName.equals("DiscountAmt") || columnName.equals("IsOverUnderPayment"))
		{
			// eventual differences on the correct field 
			if (isOverUnderPayment() && !columnName.equals("PayAmt"))
			{
				BigDecimal OverUnderAmt = InvoiceOpenAmt.subtract(getPayAmt()).subtract(getDiscountAmt()).subtract(getWriteOffAmt());
				setOverUnderAmt(OverUnderAmt);
			}
			else 
			{
				if(!invoice.isSOTrx())
				{
					BigDecimal WriteOffAmt = InvoiceOpenAmt.subtract(getPayAmt()).subtract(getDiscountAmt());
					setWriteOffAmt(WriteOffAmt);
				}
			}
		}
		else if (columnName.equals("WriteOffAmt"))
		{
			// eventual differences on the correct field 
			if (isOverUnderPayment())
			{
			...
			{
				setOverUnderAmt(Env.ZERO);
				BigDecimal PayAmt = InvoiceOpenAmt.subtract(getWriteOffAmt()).subtract(getDiscountAmt());
				setPayAmt(PayAmt);
			}
		}
		else if (columnName.equals("OverUnderAmt"))
		{
			...
			BigDecimal PayAmt = InvoiceOpenAmt.subtract(getWriteOffAmt()).subtract(getDiscountAmt()).subtract(getOverUnderAmt());
			setPayAmt(PayAmt);
		}
	...
	{
		if (getDiscountAmt().signum() != 0)
			setDiscountAmt(Env.ZERO);
		if (getWriteOffAmt().signum() != 0)
			setWriteOffAmt(Env.ZERO);
		if (getOverUnderAmt().signum() != 0)
			setOverUnderAmt(Env.ZERO);
		setIsOverUnderPayment(false);
	}

	private void checkAmtNoInvoice(int windowNo, String columnName) throws Exception
	{
		resetAmt();
	}
	private void checkAmtMultiCurrency(int windowNo, MInvoice invoice, String columnName, BigDecimal InvoiceOpenAmt, BigDecimal DiscountAmt) throws Exception
	{
		MCurrency currency = MCurrency.get(getCtx(), getC_Currency_ID());
		
		BigDecimal payCurrencyRate = MConversionRate.getRate (invoice.getC_Currency_ID(), getC_Currency_ID(), getDateAcct(), getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
		if ((payCurrencyRate == null) || (payCurrencyRate.compareTo(Env.ZERO) == 0))
		{
		...
		BigDecimal invCurrencyRate = MConversionRate.getRate (invoice.getC_Currency_ID(), getC_Currency_ID(), invoice.getDateAcct(), getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
		if ((invCurrencyRate == null) || (invCurrencyRate.compareTo(Env.ZERO) == 0))
		{
			if (invoice.getC_Currency_ID() != 0)
				p_changeVO.addError(Msg.getMsg(getCtx(), "NoCurrencyConversion"));
			return;
		...
		//
		BigDecimal InvoiceOpenAmtAsOf = InvoiceOpenAmt.multiply(invCurrencyRate).setScale(currency.getStdPrecision(), BigDecimal.ROUND_HALF_UP);
		InvoiceOpenAmt = InvoiceOpenAmt.multiply(payCurrencyRate).setScale(currency.getStdPrecision(), BigDecimal.ROUND_HALF_UP);
		DiscountAmt = DiscountAmt.multiply(payCurrencyRate).setScale(currency.getStdPrecision(), BigDecimal.ROUND_HALF_UP);
		//
		
		if (columnName.equals("C_Invoice_ID") || columnName.equals("DateAcct") || columnName.equals("C_Currency_ID") || columnName.equals("C_ConversionType_ID"))
		{
			resetAmt();

			setPayAmt(InvoiceOpenAmt);
			setDiscountAmt(DiscountAmt);
			BigDecimal WriteOffAmt = InvoiceOpenAmtAsOf.subtract(InvoiceOpenAmt).subtract(DiscountAmt);
			setWriteOffAmt(WriteOffAmt);
		}
		else if (columnName.equals("PayAmt") || columnName.equals("DiscountAmt"))
		{
				BigDecimal WriteOffAmt = InvoiceOpenAmt.subtract(getPayAmt()).subtract(getDiscountAmt()).subtract(getOverUnderAmt());
				setWriteOffAmt(WriteOffAmt);
		}
		else if (columnName.equals("WriteOffAmt") || columnName.equals("OverUnderAmt"))
		{
			BigDecimal PayAmt = InvoiceOpenAmtAsOf.subtract(getWriteOffAmt()).subtract(getDiscountAmt()).subtract(getOverUnderAmt());
			setPayAmt(PayAmt);
		}
	}
...
//			if(mo.getC_DocType_ID() == 1001462)
//			{
//				//Type d'ordre = Vente directe !
//				MOrderVDPayment movdp = new MOrderVDPayment(getCtx(), 0, get_Trx());
//				movdp.setAD_Org_ID(mo.getAD_Org_ID());
//				movdp.setC_Order_ID(mo.getC_Order_ID());
//				movdp.setPayAmt(getPayAmt());
//				movdp.setC_Payment_ID(getC_Payment_ID());
//				movdp.setC_BankAccount_ID(getC_BankAccount_ID());
//				movdp.setZSubPaymentRule_ID(get_ValueAsInt("ZSubPaymentRule_ID"));
//				movdp.setDateTrx(getDateTrx());	
//				movdp.save(get_Trx());
//			}
//		}
//		else {
...
//				{
//			MOrder mo = new MOrder(getCtx(), getC_Order_ID(), get_Trx());
//			if(mo.getC_DocType_ID() == EggoCtx.TYPEDOC_CONTRATSTORE_ID ||  mo.getC_DocType_ID() == EggoCtx.VENTE_EXPO_ID)
//			{
//				Object[] params = new Object[]{ new Integer( mo.getAD_Client_ID()) , new Integer(mo.getC_Order_ID()) , new Integer(mo.getC_Invoice_ID())};  
//				int schedID = DB.getSQLValue(get_Trx(), "select Z_OrderPaymSchedule_id from Z_OrderPaymSchedule where ad_client_id = ? and c_order_id = ? and c_invoice_id = ? ",params);
//				if (schedID >= 1)
//				{
//					MOrderPayment mopay = new MOrderPayment(getCtx(), 0, get_Trx());
//					mopay.setClientOrg(mo.getAD_Client_ID(), mo.getAD_Org_ID());
//					mopay.setZ_OrderPaymSchedule_ID(schedID);
//					mopay.setPayAmt(getPayAmt());
//					mopay.setC_Payment_ID(getC_Payment_ID());
//					mopay.setC_BankAccount_ID(getC_BankAccount_ID());
//					mopay.setZSubPaymentRule_ID(get_ValueAsInt("ZSubPaymentRule_ID"));
//					mopay.setDateTrx(getDateTrx());	
//					mopay.save(get_Trx());
//				}
//			}
//		}	
...
//		
		...
		if(isOrderAllocate){
			...
			allocFlow.paymentAllocate(getC_Payment_ID());
		}
	...
	private boolean isFromEch = false;

	public boolean isFromEch() {
		return isFromEch;
	}

	public void setisFromEch(boolean isFromEch) {
		this.isFromEch = isFromEch;
	}
}   //  MPayment
