...
public class MInventory extends X_M_Inventory implements DocAction
{
	...
	public void logStorageEvent(MInventoryLine line,BigDecimal Qty){
		// BBR : SYNCHRO STOCK
		MDocType dt = MDocType.get(getCtx(), getC_DocType_ID());
		if(dt.isZ_IsStorageEvent()){
			X_Z_StorageEvent ev = new X_Z_StorageEvent(getCtx(), 0, get_Trx());
			ev.setAD_Client_ID(line.getAD_Client_ID());
			ev.setAD_Org_ID(line.getAD_Org_ID());
			ev.setM_Locator_ID(line.getM_Locator_ID());
			ev.setM_Product_ID(line.getM_Product_ID());
			ev.setM_AttributeSetInstance_ID(line.getM_AttributeSetInstance_ID());
			ev.setQtyType(MStorageDetail.QTYTYPE_OnHand);
			ev.setQty(Qty);
			ev.set_Value("Z_IsInventory",false);
			ev.save();
		}
		// BBR : SYNCHRO STOCK
	}
	...
	{
		//DELETE from M_INventoryLine where QTyBook=QtyCount=0 Attention à QtyInternalUse !
		String sql = "DELETE FROM M_INVENTORYLINE where M_INVENTORY_ID = ? and QtyCount=0 and QtyBook = 0 and QtyInternalUse = 0";
		int no = DB.executeUpdate(sql, getM_Inventory_ID(), get_Trx());
		log.fine(no+" records no transaction");
		...
		for (MInventoryLine line : lines) {
			...
			if (line.getM_AttributeSetInstance_ID() == 0)
			{
				...
				//
				BigDecimal qtyNew = BigDecimal.ZERO;
				if (qtyDiff.signum() > 0)
				{
					//	Storage
					MStorageDetail storageOnHand = Storage.getCreateDetails(getCtx(), line.getM_Locator_ID(),
							line.getM_Product_ID(), 0, X_Ref_Quantity_Type.ON_HAND, get_Trx());
					qtyNew = storageOnHand.getQty().add(qtyDiff);
				...
				else	//	negative qty
				{
					...
					for (MInventoryLineMA ma : mas) {
						...
						qtyNew = ma.getMovementQty().add(maxDiff);	//	Storage+Diff
