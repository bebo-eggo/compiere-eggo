...
//			nox = isInOutLineExists(getM_InOutLine_ID(), getC_OrderLine_ID());
//		if(nox>0)
//			setM_InOutLine_ID(-1);
		return true;
	}	//	beforeSave
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (success && (getC_OrderLine_ID() != 0))
		{
			...
			if (m_isInOutLineChange)
			{
				...
				{
					//Anomalie #64108
					String sql = "SELECT il.qtyentered FROM M_inoutline il " +
							"INNER JOIN m_inout i ON i.m_inout_id    = il.m_inout_id " +
							"WHERE i.docstatus IN ('IP') AND il.C_orderline_id   = ? ";
					BigDecimal qty = DB.getSQLValueBD(get_Trx(), sql, orderLine.getC_OrderLine_ID());
...
//		Timestamp orderDate = null;
		...
		if (getC_InvoiceLine_ID() != 0)
		{
			try
			{
				//ZCOM999 pstmt = DB.prepareStatement(sql, (Trx) null);
				pstmt = DB.prepareStatement(sql, get_Trx()); //ZCOM999
	...
	protected boolean beforeDelete ()
	{
		...
		{
			//REDMINE 27378 
			//il faut definir l'organisation du document pour la selection de la periode, si non on aura un NullPointerExeption(voir l'mplementation de  MPeriod.isOpen)
			ArrayList<Integer> orgs = new ArrayList<Integer>();
			orgs.add(getAD_Org_ID());
			String msg = MPeriod.isOpen(getCtx(), getAD_Client_ID(), orgs, getDateAcct(), MDocBaseType.DOCBASETYPE_MatchPO);
			if (msg != null)
			{
				log.warning(msg);
				return false;
	...
	public int isInOutLineExists(int M_InOutline_ID, int C_OrderLine_ID)
	{
		return DB.getSQLValue(null, "Select count(M_MATCHPO_ID) from M_MatchPO where M_INOUTLINE_ID = ? and C_orderLine_ID = ?", M_InOutline_ID, C_OrderLine_ID );
	}
