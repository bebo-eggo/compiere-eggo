...
public class MPaySelectionLine extends X_C_PaySelectionLine
{
	...
	{
		try {
			if (newPayAmt != null && newPayAmt.length() != 0 && (! "Y".equals(getCtx().getContext(windowNo, "FromDiscountAmt")))&&(!"Y".equals(getCtx().getContext(windowNo,"FromC_invoice"))))
				
			{
				...
				setDiscountAmt(Env.ZERO);
				//67534
				getCtx().setContext(windowNo,"FromPayAmt", "Y");			
			}
		}
		finally {
			getCtx().setContext(windowNo,"FromDiscountAmt", "NULL");
			getCtx().setContext(windowNo,"FromC_invoice", "NULL");	
		}
		//67534
	}	//	setPayAmt
	/**
	 * 	Set Discount Amt - Callout
	 *	@param oldDiscountAmt old value
	 *	@param newDiscountAmtnew value
	 *	@param windowNo window
	 *	@throws Exception
	 */
	...
	//67534
		...
		{
			try {
				if (newDiscountAmt != null && newDiscountAmt.length() != 0 && (! "Y".equals(getCtx().getContext(windowNo,"FromPayAmt"))) && (!"Y".equals(getCtx().getContext(windowNo, "FromC_invoice"))))
				{
					...
					setPayAmt(PayAmt);
					getCtx().setContext(windowNo,"FromDiscountAmt", "Y");			
				}	
			}
			finally {
				getCtx().setContext(windowNo,"FromPayAmt", "NULL");
			}
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		MPaySelection psel = new MPaySelection(getCtx(), getC_PaySelection_ID(), get_Trx());	// EG60
        ...
        if  ( leaveToSel.compareTo(getPayAmt()) < 0 ) throw new CompiereException("Montant maximum de la sélection atteind");
	...
	private void setHeader()
	{
		...
		String sql = "UPDATE C_PaySelection ps "
			+ "SET TotalAmt = (SELECT COALESCE(SUM(psl.PayAmt),0) "
				...
				+ "Z_TotalDiscount = (SELECT COALESCE(SUM(psl.DiscountAmt),0) "//#67680
				+ "FROM C_PaySelectionLine psl "
				+ "WHERE ps.C_PaySelection_ID=psl.C_PaySelection_ID AND psl.IsActive='Y') "
