...
public class MPInstance extends X_AD_PInstance
{
	...
	/** ZCOM999 / #39171 Unicité d'exécution d'un processus*/
	...
	public MPInstancePara[] getParameters()
	{
		...
		//
		String sql = "SELECT * FROM AD_PInstance_Para WHERE AD_PInstance_ID=? order by seqNo";
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (!newRecord
			...
			&& !isLocked  	/** ZCOM999 / #39171 Unicité d'exécution d'un processus*/
			)
		{
		...
		/*
		if(is_ValueChanged("IsProcessing") && !isProcessing())
		{
			if(prc.get_ValueAsBoolean("isEventLog"))
			{
				DateFormat dateFormat = new SimpleDateFormat();  
				dateFormat = new SimpleDateFormat("dd/MM/yyyy");
				java.util.Date date = new java.util.Date();
				//Timestamp acctDate = new Timestamp(getCtx().getContextAsTime("#Date"));
				String date1 = dateFormat.format(date);
				MChat mc = null;
				int chat_ID = DB.getSQLValue(get_Trx(), "Select Max (CM_CHAT_ID) from CM_CHAT where AD_Table_ID = ? and Record_ID = ?  ", get_Table_ID(), getRecord_ID());
				if(chat_ID<=0){
					mc = new MChat(getCtx(), chat_ID, get_Trx());
					mc.setRecord_ID(getRecord_ID());
					mc.setAD_Table_ID(get_Table_ID());
					mc.setAD_Org_ID(0);
					mc.setConfidentialType("A");
					mc.setDescription(get_Table_ID() + "#" + getRecord_ID());
					mc.save();
				}else
					mc = new MChat(getCtx(), chat_ID, get_Trx());
				String data = "Date : "+date1+"/n"+"Processus : "+prc.getName()+"/n";
				MPInstancePara mpa [] = getParameters();
				for (int i = 0; i < mpa.length; i++) {
					data = data+mpa[i].getSeqNo()+":"+mpa[i].getInfo()+(mpa[i].getInfo_To()!=null?"-"+mpa[i].getInfo_To():"")+"/n";
				}
				
				MChatEntry mce = new MChatEntry(mc, data);
				mce.save();
			}
		}
		*/
