...
import com.audaxis.compiere.db.QueryUtilPO;
...
import com.audaxis.compiere.pos.process.POSAllocationFlow;
...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	public static MInvoice copyFrom (MInvoice from, Timestamp dateDoc,
			...
			boolean counter, Trx trx, boolean setOrder, boolean setInOut)
	{
	...
	public MInvoice (Ctx ctx, int C_Invoice_ID, Trx trx)
	{
		...
		if (C_Invoice_ID == 0)
		{
			...
			set_ValueNoCheck("escompte2",Env.ZERO);
			//
			setIsSOTrx (true);
			...
			setProcessing(false);
			set_ValueNoCheck("C_ArcoValidator_ID",null);
			set_ValueNoCheck("Z_ValidationInvoiceID",null);
			set_ValueNoCheck("Z_UrlARCO",null);
		}
	}	//	MInvoice
	...
	public void setOrder (MOrder order)
	{
		...
		// setC_PaymentTerm_ID(order.getC_PaymentTerm_ID());
		// setPOReference(order.getPOReference());
		if (getC_PaymentTerm_ID() <= 1) setC_PaymentTerm_ID(order.getC_PaymentTerm_ID()); // To avoid copyfrom erase
		if (getPOReference() == null || getPOReference().isEmpty() ) setPOReference(order.getPOReference());
	...
	public int copyLinesFrom (MInvoice otherInvoice, boolean counter, boolean setOrder, boolean setInout)
	{
		...
		for (MInvoiceLine fromLine : fromLines) {
			...
			if (getC_BPartner_ID() != otherInvoice.getC_BPartner_ID() 
					|| getC_DocTypeTarget_ID() != otherInvoice.getC_DocTypeTarget_ID())
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
			//Be sure that IsTaxIncluded = IsTaxIncluded from PriceList
			String IsTaxIncluded = DB.getSQLValueString(get_Trx(), "SELECT IsTaxIncluded FROM M_PriceList WHERE M_PriceList_ID=?", getM_PriceList_ID());
			if(IsTaxIncluded!=null)
				setIsTaxIncluded(IsTaxIncluded.equals("Y")?true:false);
		}
		...
		{
			MBPartner mb = new MBPartner(getCtx(), getC_BPartner_ID(), get_Trx());
			int ii = mb.getC_PaymentTerm_ID();
			if (ii != 0)
				setC_PaymentTerm_ID (ii);
			else
			{
				...
				if (ii != 0)
					...
					ii = getCtx().getContextAsInt( "#C_PaymentTerm_ID");
						if (ii != 0)
							setC_PaymentTerm_ID (ii);
				}
			}
		...
		/** Redmine 62511 : remplcer par une nouvelle logique en dessous
		//Evolution #37152 Modification écran "Maintenance facture fournisseur" => PO reference dans communication
		if (!isSOTrx())
		{
			if (get_ValueAsString("Z_PaymentCommunication").isEmpty() && get_ValueAsString("Z_PaymentCommunicationStr").isEmpty())
			{
				if (getPOReference()!= null && !getPOReference().isEmpty())
					set_ValueNoCheck("Z_PaymentCommunication", getPOReference());
			}
		}
		**/
		
		//Redmine 62511
		if (!isSOTrx() && (is_ValueChanged("POReference") || is_ValueChanged("Z_PaymentCommunication"))){
			if(getZ_PaymentCommunicationStr() == null)
				setZ_PaymentCommunication(getPOReference());
		}
		if(getZ_PaymentCommunicationStr()!= null)
			setZ_PaymentCommunication(null);
		//
		
		//Evolution #39119 EC-SFD-SMILE_773-Gestion_escompte V1.0
		String sql = "SELECT PO_PaymentTerm_ID FROM C_BPartner WHERE C_BPartner_ID=?";
		int PO_PaymentTerm_ID = DB.getSQLValue(null, sql, getC_BPartner_ID());
		
		if(PO_PaymentTerm_ID!=0 && getC_PaymentTerm_ID()<=0)
			setC_PaymentTerm_ID(PO_PaymentTerm_ID);
		
		if (getC_PaymentTerm_ID()>0)
		{
			MPaymentTerm payterm=new MPaymentTerm (getCtx(),getC_PaymentTerm_ID(),get_Trx());		
			Calendar cal = Calendar.getInstance();
			Timestamp dateInvoiced = getDateInvoiced();
			cal.setTime(dateInvoiced);
			cal.add(Calendar.DATE,payterm.get_ValueAsInt("NetDays"));
			Timestamp DueDate=new Timestamp(cal.getTimeInMillis());
			if(get_Value("DueDate") == null)//#49149
				set_ValueNoCheck("DueDate", DueDate);
		
			if (payterm.get_ValueAsInt("DiscountDays") > 0) {
				cal.setTime(dateInvoiced);
				cal.add(Calendar.DATE,payterm.get_ValueAsInt("DiscountDays"));
				Timestamp DiscountDate=new Timestamp(cal.getTimeInMillis());
				set_ValueNoCheck("DiscountDate",DiscountDate);
			}
			else if(get_Value("DiscountDate") == null)//#49149
				set_ValueNoCheck("DiscountDate",DueDate); 
		}

// 		NO !!!		
//		if(is_ValueChanged("escompte2"))		
//		{   BigDecimal TotalLines=DB.getSQLValueBD(get_Trx(), "SELECT COALESCE(SUM(LineNetAmt),0) FROM C_InvoiceLine il WHERE il.C_Invoice_ID=?",getC_Invoice_ID());
//		    BigDecimal GrandTotalLines=DB.getSQLValueBD(get_Trx(), "SELECT COALESCE(SUM(TaxAmt),0) FROM C_InvoiceTax it WHERE it.C_Invoice_ID=?",getC_Invoice_ID());
//		  if (!TotalLines.equals(Env.ZERO))
//		  {  if(new MPriceList(getCtx(), getM_PriceList_ID(), get_Trx()).isTaxIncluded())
//			setGrandTotal(TotalLines.subtract(get_ValueAsBigDecimal("escompte2")));	  		
//			else
//			setGrandTotal((TotalLines.add(GrandTotalLines)).subtract(get_ValueAsBigDecimal("escompte2")));
//			setTotalLines(TotalLines.subtract(get_ValueAsBigDecimal("escompte2")));
//		  }
//		}
		//#39119
		...
		/* */
		 ...
		 //#75603
//		if( !isSOTrx() && "A".equalsIgnoreCase((String) get_Value("Z_ActionARCO")))
//			set_Value("Z_ApprovedInv", true);//#75603
		if (getC_Order_ID() > 0 && isSOTrx())
		{
			MOrder mo = new MOrder(getCtx(), getC_Order_ID(), get_Trx());
			if(mo.get_Value("Z_RefChantier")!=null)
				set_Value("Z_RefChantier", mo.get_Value("Z_RefChantier"));
			if(mo.get_Value("PROM_ExtCode")!=null)
				set_Value("PROM_ExtCode", mo.get_Value("PROM_ExtCode"));
		}
		return true;
	}	//	beforeSave
	...
	{
		if(get_Value("Z_Com") == null || get_Value("Z_ComID") == null)
			calculateCommunication();//Redmine 19228 & 34898 & 35334
		...
		if(isPosted())
		{
			WSUtil.updateInvoice(getCtx(), MInvoice.get(getCtx(),get_ID()), false, get_Trx());
		}
		
		return true;
	}	//	afterSave
	
	//Redmine 19228 & ZCOM 999 35334
	private void calculateCommunication() {
		if (!isSOTrx())
			return;
		...
		String str_C_BPartenr_ID = String.valueOf(getC_BPartner_ID());
		String str_C_Invoice_ID = String.valueOf(getC_Invoice_ID());
		String modulo;
		
		StringBuffer zcomid, zcom;
		
		zcomid = new StringBuffer(str_C_BPartenr_ID.subSequence(str_C_BPartenr_ID.length()-3, str_C_BPartenr_ID.length()));
		zcomid.append(str_C_Invoice_ID);
		
		BigInteger numValue = new BigInteger(zcomid.toString());
		BigInteger[] results = numValue.divideAndRemainder(new BigInteger("97"));
		
		if (results[1].intValue()==0)
			modulo = "97";
		else{
			if (results[1].toString().length()==1)
				modulo = "0"+results[1].toString();
			else
				modulo = results[1].toString();
		}
		
		zcomid.append(modulo);
		
		zcom = new StringBuffer("+++").append(str_C_BPartenr_ID.subSequence(str_C_BPartenr_ID.length()-3, str_C_BPartenr_ID.length()));
		zcom.append("/").append(str_C_Invoice_ID.subSequence(0, 4));
		zcom.append("/").append(str_C_Invoice_ID.subSequence(str_C_Invoice_ID.length()-3, str_C_BPartenr_ID.length()));
		zcom.append(modulo).append("+++");
		
		set_Value("Z_Com", zcom.toString());
		set_Value("Z_ComID", zcomid.toString());
		
		int no = DB.executeUpdate("UPDATE C_Invoice SET Z_Com='"+zcom.toString()+"', Z_ComID='"+zcomid.toString()+"' WHERE C_Invoice_ID=?", getC_Invoice_ID(), get_Trx());
		s_log.info("UPDATED ROW= "+no);
	}
	...
	public boolean calculateTaxTotal()
	{
		log.fine("");
		//	Delete Taxes
		if( 0 > DB.executeUpdate("DELETE FROM C_InvoiceTax WHERE C_Invoice_ID=?", new Object[]{getC_Invoice_ID()}, get_Trx())){
			return false;
		}

		Object[][] taxIDs = QueryUtil.executeQuery("SELECT DISTINCT C_Tax_ID FROM C_InvoiceLine WHERE C_Invoice_ID = ? AND C_Tax_ID IS NOT NULL", new Object[]{getC_Invoice_ID()}, get_Trx());
		//	Lines
		...
		Object[][] sumLineNet = QueryUtil.executeQuery("SELECT SUM(LineNetAmt) FROM C_InvoiceLine WHERE C_Invoice_ID = ?", new Object[]{getC_Invoice_ID()}, get_Trx());
	...
	public String completeIt()
	{
		...
		if(!isSOTrx() && getTotalLines().equals(BigDecimal.ZERO)){   //36501
			m_processMsg = Msg.getMsg(getCtx(), "TotlaInvoiceIsZero");
			return DocActionConstants.STATUS_Invalid;					
		}				//36501
		...
		if (0 > DB.executeUpdate(sql, new Object[] { getC_Invoice_ID(), getC_Invoice_ID(), isSOTrx() ? "Y" : "N" },
				get_Trx())) {
		...
		if(0 > DB.executeUpdate(sql, new Object[] { getC_Invoice_ID()}, get_Trx()) ){
		...
		if (!isSOTrx() && !isReversal()) {
			...
			for (MInvoiceLine line : lines) {
				...
				if (line.getC_OrderLine_ID() != 0 && line.getM_Product_ID() != 0) {
					...
					// before Shipment
						...
						MMatchPO po = MMatchPO.create(line, line.getM_InOutLine_ID()!=0 ?new MInOutLine(getCtx(), line.getM_InOutLine_ID(), get_Trx()):null, getDateInvoiced(), matchQty);
						matchPOs.add(po);
			...
			for (MInvoiceLine line : lines) {
				...
				if (line.getM_InOutLine_ID() != 0 && line.getM_Product_ID() != 0) {
					//#Anomalie #91863
						MInOutLine line_out= new MInOutLine(getCtx(),line.getM_InOutLine_ID(), get_Trx());
						MInOut shipment= new MInOut(getCtx(),line_out.getM_InOut_ID(),get_Trx());
						s_log.severe("shipment in treating invoice"+shipment.getDocStatus());
		...
		// Calculate Escompte
		if (!isSOTrx() && getC_PaymentTerm_ID()>0)
		{
			BigDecimal discount =DB.getSQLValueBD(get_Trx(), "select  pt.discount from c_paymentterm pt where pt.c_paymentterm_id = ?",getC_PaymentTerm_ID());
			if (discount!=null)
			{
				BigDecimal escompte2 = discount.multiply(getTotalLinesOnlyProduct()).setScale(2, BigDecimal.ROUND_HALF_UP);
				set_Value("escompte2", escompte2);
			}
		}
...
//		EggoCtx.setConstants();
		if(!isSOTrx()  )
		{
			int m_Process_ID = DB.getSQLValue(get_Trx(), "select max(ad_process_id) from ad_process where classname like '%InvoicePOBarCode%' "); 
			if (m_Process_ID > 1) {
				MProcess process = new MProcess(getCtx(), m_Process_ID, get_Trx());
				MPInstance instance = new MPInstance(getCtx(), m_Process_ID, -1);
				if (!instance.save()) {
					m_processMsg = "Could print BarCode (no class for %InvoicePOBarCode% )";
					return DocActionConstants.STATUS_Invalid;
				}
				ProcessInfo processInfo = new ProcessInfo("Print Invoice PO Barcode",
						m_Process_ID);
				processInfo.setAD_PInstance_ID(instance.getAD_PInstance_ID());
				List<ProcessInfoParameter> pipList = new ArrayList<ProcessInfoParameter>();
				ProcessInfoParameter pip1 = new ProcessInfoParameter("C_Invoice_ID", get_ID(),
						null, null, null);
				pipList.add(pip1);
				...
				if (processInfo.isError()) {
					m_processMsg = "Could print BarCode (Unable to execute Process ) ";
					return DocActionConstants.STATUS_Invalid;
				}
			}
		}
		...
		if(isOrderAllocate){
			POSAllocationFlow allocFlow = new POSAllocationFlow(getCtx(), get_Trx());
			allocFlow.invoiceAllocate(getC_Invoice_ID());
		}
		//WSUtil.updateInvoice(getCtx(), this, false, get_Trx());
		
		m_processMsg = info.toString().trim();
		return DocActionConstants.STATUS_Completed;
	}	//	completeIt

	private BigDecimal getTotalLinesOnlyProduct() {
		// TODO Auto-generated method stub
		BigDecimal totalLines = BigDecimal.ZERO;
		MInvoiceLine[] invLines = getLines(" AND M_Product_ID > 0");
		for (int i = 0; i < invLines.length; i++) {
			totalLines = totalLines.add(invLines[i].getLineNetAmt());
		}
		return totalLines;
	}
	...
	private MInvoice createCounterDoc()
	{
		...
		MInvoice counter = copyFrom(this, getDateInvoiced(),
				C_DocTypeTarget_ID, true, get_Trx(), false, false);
		MDocType dt = MDocType.get(getCtx(), C_DocTypeTarget_ID);
	...
	public boolean voidIt()
	{
				...
				|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			// Reset OrderPaymSchedule
			DB.executeUpdate("UPDATE Z_OrderPaymSchedule set c_invoice_id = null where c_invoice_id = ? ", get_ID(), get_Trx());
		}
		else
		{
	...
	{
		log.info(toString());
		Timestamp newInvoiceDate = new Timestamp(GregorianCalendar.getInstance().getTimeInMillis());
		...
		if(isSOTrx())//70047
		{
			if(!DateUtils.isSameDay(getDateAcct(), getDateInvoiced()))
			{
				Timestamp oldDateAcct = getDateAcct();
				setDateAcct(getDateInvoiced());
				String reversalPeriod = DocumentEngine.isPeriodOpen(this);
				setDateAcct(oldDateAcct);//restore date
				if (reversalPeriod != null && reversalPeriod.contains("@PeriodClosed@") && m_processMsg == null)
				{
					newFact = true;
				}
			}
		...
		//	Deep Copy
		MInvoice reversal = copyFrom (this, newInvoiceDate,
				getC_DocType_ID(), false, get_Trx(), true, true);
		reversal.setDateAcct(newInvoiceDate);reversal.setDateInvoiced(newInvoiceDate);
		if (reversal == null)
		{
		...
		if (!isCash)
		{
			MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, newInvoiceDate,
					getC_Currency_ID(),
	...
	public void setC_PaymentTerm_ID( String oldC_PaymentTerm_ID, String newC_PaymentTerm_ID, int windowNo ) throws Exception
	{
		...
		if ( C_PaymentTerm_ID == 0 )	//	not saved yet
			return;
		//
		...
		{
		String sqlOrg = "Select paymentTermDueDate(?, ?) from dual";
		
		//	Use newest price list - may not be future
		try
		{
			PreparedStatement pstmt = DB.prepareStatement(sqlOrg, (Trx) null);
			pstmt.setInt(1, C_PaymentTerm_ID);
			pstmt.setTimestamp(2, getDateInvoiced());
			ResultSet rs = pstmt.executeQuery();
			if (rs.next())
			{
				set_ValueNoCheck("DueDate", rs.getTimestamp(1));
		...
		{
			log.log(Level.SEVERE, sqlOrg, e);
		}
		}
		
		//62531
		if(getDateInvoiced() != null && !isSOTrx() && (getDocStatus() != null && getDocStatus().equalsIgnoreCase("DR")))
		{
	        //
			Calendar cal = Calendar.getInstance();
			cal.setTime(getDateInvoiced());
			cal.add(Calendar.DATE,pt.get_ValueAsInt("NetDays"));
		
			cal.setTime(getDateInvoiced());
			
			if (pt.get_ValueAsInt("DiscountDays") > 0)
				cal.add(Calendar.DATE,pt.get_ValueAsInt("DiscountDays"));
			else
				cal.setTime(DueDate==null?get_ValueAsTimestamp("DueDate"):DueDate);
				
			set_ValueNoCheck("DiscountDate", cal.getTime());
		}
		//62531
		return;
	...
	@UICallout public void setC_DocTypeTarget_ID (String oldC_DocTypeTarget_ID,
		...
		//#75603
		if (!isSOTrx()) {
			MDocType doc = new MDocType(getCtx(), C_DocTypeTarget_ID, get_Trx() );
			set_Value("Z_ActionARCO",doc.get_Value("Z_ActionARCO"));
		}//#75603
	...
	@UICallout public void setC_BPartner_ID (String oldC_BPartner_ID,
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID,"
			...
			+ " p.SO_CreditLimit, p.SO_CreditLimit-p.SO_CreditUsed AS CreditAvailable, p.Z_ApprovedInv, "
			+ " l.C_BPartner_Location_ID,c.AD_User_ID,"
			+ " COALESCE(p.PO_PriceList_ID,g.PO_PriceList_ID) AS PO_PriceList_ID, p.PaymentRulePO,p.PO_PaymentTerm_ID, p.ZSubPaymentRulePO_ID "
			+ "FROM C_BPartner p"
			+ " INNER JOIN C_BP_Group g ON (p.C_BP_Group_ID=g.C_BP_Group_ID)"
			+ " LEFT OUTER JOIN C_BPartner_Location l ON (p.C_BPartner_ID=l.C_BPartner_ID AND l.IsBillTo='Y' AND l.IsActive='Y')"
			+ " LEFT OUTER JOIN AD_User c ON (p.C_BPartner_ID=c.C_BPartner_ID) "
			+ "WHERE p.C_BPartner_ID=? AND p.IsActive='Y' order by l.ISEGGO DESC ";		//	#1
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if(proxy==null || proxy.getM_PriceList_ID()<=0)
				{
					 ii= Integer.valueOf(rs.getInt(IsSOTrx ? "M_PriceList_ID" : "PO_PriceList_ID"));
					if (!rs.wasNull())
						setM_PriceList_ID( ii);
					else
				...
				//	Approved. Invoice Redmine 40276
				s = rs.getString("Z_ApprovedInv");
				if (s != null && s.length() != 0)
					set_Value("Z_ApprovedInv", s);
				//
				//62531
				if(!isSOTrx() && rs.getInt("ZSubPaymentRulePO_ID") > 0 && (getDocStatus() != null && getDocStatus().equalsIgnoreCase("DR")))
				{
					set_ValueNoCheck("ZSubPaymentRule_ID", rs.getInt("ZSubPaymentRulePO_ID"));
				}
				//62531
			}
			rs.close();
		...
		// #39174 EC-SFD-SMILE_73 Ventilation et refacturation
        if (!IsSOTrx)
		{
		String sqlOrg = "SELECT IsRefact, IsMag from AD_Org where AD_Org_ID=?";
		//	Use newest price list - may not be future
		try
		{
			PreparedStatement pstmt = DB.prepareStatement(sqlOrg, (Trx) null);
			...
			if (rs.next())
			{
                setContext( WindowNo, "IsRefact", rs.getString(1));
				setContext( WindowNo, "IsMag", rs.getString(2));
			}
			rs.close();
			pstmt.close();
     ...
     // #39174 EC-SFD-SMILE_73 Ventilation et refacturation
		...
		checkBPPOReference(WindowNo, "C_BPartner_ID");
//        
//		sql = "SELECT PO_PaymentTerm_ID FROM C_BPartner WHERE C_BPartner_ID=?";
//		int PO_PaymentTerm_ID = DB.getSQLValue(null, sql, C_BPartner_ID);
//		if (PO_PaymentTerm_ID!=0)
//		{
//			MPaymentTerm payterm=new MPaymentTerm (getCtx(),PO_PaymentTerm_ID,get_Trx());		
//			Calendar cal = Calendar.getInstance();
//			cal.add(Calendar.DATE,payterm.get_ValueAsInt("NetDays"));
//			
//		}
	...
	@UICallout 
	public void setPOReference (String oldPOReference,
			String newPOReference, int windowNo) throws Exception {
		
		if (newPOReference == null || newPOReference.length() == 0)
			return;
		
		checkBPPOReference(windowNo, "POReference");
	...
	@UICallout public void setDateInvoiced (String oldDateInvoiced ,
		...
		if(getC_DocTypeTarget_ID()>0 && !isSOTrx())
		{
			...
			MDocType md = new MDocType(getCtx(), getC_DocTypeTarget_ID(), get_Trx());
			String test = MPeriod.isOpen(getCtx(), getAD_Client_ID(), orgs, dateInvoiced, md.getDocBaseType());
			Timestamp dt = dateInvoiced;
			int nob = 0;
			int tt = 0;
			Timestamp dateDay = new Timestamp (System.currentTimeMillis ());
			while(test!=null && nob<6)
			{
				nob++;
				Calendar cal = Calendar.getInstance();
				cal.setTime(dt);
				...
				dt = new Timestamp(cal.getTimeInMillis());
				test = MPeriod.isOpen(getCtx(), getAD_Client_ID(), orgs, dt, md.getDocBaseType());
			}
			if(test==null)
				setDateAcct(dt);
			else
		...
		if (PO_PaymentTerm_ID!=0)
		{
		MPaymentTerm payterm=new MPaymentTerm (getCtx(),getC_PaymentTerm_ID()>0?getC_PaymentTerm_ID():PO_PaymentTerm_ID,get_Trx());		
		Calendar cal = Calendar.getInstance();
		cal.setTime(dateInvoiced);
		cal.add(Calendar.DATE,payterm.get_ValueAsInt("NetDays"));
		//set_ValueNoCheck("DueDate", cal.getTime());
	...
	public boolean equals(Object o)
	{
	 ...
	 /** Set Communication.
    @param Z_PaymentCommunication Communication that is reported on electronic fund transfert */
    public void setZ_PaymentCommunication (String Z_PaymentCommunication)
    {
        set_Value ("Z_PaymentCommunication", Z_PaymentCommunication);
	
    }
    //Evolution #47750 : 380 - Facture achat - Validation com strd
    /** Get Communication.
    @return Communication that is reported on electronic fund transfert */
    public String getZ_PaymentCommunication() 
    {
        return (String)get_Value("Z_PaymentCommunication");

    }
    
    /** Set Communication structurée.
    @param Z_PaymentCommunicationStr Communication structurée */
    public void setZ_PaymentCommunicationStr (String Z_PaymentCommunicationStr)
    {
        set_Value ("Z_PaymentCommunicationStr", Z_PaymentCommunicationStr);
        
    }
    
    /** Get Communication structurée.
    @return Communication structurée */
    public String getZ_PaymentCommunicationStr() 
    {
        return (String)get_Value("Z_PaymentCommunicationStr");

    } //#47750
    
	private boolean isOrderAllocate = false;

	public boolean isOrderAllocate() {
		return isOrderAllocate;
	}

	public void setOrderAllocate(boolean isOrderAllocate) {
		this.isOrderAllocate = isOrderAllocate;
	}
	
	//Redmine 62517
	private void checkBPPOReference(int WindowNo, String columnName){
		int C_BPartner_ID = getC_BPartner_ID();
		String POReference = getPOReference();
		
    if (!isSOTrx() && POReference!=null && C_BPartner_ID != 0){
    	if (columnName.equals("C_BPartner_ID"))
    		C_BPartner_ID = getCtx().getContextAsInt( WindowNo, "C_BPartner_ID");
    	if (columnName.equals("POReference"))
    		POReference = getCtx().getContext( WindowNo, "POReference");
    	
        Iterable<MInvoice> invoices = QueryUtilPO.executeQueryPOs(getCtx(), get_Trx(), 
        		"SELECT * FROM C_Invoice WHERE ISSOTRX='N' AND POReference = ? AND POReference IS NOT NULL "
        		+ "AND C_BPartner_ID = ? AND C_Invoice_ID <> ? AND C_Invoice_ID <> 0", 
        		MInvoice.class, POReference, C_BPartner_ID, getC_Invoice_ID());
  
        String msg = "";
        for (MInvoice inv : invoices) {
			msg +=inv.getDocumentNo() + ", ";
		}
        if (msg.length()>0){
        	msg = msg.substring(0, msg.length()-2);
        	p_changeVO.addWarning(Msg.getMsg(getCtx(), "EC_DUPLICAT_POREFERENCE") + msg);
        }
    }
	}
	//#75603
	@UICallout
	public void setZ_ActionARCO(String oldZ_ActionARCO,
			String newZ_ActionARCO, int windowNo) throws Exception
			{
		if( !isSOTrx() && "A".equalsIgnoreCase(newZ_ActionARCO))
			set_Value("Z_ApprovedInv", true);
