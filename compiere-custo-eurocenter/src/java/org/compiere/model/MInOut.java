...
public class MInOut extends X_M_InOut implements DocAction
{
	 ...
	 */
	public MInOut (MOrder order, int C_DocTypeShipment_ID,
			Timestamp movementDate ,int AD_org_ID)
	{
		this (order.getCtx(), 0, order.get_Trx());
		setOrder(order);
		setAD_Org_ID(AD_org_ID);
		MOrgPOS orgPos = MOrgPOS.getOrgPos(getCtx(), AD_org_ID, get_Trx());
		setM_Warehouse_ID(MLocator.get(getCtx(), orgPos.getM_LocatorStock_ID()).getM_Warehouse_ID());
		if (C_DocTypeShipment_ID == 0)
			C_DocTypeShipment_ID = DB.getSQLValue(null,
					"SELECT C_DocTypeShipment_ID FROM C_DocType WHERE C_DocType_ID=?",
					order.getC_DocType_ID());
	...
	{
		//setClientOrg(order);
		setAD_Client_ID(order.getAD_Client_ID());
			...
			String newC_Order_ID, int windowNo) throws Exception
	{
		...
		//p_changeVO.setContext(getCtx(), windowNo, "AD_Org_ID", order.getAD_Org_ID());
		if (order.get_ID() != 0)
			setOrder(order);
		if(!isSOTrx())
		{
			...
			MLocator[] mls  = MLocator.getWithWhereClause(getCtx(), getM_Warehouse_ID(), 
				...
				String Role_Exped_93 = Msg.getMsg(getCtx(), "Z_ROLE_ONLY_93_RECP");
		
				if(Role_Exped_93.contains(""+Role_ID) && mls.length>0 &&  !isSOTrx())
				{
					//Force stock 01
					int warehouse_ID = DB.getSQLValue(get_Trx(), "Select max( m_warehouse_ID) from m_warehouse where  Value like '%-01%' and AD_Org_ID = ? ", getAD_Org_ID());
					if(warehouse_ID>0)
						setM_Warehouse_ID(warehouse_ID);
				}
		}
	}	//	setC_Order_ID
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		String roles = Msg.getMsg(getCtx(), "ROLES_NO_ORDER_INOUT");
		if(!roles.contains(""+getCtx().getAD_Role_ID()) && !isSOTrx() && getC_Order_ID() <= 0 && getDocStatus().equals("DR"))
		{
		...
		if(Role_Exped_93.contains(""+Role_ID) && mls !=null && mls.length>0 &&  !isSOTrx())
		{
			//Message d'erreur
			log.saveError("Save Error", Msg.getMsg(getCtx(), "Z_ROLE_ONLY_93_MSG"));
			return false;
		}
		
		String NotAbleToRec = Msg.getMsg(getCtx(), "XX_ROLE_NOT_01_RECP");
		if(NotAbleToRec.contains(""+getCtx().getAD_Role_ID()) && !isSOTrx() && !isFromOV)
		{
			MWarehouse mw = new MWarehouse(getCtx(), getM_Warehouse_ID(), get_Trx());
			if(mw!=null && mw.getValue().contains("-01") )
			{
				String msg = Msg.getMsg(getCtx(), "XX_ROLE_NOT_01_RECP_M");
				//m_processMsg  = msg;
	...
	public String prepareIt()
	{
		...
		NotAbleToRec = Msg.getMsg(getCtx(), "XX_ROLE_NOT_01_EXP");
		if(NotAbleToRec.contains(""+getCtx().getAD_Role_ID()) && isSOTrx() )
		{
			...
			String sql = "Select able_exped from ( " + 
					...
					"                     when mpc.xx_role_able_exped='Y' then 'Y' else 'N' end) as able_exped, mil.M_iNout_ID from m_inoutline mil  " + 
					...
					"where M_iNOut_ID = ? " + 
					"group by able_exped, M_inOut_ID " + 
					"order by able_exped ASC ";
			
			String notProcess = org.compiere.util.QueryUtil.getSQLValueString(get_Trx(), sql, getM_InOut_ID());
		...
		Iterable<Record> lines = QueryUtil.executeQuery(
				...
				"AND ((COALESCE(iol.M_AttributeSetInstance_ID,0) !=0 AND mas.isGuaranteeDateMandatory = 'Y' AND masi.GuaranteeDate IS NULL) " +
				 ...
				 "OR  (COALESCE(iol.M_AttributeSetInstance_ID,0) = 0 AND ((IsSOTrx = 'Y' AND mas.MandatoryType!='N') OR (IsSOTrx='N' AND mas.MandatoryType='Y')))) ", 
			new Callback<Record>() {
				@Override
			...
			}, new Object[] { getM_InOut_ID() }, get_Trx());
		...
		Object[][] volume_weight = QueryUtil.executeQuery(
				"SELECT SUM(P.Volume * L.MovementQty), SUM(P.Weight * L.MovementQty) "+
	...
	public boolean updateOrderLine(MInOutLine sLine, Map<Integer, MOrderLine> m_mapOLines, MProduct product) {
		...
		if(!isReturnTrx() && product != null && product.isStocked()) {
			...
			BigDecimal QtyReserved = oLine.getReservedQty();
			...
			if(dt.isZ_IsStorageEvent()){
				...
				ev.setM_Product_ID(oLine.getM_Product_ID());
			...
			if ((product != null) && product.isStocked() )
			{
				...
				if (sLine.getM_AttributeSetInstance_ID() == 0)
				{
					...
					for (MInOutLineMA ma : mas) {
						...
						if(dt.isZ_IsStorageEvent()){
							X_Z_StorageEvent ev = new X_Z_StorageEvent(getCtx(), 0, get_Trx());
							ev.setAD_Client_ID(sLine.getAD_Client_ID());
							ev.setAD_Org_ID(sLine.getAD_Org_ID());
							ev.setM_Locator_ID(sLine.getM_Locator_ID());
							ev.setM_Product_ID(sLine.getM_Product_ID());
							ev.setM_AttributeSetInstance_ID(0);
							ev.setQtyType(MStorageDetail.QTYTYPE_OnHand);
							ev.setQty(QtyMA);
							ev.save();
						}
						// BBR : SYNCHRO STOCK
				...
				if (mtrx == null)
				{
					...
					if(dt.isZ_IsStorageEvent()){
						...
						ev.setM_Locator_ID(sLine.getM_Locator_ID());
						ev.setM_Product_ID(sLine.getM_Product_ID());
						ev.setM_AttributeSetInstance_ID(sLine.getM_AttributeSetInstance_ID());
						ev.setQtyType(MStorageDetail.QTYTYPE_OnHand);
						ev.setQty(Qty);
						ev.save();
					}
	...
	public String completeIt()
	{
		...
		if(0 > DB.executeUpdate(sql, new Object[] {getM_InOut_ID()}, get_Trx())) {
...
//				if (asi.save())
//				{
//					if(0 > DB.executeUpdate("UPDATE M_InOutLine SET M_AttributeSetInstance_ID=? " +
...
//		if( 0 > DB.executeUpdate( "UPDATE M_StorageDetail S SET QtyAllocated = QtyAllocated - "
...
//			return Msg.getMsg(getCtx(), "CannotReverseQtyAllocated");
		...
		if( 0 > DB.executeUpdate( "UPDATE C_OrderLine OL SET QtyAllocated = QtyAllocated - "
		...
		if( 0 > DB.executeUpdate( "UPDATE M_InOutLine L SET QtyAllocated = 0"
									+ " WHERE L.M_InOut_ID = ? " 
	...
	public boolean voidIt()
	{
				...
				|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			if( 0 > DB.executeUpdate( "UPDATE M_InOutLine L SET QtyEntered=0, MovementQty= 0,  "
	...
	public boolean reverseCorrectIt(MOrder order)
	{
		...
		MDocType dt = MDocType.get(getCtx(), getC_DocType_ID());
		//
		boolean newFact = false;//Evolution #71000
		Timestamp now = new Timestamp(GregorianCalendar.getInstance().getTimeInMillis());//Evolution #71000
		
		m_processMsg = DocumentEngine.isPeriodOpen(this);
		if (m_processMsg != null)
			newFact  = true; //Evolution #71000
		...
		//	Deep Copy
		MInOut reversal = copyFrom (this,  newFact?now:getMovementDate(),
