 ...
 */
 public class MBankStatementLine extends X_C_BankStatementLine
 {
	...
	/**
	 * 	Set TrxAmt - Callout
	 *	@param oldTrxAmt old value
	 *	@param newTrxAmt new value
	 *	@param windowNo window
	 *	@throws Exception
	 */
	@UICallout public void setTrxAmt (String oldTrxAmt, 
			String newTrxAmt, int windowNo) throws Exception
	{
		if(CThreadUtil.isCalloutActive())
			return;
		
		if (newTrxAmt == null || newTrxAmt.length() == 0)
			return;
		BigDecimal TrxAmt = new BigDecimal(newTrxAmt);
		super.setTrxAmt(TrxAmt);
		
		setAmt(windowNo, "TrxAmt");
	...
	private void setAmt (int windowNo, String columnName)
	{
		...
		//  Calculate Charge
		//69411
		else if(columnName.equals("TrxAmt"))
		{
			setChargeAmt(bd);
		}
		//69411
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getC_Payment_ID() != 0)
		{
			...
			set_Value("Pay_SubPaymentRule_ID", payment.get_Value("ZSubPaymentRule_ID"));
			//	Set References
	...
	private void updateHeader()
	{
		...
		sql = "UPDATE C_BankStatement bs"
			+ " SET EndingBalance=BeginningBalance+StatementDifference "
				+ ", CashAmount=BeginningBalance+StatementDifference " //Redmine 43450
			+ "WHERE C_BankStatement_ID=" + getC_BankStatement_ID();
		DB.executeUpdate(sql, get_Trx());
	}	//	updateHeader
