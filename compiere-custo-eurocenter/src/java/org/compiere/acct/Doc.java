...
import com.audaxis.compiere.eurocenter.util.AcctUtil;
import com.audaxis.compiere.eurocenter.ws.WSUtil;

/**
 *  Posting Document Root.
 *  @author Jorg Janke
 *  @version  $Id: Doc.java 8746 2010-05-11 13:15:04Z ragrawal $
 */
public abstract class Doc implements AccountingInterface
{
			...
			int AD_Table_ID, int Record_ID, boolean force, Trx trx)
	{
		...
		if(trx==null)
		{
			trx = Trx.get(AD_Table_ID+ "|" +Record_ID);
			isCloseTrx = true;
		}//#50749
		Doc doc = get (ass, AD_Table_ID, Record_ID, trx);
		if (doc != null)
		{
			String result = doc.post (force, true);	//	repost
			//#50749 : commiter la transaction
			if(isCloseTrx)
			{
				trx.commit();
				trx.close();
	...
	/** B Partner	    			*/
	protected int					m_C_BPartner_ID = -1;
	...
	public boolean isAN() {
		...
		if (getC_Period_ID() > 0)
		{
			MPeriod period = MPeriod.get(p_po.getCtx(), getC_Period_ID());
			if (period != null)
				m_IsAN = period.get_ValueAsBoolean("IsAN");//63043
		}
	...
	public final String post (boolean force, boolean repost)
	{
		...
		//ZCOM #24740 Eviter les deadlocks
		StringBuffer sql2 = new StringBuffer ("SELECT Processing FROM ");
		sql2.append(get_TableName()).append( " WHERE ")
			.append(get_TableName()).append("_ID=?")
			.append(" AND Processed='Y'"); // AND IsActive='Y'");
		if (!force)
			sql2.append(" AND (Processing='N' OR Processing IS NULL)");
		if (!repost)
			sql2.append(" AND Posted='N'");
		sql2.append(" FOR UPDATE NOWAIT");
		try {
			DB.executeUpdate(getTrx(), sql2.toString(), new Object[]{get_ID()});
		} catch (CompiereSQLException e) {
			// On a un Lock, on sort
			log.log(Level.SEVERE, "Deadlock", e);
			return "Document is locked at DB "+get_TableName()+" " + get_TableName() +"_ID = "+get_ID();
		}
		//ZCOM #24740
		...
		if (DB.executeUpdate(sql.toString(), new Object[]{get_ID()}, getTrx()) == 1)	//	outside p_trx
		...
		if(p_Status.equals(STATUS_Posted))
		{
			AcctUtil.FactAcctToPositiveProcess2(getCtx(), p_po.get_ID(), p_po.get_Table_ID(), getTrx());
			getTrx().commit();

			if(MInvoice.Table_Name.equals(get_TableName()))
				WSUtil.updateInvoice(getCtx(), MInvoice.get(getCtx(),get_ID()), false, getTrx());
		}
	...
	private int deleteAcct()
	{
		...
		int no = DB.executeUpdate(sql, new Object[]{get_Table_ID(), p_po.get_ID()}, getTrx());
		if (no != 0)
			log.info("#=" + no);
		return no;
	}	//	deleteAcct
	...
	private void unlock()
	{
		...
		sql.append(get_TableName()).append( " SET Processing='N' WHERE ")
			.append(get_TableName()).append("_ID=?");
		//DB.executeUpdate(sql.toString(), new Object[]{p_po.get_ID()}, (Trx) null);		//	outside p_trx
		DB.executeUpdate(sql.toString(), new Object[]{p_po.get_ID()}, getTrx()); //#50749
	}   //  unlock
	...
	public int getValidCombination_ID (int AcctType, MAcctSchema as)
	{
		...
		else if (AcctType == ACCTTYPE_OverhdAbsorptionAcct)
		{
			sql = " SELECT COALESCE(wcc.Overhead_Absorption_Acct, b.Overhead_Absorption_Acct) "
				...
				+ " WHERE wcc.M_WorkCenter_ID = ? AND wcc.M_CostElement_ID = " + m_costelement_id 
				+ " AND wcc.C_AcctSchema_ID=? "
				+ " AND wcc.M_CostType_ID = " + as.getM_CostType_ID();
			para_1 = m_workcenter_id;
		}
		else
		{
	...
	private final boolean save(Trx trx)
	{
		...
		int no = DB.executeUpdate(sql.toString(), params, trx);
		return no == 1;
	}   //  save
	/**
	 *  Get DocLine with ID
	...
	/**
	 * ZCOM999 Backward compatibility 3.3.0
	 * @return
	 */
