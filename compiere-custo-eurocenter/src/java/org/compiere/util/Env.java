...
public final class Env extends EnvConstants
{
	...
	public static void reset (boolean finalCall)
	{
		...
		//ZCOM837 closeHiddenWindows();
		
	       //ZCOM837           
	       if (!finalCall)
	       {
	           //le temp c'est pour sauvegarder la fenetre Menu seulement aprés cache reset
	           s_temp.clear();
	           int j=0;
	           for(int i=0; i< s_windows.size(); i++)
	           {
	              if (s_windows.get(i) != null)
	              {
	                  Container win = (Container)s_windows.get(i);
	                  if (win.getName().equals("Menu"))
	                      j=i;
	              }
	           }
	           if (j < s_windows.size()) {
	              s_temp.add(s_windows.get(j));
	              s_temp.set(0,s_windows.get(j));
	           }
	       }
	       s_windows.clear();
	       //ZCOM837
		...
		//BBR : TEST CONSTANT EC
		//PosCtx.setConstants(getCtx(), null);
		//EggoCtx.setConstants(getCtx().getAD_Client_ID());
		//BBR : TEST CONSTANT EC
		...
		if (!finalCall)
		{
			...
			//ZCOM837
	           if (s_temp != null && s_temp.size() > 0) {
	               s_windows.add(s_temp.get(0));
	               s_windows.set(0,s_temp.get(0));
	           }
	        //ZCOM837
			...
			if(!shutdownHookAlreadyAdded)
				Runtime.getRuntime().addShutdownHook(new Thread() {
					...
					public void run() {
						...
						// RESET AD_PINSTANCE ISPROCESSING=Y
						Trx trx =  Trx.get("ShuttingDown(CleanInstance");
						int no = DB.executeUpdate("UPDATE AD_PInstance SET IsProcessing='N' where IsProcessing='Y'",trx);
						s_log.info(no+" Processes killed");
						trx.commit();
						trx.close();
						// RESET AD_PINSTANCE ISPROCESSING=Y
	...
	private static ArrayList<Container> s_temp = new ArrayList<Container>(1); //ZCOM837
