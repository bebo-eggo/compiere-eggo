	...
	implements ActionListener, ChangeListener
{
	...
	public static ArrayList< FieldVO > getPriceTabFieldVOs( Ctx ctx, boolean isProduct )
	{
		...
		fieldVOs.add( new FieldVO( "ISO_CODE", Msg.translate( ctx, "C_Currency_ID" ), DisplayTypeConstants.String ) );//Redmine 30580
	...
	private void initPriceTab()
	{
		...
		//
		int i = 0;
		m_tablePrice.setColumnClass(i++, String.class, true);      //  Product/Partner
		m_tablePrice.setColumnClass(i++, Double.class, true);  	 //  Price
		m_tablePrice.setColumnClass(i++, Double.class, true);      //  Quantity
		m_tablePrice.setColumnClass(i++, BigDecimal.class, true);  //  Discount (%) to limit precision
		m_tablePrice.setColumnClass(i++, String.class, true);      //  DocNo
		m_tablePrice.setColumnClass(i++, Timestamp.class, true);   //  Date
		m_tablePrice.setColumnClass(i++, String.class, true);   	 //  Org
		//
	...
	public static Vector<Vector<Object>> queryBPartner ( int m_M_Product_ID )
	{
		String sql = "SELECT bp.Name,l.PriceActual,l.PriceList,l.QtyInvoiced,"		//	1,2,3,4
			...
			+ "o.Name "
			+ ", c.ISO_CODE "//Redmine 30580
			+ "FROM C_Invoice i"
			+ " INNER JOIN C_InvoiceLine l ON (i.C_Invoice_ID=l.C_Invoice_ID)"
			+ " INNER JOIN C_DocType dt ON (i.C_DocType_ID=dt.C_DocType_ID)"
			+ " INNER JOIN AD_Org o ON (i.AD_Org_ID=o.AD_Org_ID)"
			+ " INNER JOIN C_BPartner bp ON (i.C_BPartner_ID=bp.C_BPartner_ID) "
			+ " INNER JOIN C_Currency c ON (i.C_Currency_ID=c.C_Currency_ID) "//Redmine 30580
	...
	private static Vector<Vector<Object>> fillTable (String sql, int parameter, int parameter2)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getString("ISO_CODE"));// Currency - Redmine 30580
	...
	public static ArrayList< FieldVO > getReservedOrderedTabFieldVOs( Ctx ctx, boolean isProduct, boolean reserved )
	{
		...
		fieldVOs.add( new FieldVO( "PriceActual", Msg.translate( ctx, "PriceActual" ), DisplayTypeConstants.Amount ) );
		//52370
		fieldVOs.add( new FieldVO( "C_Currency_ID", Msg.translate( ctx, "C_Currency_ID" ), DisplayTypeConstants.String ) ); // Currency - Redmine 30580
		///
																			...
																			int m_M_Warehouse_ID)
	{
		String sql = "SELECT bp.Name, ol.PriceActual,ol.PriceList,ol.QtyReserved,"
				...
				+ " INNER JOIN C_Currency c ON (o.C_Currency_ID=c.C_Currency_ID) "//Redmine 30580
...
//			+ " AND o.IsSOTrx=" + (reserved ? "'Y'" : "'N'");
		...
		String sql =  " SELECT bp.Name, "
				...
				+ "   ol.PriceList, "
				+ "   case  "
				+ "   when AD_OrgInfo.ISPOS = 'Y' " //MERGE
				+ "     then ol.QtyInProcess "
				+ "     else ol.QtyReserved "
				+ "   end "
				+ "   as QtyReserved, "
				+ "   o.DateOrdered, "
				...
				+ "   w.Name, c.ISO_CODE "//Redmine 30580 "
				+ " FROM C_Order o "
				+ " LEFT join AD_OrgInfo on AD_OrgInfo.AD_Org_ID = o.AD_Org_ID " //MERGE
				+ " INNER JOIN C_Currency c ON (o.C_Currency_ID=c.C_Currency_ID) "//Redmine 30580
				...
				+ " ON (ol.M_Warehouse_ID=w.M_Warehouse_ID) "
				+ " INNER JOIN C_BPartner bp "
				+ " ON (o.C_BPartner_ID  =bp.C_BPartner_ID) "
				+ " WHERE"  
				+ "  ( "
				+ "        ( ol.QtyInProcess<>0 and AD_OrgInfo.ISPOS = 'Y' and o.IsSoTrx = 'Y') "//MERGE
				+ "     or ( ol.QtyReserved<>0 and  AD_OrgInfo.ISPOS <> 'Y' )  "//MERGE
				+ "  ) "
				+ " AND o.C_BPartner_ID=?"
				+ " AND o.IsSOTrx=" + (reserved ? "'Y'" : "'N'");
	...
	private void initReservedOrderedTab (boolean reserved)
	{
		...
		//
		int i=0;
		table.setColumnClass(i++, String.class, true);      //  Product/Partner
