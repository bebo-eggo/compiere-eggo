...
public final class AEnv
{
	...
	public static void zoom (int AD_Table_ID, int Record_ID)
	{
		...
		if (PO_Window_ID != 0)
		{
			...
			//ZCOM999 si PO Window alors false trou de balle AD_Window_ID=ZoomTarget.getZoomAD_Window_ID(TableName, 0, whereClause, true);
			AD_Window_ID=ZoomTarget.getZoomAD_Window_ID(TableName, 0, whereClause, false); //ZCOM999
	...
	//**	Window Cache		*//*
	private static final CacheBase s_windows 
		= new CacheBase("GridWindowVO",CacheType.MEDIUM);
	...
	public static GridWindowVO getMWindowVO (int WindowNo, int AD_Window_ID, int AD_Menu_ID)
	{
		...
		{
			mWindowVO = (GridWindowVO) s_windows.get(AD_Window_ID);
			if (mWindowVO != null)
			{
		...
		int AD_Table_ID, int Record_ID, boolean force)
	{
		...
		String error = Doc.postImmediate(ass, AD_Table_ID, Record_ID, 
				true,null);
