...
public class Doc_Allocation extends Doc
{
	 ...
	 * 	@param trxName trx
	 */
	public Doc_Allocation (MAcctSchema[] ass, ResultSet rs, Trx trxName)
	{
		super (ass, MAllocationHdr.class, rs, MDocBaseType.DOCBASETYPE_PaymentAllocation, trxName);
	}   //  Doc_Allocation

	/**	Tolearance G&L				*/
	protected static final BigDecimal	TOLERANCE = new BigDecimal (0.02);
	/** Facts						*/
	protected ArrayList<Fact>		m_facts = null;

	protected Timestamp m_ZDateDoc; //ZCOM205
	...
	public String loadDocumentDetails()
	{
		...
		setDateDoc(alloc.getDateAcct()); //ZCOM999 remplate DateTrx
		//	Contained Objects
		p_lines = loadLines(alloc);
		
		/*ZCOM205** Truc de vincent ici : pour les taux de change on a besoin de la date
		 * de paiement, dans ce cas la logique de posting est correcte ==> on prend cette date
		 * et on la met sur l'allocation
		 * on poste
		 * A la fin on reset la date sur les ecritures postees
		 * ... et la periode
		 */
		/*ZCOM205** START */
		...
		ResultSet rs2 = null;
		try {
			// TODO Ce n'est pas parfait car on prend le PREMIER paiement sur l'allocation...
			String sql = "SELECT C_Payment_ID FROM C_AllocationLine WHERE C_AllocationHdr_ID=?";
			ps = DB.prepareStatement(sql, null);
			ps.setInt(1, get_ID());
			rs2 = ps.executeQuery();
			rs2.next();
			int m_C_Payment_ID = rs2.getInt("C_Payment_ID");
			if (m_C_Payment_ID != 0){
				MPayment payment = new MPayment(getCtx(),m_C_Payment_ID,null);
				m_ZDateDoc = m_DateDoc; 
				m_DateDoc = payment.getDateAcct();
				m_DateAcct = m_DateDoc;
				m_C_Period_ID = -1;
				setPeriod();
				// 18/02/2008 Si on a un paiement sur une période cloturée, le m_C_Period_ID est -1
				if (m_C_Period_ID == -1) {
					org.compiere.model.MPeriod tmp = MPeriod.getOfCalendar(getCtx(), MCalendar.getDefault(getCtx()).getC_Calendar_ID(), m_DateAcct);
					m_C_Period_ID = tmp.getC_Period_ID();
				}
			}
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			else if (invoice.isSOTrx())
			{
				...
				if (as.isAccrual())
				{   
					BigDecimal newCr = null;
					BigDecimal newDr = null;
					...
					|| getC_Currency_ID() != line.getInvoiceC_Currency_ID()))	//	allocation <> invoice currency
			{
				...
				if (p_Error != null)
				{ //ZCOM999
					log.severe("ERREUR DE TAUX DE CHANGE : "+p_Error); //ZCOM999	
					return null;
				} //ZCOM999
			}
		...
		/*ZCOM205** RESET THE Allocation date */
		if (((Doc_Allocation)fact.m_doc).m_ZDateDoc != null){
			m_DateDoc = ((Doc_Allocation)fact.m_doc).m_ZDateDoc;
			m_DateAcct = ((Doc_Allocation)fact.m_doc).m_DateDoc;
			MPeriod tmp = MPeriod.getOfCalendar(getCtx(), MCalendar.getDefault(getCtx()).getC_Calendar_ID(), m_DateAcct);
			m_C_Period_ID = tmp.getC_Period_ID();
			FactLine[] fls = fact.getLines();
			for (int i = 0; i < fls.length; i++) {
				FactLine fl = fls[i];
				if (m_DateAcct != null){
					fl.setDateAcct(m_DateAcct);
					fl.setC_Period_ID(m_C_Period_ID);
				}
			}
		}
		/*ZCOM205*/
	...
	protected MAccount getPaymentAcct (MAcctSchema as, int C_Payment_ID)
	{
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				if (MDocBaseType.DOCBASETYPE_APPayment.equals(rs.getString(2)))
					accountType = Doc.ACCTTYPE_PaymentSelect; //ZCOM379
				//	Prepayment
		...
		//ZCOM379
		if (accountType == Doc.ACCTTYPE_PaymentSelect ) {
			int para_1 = 0;
			String sql2 = null;
			Object[] objs = getBankDocAccount("B_PaymentSelect_Acct",C_Payment_ID);
			if (objs != null) {
				para_1 = ((Integer)objs[1]).intValue();
				sql2 = (String)objs[0];
			}
			int Account_ID = 0;
			try
			{
				...
				if (para_1 == -1)   //  GL Accounts
					pstmt2.setInt (1, as.getC_AcctSchema_ID());
				else
				{
					pstmt2.setInt (1, para_1);
					pstmt2.setInt (2, as.getC_AcctSchema_ID());
				}
			...
			{
				log.log(Level.SEVERE, "AcctType=" + accountType + " - SQL=" + sql, e);
			}
			//	No account
			if (Account_ID == 0)
			{
	 ...
	 */
	protected String createRealizedGainLoss (MAcctSchema as, Fact fact, MAccount acct,
		MInvoice invoice, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		String sql = "SELECT " 
			...
			+ " AND C_AcctSchema_ID=?"
/*ZCOM958*/ + " AND Account_ID NOT IN (SELECT vc.account_id FROM C_ValidCombination vc INNER JOIN C_AcctSchema_GL ac ON vc.C_ValidCombination_ID = ac.CurrencyBalancing_Acct WHERE ac.C_AcctSchema_ID = fact_acct.c_AcctSchema_id)  "
			+ " AND PostingType='A'";
		...
		// Ben non si la facture est comptabilisée à zéro ...
		if (invoiceSource == null) invoiceSource = Env.ZERO; //ZCOM999
		if (invoiceAccounted == null) invoiceAccounted = Env.ZERO; //ZCOM999
		...
		if (getC_Currency_ID() != invoice.getC_Currency_ID())
		{
			...
			if (allocationSourceNew == null)
				allocationSourceNew = allocationSource; //ZCOM999 je me fiche que ce ne soit pas bon car Compiere
				// n est pas capable de DIRE ce qui ne va pas
				//ZCOM999 return "Gain/Loss - No Conversion from Allocation->Invoice";
		...
		if (allocationAccounted == null) allocationAccounted = Env.ZERO;
		//ZCOM999 Avoid NPE

		//	Full Payment in currency
		if (allocationSource.compareTo(invoiceSource) == 0
			|| invoiceSource.doubleValue() == 0 //ZCOM999 Avoir divide by zero		
		)
		{
	 ...
	 * ZCOM205
	 * VHA 18/01/2006 
	 * Mauvais test de period due a un Patch
	 */
	public boolean isPeriodOpen() {
		...
		period = MPeriod.getOfCalendar(getCtx(), MCalendar.getDefault(getCtx()).getC_Calendar_ID(), tmp);
		
		super.isPeriodOpen(); // Needed tot set the period
		
		if (period != null && period.isOpen(MDocBaseType.DOCBASETYPE_PaymentAllocation, tmp) == null)
			return true;
		else 
			return false;
	}
...
class Doc_AllocationTax
{
	...
	public boolean createEntries (MAcctSchema as, Fact fact, DocLine line)
	{
		...
		for (int i = 0; i < m_facts.size(); i++)
		{
			...
			//ZCOM436 START
			MInvoice inv = new MInvoice(Env.getCtx(), ((DocLine_Allocation)line).getC_Invoice_ID(), null);
			if (inv.getGrandTotal().signum() == 1){
			//ZCOM436 END
			...
			if (m_DiscountAmt.signum() != 0)
			{
				...
				if (Env.ZERO.compareTo(factAcct.getAmtSourceDr()) != 0)
				{
					...
					if (amount.signum() != 0)
					{
						...
						//ZCOM436 Voir SOLG86
						// On est ici en FOURNISSEUR : facture ou note de credit (+ ou -)
						amount = Env.ZERO.subtract(amount);
						//ZCOM436 END
