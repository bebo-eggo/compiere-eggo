 ...
 *
 *  @author Jorg Janke
 *  @version  $Id: MLookupFactory.java 8795 2010-05-20 23:34:54Z rthng $
 */
public class MLookupFactory
{
	...
	//	int AD_Table_ID = 0;
		...
		if (isDisplayIdentifiers)
			//ZCOM999 On put avoir IsStranslated False et cette methode fera quand meme appel au Trl : Non !
			//ZCOM999 DisplayColumn = getLookup_DisplayColumn(language, TableName).toString();
			DisplayColumn = getLookup_DisplayColumn(language, TableName, IsTranslated).toString(); //ZCOM999
		else
	...
	static public StringBuffer getLookup_DisplayColumn (Language language,String TableName) {
		return getLookup_DisplayColumn(language, TableName, true);
	}
	 ...
	 */
	//ZCOM999 static public StringBuffer getLookup_DisplayColumn (Language language,String TableName)
	static public StringBuffer getLookup_DisplayColumn (Language language,String TableName, boolean IsTranslated)
	{
		...
		isTranslated = isTranslated && IsTranslated; //ZCOM999
		...
		for (int i = 0; i < size; i++)
		{
			...
			//  translated
			//ZCOM999 if (ldc.IsTranslated && !Env.isBaseLanguage(language, TableName))
			if (ldc.IsTranslated && !Env.isBaseLanguage(language, TableName) && IsTranslated) //ZCOM999
	...
	//	boolean isSOTrx = !"N".equals(ctx.getContext( WindowNo, "IsSOTrx"));
		...
		//ZCOM999 StringBuffer displayColumn = getLookup_DisplayColumn(language, TableName);
		StringBuffer displayColumn = getLookup_DisplayColumn(language, TableName, isTranslated); //ZCOM999
