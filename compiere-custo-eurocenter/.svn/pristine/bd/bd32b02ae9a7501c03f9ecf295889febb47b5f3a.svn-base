...
import org.compiere.model.Storage;
...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(get_ValueAsTimestamp("CreatedDate")==null)
			set_ValueNoCheck("CreatedDate", getMovementDate());
		if(get_ValueAsBoolean("IsInTransit") && is_ValueChanged("IsInTransit") && get_ValueAsTimestamp("TransitDate")==null)
			set_ValueNoCheck("CreatedDate", new Timestamp(System.currentTimeMillis()));
		return true;
	}	//	beforeSave
			...
			if(product != null && product.isStocked()) {
				...
				if (line.getM_AttributeSetInstance_ID() == 0)
				{
					...
					for (MMovementLineMA ma : mas) {
						MStorageDetail storageOnHandFrom = Storage.getCreateDetails(getCtx(), line.getM_Locator_ID(),
							line.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), X_Ref_Quantity_Type.ON_HAND,get_Trx());

						MStorageDetail storageOnHandTo = Storage.getCreateDetails(getCtx(), line.getM_LocatorTo_ID(),
								line.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), X_Ref_Quantity_Type.ON_HAND, get_Trx());
						...
						try
						{
							...
							if(dt.isZ_IsStorageEvent()){
								MLocator fromLoc = new MLocator(getCtx(), line.getM_Locator_ID(), get_Trx());
								MLocator toLoc = new MLocator(getCtx(), line.getM_LocatorTo_ID(), get_Trx());
								generatePosStorageEven(line, fromLoc,ma.getM_AttributeSetInstance_ID(),ma.getMovementQty().negate() );
								generatePosStorageEven(line, toLoc,ma.getM_AttributeSetInstance_ID(),ma.getMovementQty());
							}
						} catch (Exception e)
						{
							log.log(Level.SEVERE, "Create TFT storageEvent", e);
						}
						// BBR : SYNCHRO STOCK
				...
				if (trxFrom == null)
				{
					...
					try
					{
						...
						if(dt.isZ_IsStorageEvent()){
							MLocator fromLoc = new MLocator(getCtx(), line.getM_Locator_ID(), get_Trx());
							MLocator toLoc = new MLocator(getCtx(), line.getM_LocatorTo_ID(), get_Trx());
							generatePosStorageEven(line, fromLoc,line.getM_AttributeSetInstance_ID(),line.getMovementQty().negate() );
							generatePosStorageEven(line, toLoc,line.getM_AttributeSetInstanceTo_ID(), line.getMovementQty());
						}
					} catch (Exception e)
					{
						log.log(Level.SEVERE, "Create TFT storageEvent", e);
	...
	private void generatePosStorageEven(MMovementLine line, MLocator loc, int attrID ,BigDecimal qty) {
		

		// Annul Make Mvt
		X_Z_StorageEvent ev = new X_Z_StorageEvent(getCtx(), 0, get_Trx());
		ev.setAD_Client_ID(line.getAD_Client_ID());
		ev.setAD_Org_ID(loc.getAD_Org_ID());
		ev.setM_Locator_ID(loc.getM_Locator_ID());
		ev.setM_Product_ID(line.getM_Product_ID());
		ev.setM_AttributeSetInstance_ID(attrID);
	...
	public boolean voidIt()
	{
		...
		//100325 
		String VoidAll = Msg.getMsg(getCtx(), "TRFT_ROLE_CANCALL");
		String Void93 = Msg.getMsg(getCtx(), "TRFT_ROLE_CANC93");
		String msgErr = Msg.getMsg(getCtx(), "TRFT_ROLE_CANC_MSG");
		String AdminR = Msg.getMsg(getCtx(), "EC_ADMIN_ROLE");
		int role_ID = getCtx().getAD_Role_ID();
		if(!AdminR.contains(""+role_ID))
		{
			
			if(MOrg.get(getCtx(), getAD_Org_ID()).getValue().equals("93"))
			{
				if(!Void93.contains(""+role_ID))
				{
					m_processMsg = msgErr;
					return false;
				}
			...
			boolean ex93 = false;
			for (MMovementLine line : lines) {
				MWarehouse mw = new MWarehouse(getCtx(), line.get_ValueAsInt("M_Warehouse_ID"), get_Trx());
				if(mw.getValue().contains("93"))
				{
					ex93 = true;
					break;
				}
			}
			if(!Void93.contains(""+role_ID) && ex93)
			{
				m_processMsg = msgErr;
				return false;
			}
			
			MRole mr = new MRole(getCtx(), role_ID, get_Trx());
							
			if(VoidAll.contains(""+role_ID) && !mr.isAccessAllOrgs())
			{
				MUserOrgAccess[] moas = MUserOrgAccess.getOfUser(getCtx(), getCtx().getAD_User_ID());
				boolean ex = false;
				for (int i = 0; i < moas.length; i++) {
					if(moas[i].getAD_Org_ID()==getAD_Org_ID()){
						ex = true;
						break;
					}
				}
			...
			for (MMovementLine line : lines) {
				...
				if (old.compareTo(Env.ZERO) != 0)
				{
					...
					if(oif.isPOS()){
						...
						updateZTransferedQty(line.getM_Product_ID(),locFrom.getAD_Org_ID(),locTo.getAD_Org_ID(),old.negate());
					}
				}
			}
			DB.executeUpdate("update M_MovementConfirm Set  description = 'Void()', processed = 'Y', docaction = '--', docstatus = 'VO' where M_Movement_ID= ? ", getM_Movement_ID(),get_Trx());
