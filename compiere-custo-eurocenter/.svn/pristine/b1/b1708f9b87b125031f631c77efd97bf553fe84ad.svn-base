...
public class QueryRestriction implements Serializable
{
	...
	public String getSQL (String tableName)
	{
		...
		else
		{
			...
			if (Code instanceof String)
			{
				...
				if(getColumnDisplyType() == 0)
				//ZCOM999 SC781 sb.append(DB.TO_DATE((Timestamp)Code, false));
					sb.append(DB.TO_DATE((Timestamp)Code, true));
				else
					sb.append(DB.TO_DATE((Timestamp)Code, getColumnDisplyType() == DisplayTypeConstants.Date));//ZCOM999 SC781
			else
				sb.append(Code);
			...
			if (Query.BETWEEN.equals(Operator))
			{
				...
				if (Code_to instanceof String)
					...
					if(getColumnDisplyType() == 0)
						//ZCOM999 SC781 sb.append(DB.TO_DATE((Timestamp)Code, false));
							sb.append(DB.TO_DATE((Timestamp)Code_to, true));
						else
							sb.append(DB.TO_DATE((Timestamp)Code_to, getColumnDisplyType() == DisplayTypeConstants.Date));//ZCOM999 SC781
				else
					sb.append(Code_to);
			}
	...
	//ZCOM999 SC781: debut
	protected int columnDisplyType; 

	public int getColumnDisplyType() {
		return columnDisplyType;
	}

	public void setColumnDisplyType(int columnDisplyType) {
		this.columnDisplyType = columnDisplyType;
	}
	//ZCOM999 SC781: fin
