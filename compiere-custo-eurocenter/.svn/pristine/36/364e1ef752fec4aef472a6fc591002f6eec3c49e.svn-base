...
public class ExportPaymentSepa extends SvrProcess{
	...
	private MPaySelectionProxy p_MPaySelectionProxy = null;
	private MPaySelection p_MPayselection = null;
	...
	protected void prepare() {
		...
		p_MPayselection = new MPaySelection(getCtx(), p_C_PaySelection_ID, get_Trx());
		p_MPaySelectionProxy = new MPaySelectionProxy(p_MPayselection);
	...
	protected String doIt() throws Exception {
		...
		grpHdr.setMsgId(String.valueOf(p_C_PaySelection_ID));
		...
		ArrayList<MPaySelectionCheck> checks = p_MPaySelectionProxy.getPaySelectionChecks();
		ArrayList<String> infoPaySelCheckAccount = null;
		ArrayList<String> infoPaySelCheckBPAccount = null;
		String iso_codePaySel = ExportSepaUtil.getCurrencyIsoCode(p_MPayselection);
		String pmtMtd = ExportSepaUtil.getPmtMtd(p_MPayselection);
		...
		for (MPaySelectionCheck mPaySelectionCheck : checks) {
			...
			if (firstloop){
				if (p_MPayselection.get_ValueAsBoolean("IsExportByLot"))
					firstloop = false;
				pmtInf = f.createPaymentInstructionInformation3();
	
				//Redmine 27129
				if (p_MPayselection.get_ValueAsBoolean("IsExportByLot"))
					pmtInf.setPmtInfId(p_MPayselection.get_ValueAsString("DocumentNo"));
				else{
					payment = new MPayment(getCtx(), mPaySelectionCheck.getC_Payment_ID(), get_Trx());
					pmtInf.setPmtInfId(payment.getDocumentNo());
				}

				if(pmtMtd.equals("CHK") || pmtMtd.equals("TRF") || pmtMtd.equals("TRA"))
					pmtInf.setPmtMtd(PaymentMethod3Code.fromValue(pmtMtd));
				pmtInf.setBtchBookg(p_MPayselection.get_ValueAsBoolean("IsExportByLot"));
				...
				acctIc.setIBAN(String.valueOf(values.get(8)));
				...
				fII.setBIC(String.valueOf(values.get(9)));
			...
			amt.setValue(mPaySelectionCheck.getPayAmt());
			...
			fII.setBIC(infoPaySelCheckBPAccount.get(6));//bban
			...
			if(infoPaySelCheckBPAccount.get(1)!=null && ! infoPaySelCheckBPAccount.get(1).equalsIgnoreCase("")) //#22212
				adrline.getAdrLine().add(infoPaySelCheckBPAccount.get(1));
			if(infoPaySelCheckBPAccount.get(2)!=null && ! infoPaySelCheckBPAccount.get(2).equalsIgnoreCase(" ")) //#22212
				adrline.getAdrLine().add(infoPaySelCheckBPAccount.get(2));
			...
			acctIc.setIBAN(infoPaySelCheckBPAccount.get(7));
			...
			boolean IsVide = true; //#22212
			if(!ExportSepaUtil.IsStructuredCom(mPaySelectionCheck)){
				if (!mPaySelectionCheck.get_ValueAsString("Z_PaymentCommunication").equalsIgnoreCase("")){ //#22212
					rmtInf.getUstrd().add(mPaySelectionCheck.get_ValueAsString("Z_PaymentCommunication"));
					IsVide = false;  //#22212
				}
			}else{
				StructuredRemittanceInformation7 struct = f.createStructuredRemittanceInformation7();
				CreditorReferenceInformation2 cdtrRefInf = f.createCreditorReferenceInformation2();
				CreditorReferenceType1Choice crtc = f.createCreditorReferenceType1Choice();
				CreditorReferenceType2 credRef = f.createCreditorReferenceType2();

				crtc.setCd(DocumentType3Code.SCOR);
				cdtrRefInf.setRef(mPaySelectionCheck.get_ValueAsString("Z_PaymentCommunication"));
				String issr = MBPBankAccount.getOfBPartner(getCtx(), 
						mPaySelectionCheck.get_ValueAsInt("C_BPartner_ID"))[0].get_ValueAsString("Issr");
				credRef.setIssr(String.valueOf(issr));
				credRef.setCdOrPrtry(crtc);

				cdtrRefInf.setTp(credRef);
				struct.setCdtrRefInf(cdtrRefInf);
				rmtInf.getStrd().add(struct);

				if (!mPaySelectionCheck.get_ValueAsString("Z_PaymentCommunication").equalsIgnoreCase("") 
						|| !issr.equalsIgnoreCase("")) //#22212
					IsVide = false;  //#22212
			}
			if(!IsVide) //#22212
	...
	private ArrayList<Object> getInfoHeader(){
		...
		String sql = ""
			+ "SELECT PS.C_PaySelection_ID AS msgid, "
			+ "PS.created AS credttm, "
			+ "(SELECT COUNT(PSC.C_PaySelectionCheck_ID) FROM C_PaySelectionCheck PSC WHERE PSC.C_PaySelection_ID = PS.C_PaySelection_ID AND PSC.IsActive='Y') AS NbOftrx, "
			+ "(SELECT SUM(PSC.payamt) FROM C_PaySelectionCheck PSC WHERE PSC.C_PaySelection_ID = PS.C_PaySelection_ID AND PSC.IsActive='Y') AS ctrlsum, " 
			+ "ORG.name as Nm, "
			+ "ORGINF.taxid as id, "
			+ "ORGINF.ISSR as Issr, "
			+ "PS.paydate AS ReqdExctnDt, "
		...
		try {
			...
			pst.setInt(1, p_C_PaySelection_ID);
			rs = pst.executeQuery();
			if (rs.next()){
	...
	private Object[] getadrlineSelectionCheck(MPaySelectionCheck check){
		//#22212		 
		String adress = "Address1"; //#37054

		String sql = ""
			+ "SELECT LO." + adress + " AS " + adress + ", LO.postal || ' ' || (SELECT CT.NAME FROM C_Country CT WHERE CT.C_Country_ID = LO.C_Country_ID), "
			+ "(SELECT CT.countrycode FROM C_Country CT WHERE CT.C_Country_ID = LO.C_Country_ID) "
			+ "FROM C_PaySelectionCheck PSL "
			...
			+ "WHERE PSL.C_PaySelectionCheck_ID = ? AND PSL.IsActive='Y' " ;
		...
		try
		{
			...
			pst.setInt(1, check.getC_PaySelectionCheck_ID());
	...
	private ArrayList<String> getInfoPaySelectionCheckAccount(MPaySelectionCheck PaySelectionCheck){
		ArrayList<String> values = new ArrayList<String>();
		StringBuffer sql = new StringBuffer("SELECT Z_RIBCLEAN(acct.iban), Z_RIBCLEAN(acct.bban), cur.iso_code, p.documentno ");
		sql.append("FROM C_PaySelectionCheck ck ");
		...
		sql.append("INNER JOIN c_currency cur ON (cur.C_Currency_ID = p.C_Currency_ID) ");
		sql.append("WHERE ck.C_PaySelectionCheck_ID =? AND ck.IsActive='Y'");

		PreparedStatement pst = null;
		ResultSet rs = null;
		try {
			pst = DB.prepareStatement(sql.toString(), get_Trx());
			pst.setInt(1, PaySelectionCheck.getC_PaySelectionCheck_ID());
			rs = pst.executeQuery();
			if (rs.next()){
	...
	private ArrayList<String> getInfoPaySelectionCheckBPAccount(MPaySelectionCheck PaySelectionCheck){
		ArrayList<String> values = new ArrayList<String>();
		StringBuffer sql = new StringBuffer("SELECT BPB.A_CountryCode, BPB.A_Street, BPB.A_Zip || ' ' || BPB.A_City," );
		 sql.append("Z_SubCode, Z_IDBk, Z_OEurope, Z_RIBCLEAN(bban), Z_RIBCLEAN(iban), AccountNo, BPB.ISO_Code, BPB.A_Name ");
		sql.append("FROM C_PaySelectionCheck ck ");
		sql.append("INNER JOIN C_BPartner BP ON (BP.C_BPartner_ID = ck.C_BPartner_ID) ");
		sql.append("INNER JOIN C_BP_BankAccount BPB ON (BPB.C_BPartner_ID = BP.C_BPartner_ID) ");
		sql.append("WHERE ck.C_PaySelectionCheck_ID =? AND ck.IsActive='Y' ");
		sql.append(" AND BPB.IsActive='Y' ");// #68213
		PreparedStatement pst = null;
		ResultSet rs = null;
		try {
			pst = DB.prepareStatement(sql.toString(), get_Trx());
