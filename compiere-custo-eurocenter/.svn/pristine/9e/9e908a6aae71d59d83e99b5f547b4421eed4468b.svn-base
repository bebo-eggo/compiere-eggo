 ...
 */
public class OrderPOCreateEggo extends SvrProcess
{
	/**	Order Date From		*/
	...
	protected String doIt() throws Exception
	{
		...
		for (int i = 0; i < BPErrorList.size(); i++) {
			errFiles = errFiles.concat("\n"+BPErrorList.get(i));
		}
		this.getProcessInfo().setSummary(errFiles+"|"+"@Created@ " + counter);
		if(errFiles!=null && errFiles.length()>0)
			return errFiles+" "+Msg.getMsg(getCtx(), "XX_MANDATORY_FILES")+"|"+"@Created@ " + counter;
...
//					EggoCtx.setConstants();
					...
					if(nbFile>0 && nbAttachFiles==0 && BPFileMand!=null && BPFileMand.contains(mb.getValue()) && dti.isStore() && !dti.isSAV() && !dti.isDevis())
					{
						//Delete PO
						po.delete(true);
