...
public class MJournal extends X_GL_Journal implements DocAction
{
	...
	protected static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MJournal.class);
	/**
	 * 
	 */
	...
	public void setC_DocType_ID(String oldC_DocType_ID, String newC_DocType_ID, int windowNo) throws Exception
	{
		...
		MDocType docType = new MDocType(getCtx(), get_ValueAsInt("C_DocType_ID"), null);
		setGL_Category_ID(docType.getGL_Category_ID());
	} // setC_DocType_ID
	...
	public void setC_Period_ID(int C_Period_ID)
	{
		...
		if (period != null)
		{
			...
			//79006
			else if(!period.isStandardPeriod()&&!period.isInPeriod(dateAcct))		
			{			
				addError(Msg.getMsg(getCtx(), "DateIncompatible"));			
			}			
	...
	public int copyLinesFrom (MJournal fromJournal, Timestamp dateAcct, char typeCR)
	{
		...
		for (MJournalLine element : fromLines) {
			...
			PO.copyValues(element, toLine, element.getAD_Client_ID(), element.getAD_Org_ID());//Redmine 17792
	...
	public void setProcessed (boolean processed)
	{
		...
		int noLine = DB.executeUpdate(get_Trx(), sql);
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//Redmine 17966
		int C_Period_ID = MPeriod.getC_Period_ID(getCtx(), getAD_Org_ID(), getDateAcct());
		if (C_Period_ID == 0){
		...
		if (C_Period_ID != getC_Period_ID()){
			...
			if (!MPeriod.PERIODTYPE_AdjustmentPeriod.equals(period.getPeriodType())){
				log.saveError("PeriodNotValid", Msg.getMsg(getCtx(), "DateAcct_Period"));
				return false;
			}
			//79006
			else if(!period.isStandardPeriod()&&!period.isInPeriod(getDateAcct()))	
			{	

				log.saveError(Msg.getMsg(getCtx(), "DateIncompatible"),"");
				return false;
			}
			//79006
		}
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (is_ValueChanged("DateAcct")
				...
				|| is_ValueChanged("C_ConversionType_ID"))
			updateDateAcctCurrency();
		//
		return success;
	...
	//Redmine 17956
	private void updateDateAcctCurrency(){
		MJournalLine[] lines = getLines(true);

		for (MJournalLine mJournalLine : lines) {
			mJournalLine.setDateAcct(getDateAcct());
			mJournalLine.setC_Currency_ID(getC_Currency_ID());
			mJournalLine.setC_ConversionType_ID(getC_ConversionType_ID());
			mJournalLine.setCurrencyRate(getCurrencyRate());
			mJournalLine.save(get_Trx());
		}
	}
	...
	public MJournal reverseCorrectIt (int GL_JournalBatch_ID)
	{
		...
		m_processMsg = DateAcctChangeValidator.isValide(getCtx(), reverse.getDateAcct(), dt.getDocBaseType(), reverse.getAD_Org_ID());
		if (m_processMsg != null)
			throw new CompiereException(m_processMsg);
		//	Get Period
