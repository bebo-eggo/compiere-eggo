...
public class MAllocationHdr extends X_C_AllocationHdr implements DocAction
{
	...
	public int copyLinesFrom (MAllocationHdr otherAllocation)
	{
		...
		for (MAllocationLine fromLine : fromLines) {
			...
			{
				/*ZCOM #27133 on veut que les affecations annulées soient liées au paiement original
	...
	/** Reversal Flag		*/
	private boolean m_void = false;

	/**
	 * 	Set Void
	 *	@param s_void m_void
	 */
	private void setVoid(boolean s_void)
	{
		m_void = s_void;
	}	//	setVoid
	/**
	...
	{
		//63103 : warning message
		if(isVoid() && !(StringUtils.isEmpty(DocumentEngine.isPeriodOpen(this))))
		{
			Timestamp today = new Timestamp (System.currentTimeMillis());	
			SimpleDateFormat sdf_date = new SimpleDateFormat("dd/MM/yyyy");
			String msgError ="@PeriodClosed@ : Date annulation = "+sdf_date.format(today);
			log.saveError("Error", msgError);
//			new ADialogDialog((Frame)null, 
//					Msg.parseTranslation(getCtx(), "Warning"), 
//					Msg.parseTranslation(getCtx(), msgError), 
//					JOptionPane.WARNING_MESSAGE);
	...
	private boolean reverseIt()
	{
		...
		if (msg != null){
			if(isVoid()){//63103 : permettre annulation en cas de période cloturé
				String msgError ="@PeriodClosed@ : cancellation Date "+sdf_date.format(today)+" ";
		...
		//	Reverse Line Amt
		BigDecimal xAmt = Env.ZERO;
		MAllocationLine[] rLines = reversal.getLines(false);
		for (MAllocationLine rLine : rLines) {
			xAmt = rLine.getAmount().negate();
			rLine.setAmount(xAmt);
			xAmt = rLine.getDiscountAmt().negate();
			rLine.setDiscountAmt(xAmt);
			xAmt = rLine.getWriteOffAmt().negate();
			rLine.setWriteOffAmt(xAmt);
			xAmt = rLine.getOverUnderAmt().negate();
			rLine.setOverUnderAmt(xAmt);
		...
		//63103:prendre la date du jour comme DateAcct de affectation extourne.
		reversal.setDateAcct(today);

		//
		if (!DocumentEngine.processIt(reversal, DocActionConstants.ACTION_Complete)&&!isVoid())
		{
