...
public class Evaluator
{
	...
	private static boolean evaluateLogicTuple (Evaluatee source, String logic)
	{
		...
		if (logic.contains("@SQL=")) {
			...
			String var = "";
			String sql = logic.replaceAll("@SQL=", "");
			String tmp = sql;
			while (tmp.contains("@")) {
				var = tmp.substring(tmp.indexOf("@"), (tmp.indexOf("@", (tmp.indexOf("@") + 1))) + 1);
				Object val = source.get_ValueAsString(var.replaceAll("@", " ").trim());
				if (val == null || val.toString().isEmpty()) val = BigDecimal.ZERO;
				sql = sql.replaceAll(var, val.toString());
				tmp = tmp.substring((tmp.indexOf("@", (tmp.indexOf("@") + 1)) + 1));
			}

			//	if (DB.getSQLValue(null, sql) == 1) return true ;
