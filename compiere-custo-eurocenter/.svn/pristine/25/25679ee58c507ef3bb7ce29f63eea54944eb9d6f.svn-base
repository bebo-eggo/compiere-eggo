...
public class MProductPO extends X_M_Product_PO
{
	 ...
	 * @return
	 */
	public static MProductPO getOfVendorProduct (Ctx ctx, int C_BPartner_ID, int M_Product_ID, int AD_Client_ID,Trx trx)
	{
		MProductPO productPO = null;
		String sql = "SELECT * FROM M_Product_PO "
			+ "WHERE C_BPartner_ID=? AND M_Product_ID = ? and AD_Client_ID= ? ";
	...
	protected boolean beforeSave(boolean newRecord) {
		if (this.isCurrentVendor())
		{
			//
			MProductPO[] mpos = getOfProduct(getCtx(), this.getM_Product_ID(), null);
			for (int i = 0; i < mpos.length; i++) {
				MProductPO mProductPO = mpos[i];
				if(mProductPO.isCurrentVendor() && 
						(this.getC_BPartner_ID() != mProductPO.getC_BPartner_ID()))
				{
					mProductPO.setIsCurrentVendor(false);
					mProductPO.save();
				}
