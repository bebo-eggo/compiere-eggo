 ...
 *
 *  @author Jorg Janke
 *  @version $Id: PO.java 8804 2010-05-24 21:29:13Z nnayak $
 */
...
implements Serializable, Cloneable, Comparator<PO>, Evaluatee
{
	...
	public boolean load (Trx trx)
	{
		...
		catch (Exception e)
		{
			...
			throw new RuntimeException(e.getMessage()); //ZCOM666
		}
		finally {
	...
	private boolean hasVirtualColumns () {
		...
		//Setting to true for now due to regressions caused.
		return true;
	}
...
//				columnName = "LineNo";
	...
	public boolean save()
	{
		...
		//ZCOM 56614 We save Warnings here because it will be erased by saveNew / saveUpdate 
		ValueNamePair warningVnp = CLogger.retrieveWarning();
		boolean ret;
		if (newRecord)
			ret = saveNew();
		else
			ret = saveUpdate();
		//Save back the warning to send to GridTable
		if (warningVnp != null)
			log.saveWarning(warningVnp.getValue(), warningVnp.getName());
		return ret;
		//ZCOM 56614
...
//			return saveUpdate();
	...
	private static boolean validateBeforeSave(PO po){
		...
		//ZCOM 56614 START
		// Si on fait des p_changeVO.addWarning(msg) dans les PO, on les recupère ici pour les sauver en CLogger
		// et les montrer plus tard
		ChangeVO p_changeVO = po.get_ChangeVO();
		if (p_changeVO != null && p_changeVO.hasWarning()) {
			if (p_changeVO.getMessages() != null && p_changeVO.getMessages().size() > 0) {
				StringBuffer msg = new StringBuffer();
				int i = 1;
				for (ResponseVO.Message message : p_changeVO.getMessages()) {
					if (p_changeVO.getMessages().size() > 1)
						msg.append("[").append(i++).append("] ");
					msg.append(message.getMessage());
					msg.append("\n");
				}
				log.saveWarning("Warning", msg.toString());
			}
		}
		//ZCOM 56614 START
		//	Model Validator
	...
	public static boolean saveAll(Trx trx, List<? extends PO> p_objects)
	{
		...
		for (Map.Entry<String, HashSet<PO>> sql : sqls.entrySet()) {
			...
			int no = DB.executeBulkUpdate(sql.getKey(), bulkParams, trx);
	...
	protected boolean saveUpdate() {
		...
		if (params != null) {
			int no = DB.executeUpdate(params.sql, params.params, m_trx);
			boolean ok = no == 1;
			if (ok) {
				ok = lobSave();
			} else {
				log.log(Level.WARNING, "#" + no + " - [" + m_trx + "] - " + p_info.getTableName() + "." + get_WhereClause(true));
				log.log(Level.WARNING, params.sql); //ZCOM
			}
	...
	private boolean saveNew() {
		...
		if (params != null) {
			int no = DB.executeUpdate(params.sql, params.params, false, m_trx);
			boolean ok = no == 1;
			if (ok) {
				ok = lobSave();
				if (hasVirtualColumns() && !load(m_trx)) // re-read Info
				{
			...
			if (ok)
			{ //ZCOM999
				m_createNew = true; //ZCOM999 Sinon po.is_new() ne marche pas dans les contrats en web service	
...
//----------> end of r9149
	...
	public boolean delete (boolean force)
	{
		...
		int no = DB.executeUpdate(sql.toString(), get_WhereClauseParams(), localTrxName);
	...
	private boolean deleteTranslations(Trx trx)
	{
		...
		.append(keyColumn).append("=?");
		int no = DB.executeUpdate(sql.toString(), new Object[]{ get_ID()}, trx);
		log.fine("#" + no);
		return no >= 0;
	}	//	deleteTranslations
	...
	public boolean lock()
	{
		...
		if (index != -1)
		{
			...
			boolean success = DB.executeUpdate(sql, get_WhereClauseParams(), (Trx) null) == 1;	//	outside p_trx
			if (success)
				log.fine("success");
			else
	...
	public boolean unlock (Trx trx)
	{
		...
		if (index != -1)
		{
			...
			boolean success = DB.executeUpdate(sql, get_WhereClauseParams(), trx) == 1;
			if (success)
			...
			if (columnName != null){
				...
				if (column != null){        
					...
					if (FieldType.isLookup(AD_Reference_ID)){
						...
						//ZCOM999 String name=MLookupFactory.getLookup_DisplayColumn (Env.getLanguage(ctx),lookupInfo.TableName).toString()+"=?";
						String name=MLookupFactory.getLookup_DisplayColumn (Env.getLanguage(ctx),lookupInfo.TableName, true).toString()+"=?";
			...
			String acctBaseTable, String whereClause, List<PO> po)
	{
		...
		sb.append(" )");
		int no = DB.executeUpdate(sb.toString(), params,po.get(0).get_Trx());
		if (no >= 0)
			log.fine("#" + no);
		else
			log.warning("#" + no
			...
			String acctBaseTable, String whereClause, List<PO> po)
	{
		...
		String table_name = po.get(0).get_TableName();
		int updatedBy = po.get(0).getUpdatedBy();
		Object[] params = new Object[2*po.size()];
		String sql = "SELECT c.ColumnName "
			+ "FROM AD_Column c INNER JOIN AD_Table t ON (c.AD_Table_ID=t.AD_Table_ID) "
			+ "WHERE t.TableName=? AND c.IsActive='Y' AND c.AD_Reference_ID=25 ORDER BY 1";
		PreparedStatement pstmt = null;
