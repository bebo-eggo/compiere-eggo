...
public class X_I_Invoice extends POExtended implements POInterface
{
    ...
    */
	...
	protected boolean afterDelete(boolean success) {
    	...
    	if(get_Value("ZZ_FileName")!=null)
    	{
    		File file = new File((String)get_Value("ZZ_FileName"));
    		if(file.exists())
    		{
    ...
    */
    ...
    /** Last Updated Timestamp 2009-05-20 20:41:20.0 */
	...
	/**
	 * Override du save d'un PO pour y impl�menter des am�liorations.
	 * 
	...
	public boolean save()
	{
		if (getCtx().getAD_User_ID() != getSalesRep_ID())
		{
			boolean t = MUser.isSalesRep(getCtx().getAD_User_ID());
			if(!t)
				return super.save();
			setSalesRep_ID(getCtx().getAD_User_ID());
			String sql =  " UPDATE I_Invoice "
					+ " SET SalesRep_ID = " + getCtx().getAD_User_ID()
					+ " WHERE SalesRep_ID <> " + getCtx().getAD_User_ID()
					+ " AND Ad_Org_ID = " + getAD_Org_ID()
					+ " AND POReference = " + getPOReference()
					+ " AND C_BPartner_ID = " + getC_BPartner_ID()
					+ " AND C_BPartner_Location_ID = " + getC_BPartner_Location_ID()
					+ " AND I_Invoice_ID <> "+getI_Invoice_ID();
			DB.executeUpdate(sql.toString(), get_Trx());
		}
		//Masse update si changement de Tiers, Adresse, Type de document
		if(is_ValueChanged("C_BPartner_ID"))
		{
			String sql =  " UPDATE I_Invoice "
					+ " SET C_BPartner_ID = " + get_ValueAsInt("C_BPartner_ID")
					+ " , C_BPartner_Location_ID = null WHERE C_BPartner_ID <> " + get_ValueAsInt("C_BPartner_ID")
					+ " AND Ad_Org_ID = " + getAD_Org_ID()
					+ " AND POReference = " + getPOReference()
					+ " AND I_Invoice_ID <> "+getI_Invoice_ID();
			DB.executeUpdate(sql.toString(), get_Trx());
		}
		if(is_ValueChanged("C_PaymentTerm_ID"))
		{
			String sql =  " UPDATE I_Invoice "
					+ " SET C_PaymentTerm_ID = " + get_ValueAsInt("C_PaymentTerm_ID")
					+ " WHERE C_PaymentTerm_ID <> " + get_ValueAsInt("C_PaymentTerm_ID")
					+ " AND Ad_Org_ID = " + getAD_Org_ID()
					+ " AND POReference = " + getPOReference()
					+ " AND I_Invoice_ID <> "+getI_Invoice_ID();
			DB.executeUpdate(sql.toString(), get_Trx());
		}
		if(is_ValueChanged("C_DocType_ID"))
		{
			String sql =  " UPDATE I_Invoice "
					+ " SET C_DocType_ID = " + get_ValueAsInt("C_DocType_ID")
					+ " WHERE C_DocType_ID <> " + get_ValueAsInt("C_DocType_ID")
					+ " AND Ad_Org_ID = " + getAD_Org_ID()
					+ " AND POReference = " + getPOReference()
					+ " AND I_Invoice_ID <> "+getI_Invoice_ID();
			DB.executeUpdate(sql.toString(), get_Trx());
