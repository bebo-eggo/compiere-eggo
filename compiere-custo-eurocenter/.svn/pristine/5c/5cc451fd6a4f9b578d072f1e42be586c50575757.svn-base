 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: AcctProcessor.java 7773 2009-06-03 22:54:54Z gwu $
 */
public class AcctProcessor extends CompiereServer
{
	...
	private void postSession()
	{
		...
		for (int i = 0; i < m_docBaseTypes.length; i++)
		{
			...
			// #ZCOM999 ##54617
			// Skip posting for C_PaySelection
			if ("C_PaySelection".equalsIgnoreCase(TableName) || TableName.equalsIgnoreCase("Z_BankTransfert"))
				continue;
...
//				.append(" AND Processed='Y' AND Posted IN ('N','E') AND IsActive='Y'")// ZCOM999 24740 : not Include Doc In Error
				.append(" AND Processed='Y' AND Posted = 'N' AND IsActive='Y' AND Processing <> 'Y'")// ZCOM999 24740 : not Include Doc In Error
