 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MUser.java 8755 2010-05-12 18:30:14Z nnayak $
 */
public class MUser extends X_AD_User
{
	...
	public static MUser[] getOfBPartnerAll (Ctx ctx, int C_BPartner_ID)
	{
		ArrayList<MUser> list = new ArrayList<MUser>();
		String sql = "SELECT * FROM AD_User WHERE C_BPartner_ID=? Order By IsActive DESC, AD_User_ID";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
	...
	public MRole[] getRoles ()
	{
		...
		String sql = "SELECT * FROM AD_Role r "
			+ "WHERE r.IsActive='Y'"
			+ "AND ((EXISTS (SELECT * FROM AD_Role_OrgAccess ro"
			+ " WHERE r.AD_Role_ID=ro.AD_Role_ID AND ro.IsActive='Y' )) OR"
	        + " (EXISTS (SELECT * FROM AD_User_OrgAccess uo, AD_User_Roles ur" 
			+ " WHERE ur.AD_User_ID=uo.AD_User_ID AND uo.IsActive='Y' )))"
			+ " AND EXISTS (SELECT * FROM AD_User_Roles ur"
			+ " WHERE r.AD_Role_ID=ur.AD_Role_ID AND ur.IsActive='Y' AND ur.AD_User_ID=?) "
			+ " ORDER BY AD_Role_ID";
		PreparedStatement pstmt = null;
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (!newRecord && is_ValueChanged("EMail") && getEMail() != null)
			setEMailVerifyDate(null);
		...
		if(getC_BPartner_ID()>0)
		{
			...
			if(mb.get_ValueAsBoolean("IsArcoValidator") && getLDAPUser()==null) {
				setLDAPUser(getValue().toUpperCase());
			}
		}
		
		//Redmine 30745
		if (get_ValueAsBoolean("IsInvoiceValidator") && (getEMailUser()==null ||getEMailUser().length()==0)){
			log.saveError("Error", Msg.getElement(getCtx(), "EMailUser") +" " + Msg.getMsg(getCtx(), "FieldMandatory"));
			return false;
		}
		
		//Redmine 83298
		if (get_ValueAsBoolean("IsVendorNobilia") && is_ValueChanged("IsVendorNobilia"))
		{
			int no = QueryUtil.getSQLValue(get_Trx(), 
					"SELECT COUNT(*) FROM AD_User WHERE IsVendorNobilia='Y' AND AD_Client_ID=? ", getCtx().getAD_Client_ID());
			if (no>1)
			{
				log.saveError("Error", Msg.getMsg(getCtx(), "ExistVendorNobilia"));
				return false;
			}
		}
		
		if(is_ValueChanged("EMail") && getEMail() !=null && getEMail().trim().length()>0){
			String Email = getEMail().trim();
			if(EmailUtil.isEmailAdress(Email))
				setEMail(Email.toLowerCase());
			else
			{
				setEMail(Email.toLowerCase());
				p_changeVO.addWarning(Msg.getMsg(getCtx(), "EMAIL_NOT_CORRECT"));
			}
		}
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		//93706 Vérifier que au moins 1 contact par défaut IsEggo = Y
		if(getC_BPartner_ID()>0)
		{
			String sql = "Select count(*) from AD_User where C_BPartner_ID = ? and IsActive='Y' and IS_ZDefault = 'Y' ";
			int no = DB.getSQLValue(get_Trx(), sql, getC_BPartner_ID());
			if(no < 1)
			log.saveWarning("Warning", Msg.getMsg(getCtx(), "NO_DEF_USER"));
			if(no > 1)
			//Décocher les plus ancien
				DB.executeUpdate("Update AD_User set IS_ZDefault = 'N', Updated = sysdate, updatedBy = ? "
					+ "where C_BPartner_ID = ? and AD_User_ID <> ? and IS_ZDefault = 'Y' ", 
					new Object[]{ getCtx().getAD_User_ID(), getC_BPartner_ID(), getAD_User_ID()}, get_Trx());
		}
