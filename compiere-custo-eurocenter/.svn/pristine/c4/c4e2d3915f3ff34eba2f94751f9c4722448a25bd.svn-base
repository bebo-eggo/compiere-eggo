 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: ZoomTarget2.java 8753 2010-05-12 17:34:49Z nnayak $
 */
public class ZoomTarget2 {

	/**	Static Logger	*/
	private static CLogger	log	= CLogger.getCLogger (ZoomTarget2.class);
	...
	public static int getZoomAD_Window_ID (String targetTableName, String targetWhereClause,CContext m_context)
	{
		return getZoomAD_Window_ID (  targetTableName,   0 ,   targetWhereClause, true,  m_context);
	} 
	public static int getZoomAD_Window_ID (String targetTableName, int curWindow_ID, String targetWhereClause, boolean isSOTrx)
	{
		return getZoomAD_Window_ID (  targetTableName,   0 ,   targetWhereClause, true,  (CContext)Env.getCtx());
	}
	/**
	 ...
	 */
	public static int getZoomAD_Window_ID (String targetTableName, int curWindow_ID, String targetWhereClause, boolean isSOTrx,CContext m_context)
	{
...
//		int xx = Env.getCtx().getAD_Client_ID();
//		System.out.println("xx");
//		// Si AD_Client = Eggo et isSotrx = Y => Tiers Eggo
//		String sqlx = "Select AD_Client_ID from " + targetTableName + " where "
//				+ targetWhereClause;
//		// END hard code for Zooming into Production resource.
...
//		int AD_Client_ID = 0;
//		try {
//			pstmtx = DB.prepareStatement(sqlx, (Trx) null);
//			int index = 1;
//			rsx = pstmtx.executeQuery();
//
//			if (rsx.next()) {
//				AD_Client_ID = rsx.getInt(1);
//			}
//		} catch (SQLException e) {
//			log.log(Level.SEVERE, sqlx, e);
//		} finally {
//			DB.closeResultSet(rsx);
//			DB.closeStatement(pstmtx);
//		}
//
//		if (isSOTrx 
//				&& 1000000 == AD_Client_ID && (zoomWindow_ID == 123 || zoomWindow_ID == 108) && curWindow_ID==143) {
//			String ret = Msg.getMsg(Env.getCtx(), "EGGO_TIERS_WINDOW");
//			if (!ret.equals("0"))
...
//		}
       ...
       if(PO_zoomWindow_ID !=0 && zoomWindow_ID !=0 && targetWhereClause !=null && targetWhereClause.length()!=0)
       {
		...
		if(targetWhereClause!=null && targetWhereClause.length() !=0)
		{
			...
			zoomList= ZoomTarget2.getZoomTargets(targetTableName, curWindow_ID, targetWhereClause,  m_context);
	 ...
	 */
	public static ArrayList<KeyNamePair> getZoomTargets (String targetTableName, int curWindow_ID, String targetWhereClause, Object[] params,CContext m_context)
	{
		if (params.length != 1)
			return null;
		...
		return getZoomTargets(targetTableName, curWindow_ID, whereClause,  m_context);
	 ...
	 * 	@param targetWhereClause Where Clause in the format "Record_ID=<value>"
	 */
	public static ArrayList<KeyNamePair> getZoomTargets (String targetTableName, int curWindow_ID, String targetWhereClause,CContext m_context)
	{
		/**
		 * 	Window WhereClause 
		 */
		...
		//ZCOM999 on ne prend que les windows actives
		if(targetTableName.equalsIgnoreCase("C_BPartner"))
		{
			if (baseLanguage)
			{
				//ZCOM999 sql += "INNER JOIN AD_Window w ON (tt.AD_Window_ID=w.AD_Window_ID)";
				sql += "INNER JOIN AD_Window w ON (tt.AD_Window_ID=w.AD_Window_ID AND w.ISActive = 'Y')"; //ZCOM999
			...
			{
				sql += "INNER JOIN AD_Window_Trl w ON (tt.AD_Window_ID=w.AD_Window_ID AND w.AD_Language=? and exists(select 1 from AD_WINDOW_ACCESS where AD_WINDOW_ACCESS.AD_Window_ID = w.AD_Window_ID and AD_WINDOW_ACCESS.AD_ROLE_ID = ? ))";
				sql += "INNER JOIN AD_Window zz ON (zz.AD_Window_ID=w.AD_Window_ID AND zz.ISActive = 'Y')"; //ZCOM999
				sql += " LEFT OUTER JOIN AD_Window_Trl ws ON (t.AD_Window_ID=ws.AD_Window_ID AND ws.AD_Language=? and exists(select 1 from AD_WINDOW_ACCESS where AD_WINDOW_ACCESS.AD_Window_ID = ws.AD_Window_ID and AD_WINDOW_ACCESS.AD_ROLE_ID = ? ))"
					+  " LEFT OUTER JOIN AD_Window_Trl wp ON (t.PO_Window_ID=wp.AD_Window_ID AND wp.AD_Language=? and exists(select 1 from AD_WINDOW_ACCESS where AD_WINDOW_ACCESS.AD_Window_ID = wp.AD_Window_ID and AD_WINDOW_ACCESS.AD_ROLE_ID = ? ))";
			}
			sql	+= "WHERE t.TableName = ?"
				+ " AND w.AD_Window_ID <> ? AND w.isActive='Y'"
				+ " AND tt.SeqNo=10"
				+ " AND (wp.AD_Window_ID IS NOT NULL "
						+ "OR EXISTS (SELECT 1 FROM AD_Tab tt2 WHERE tt2.AD_Window_ID = ws.AD_Window_ID AND tt2.AD_Table_ID=t.AD_Table_ID AND tt2.SeqNo=10) OR w.ad_window_id IS NOT NULL )"
				
						//Bug Roles
						//+ " and exists(select 1 from AD_WINDOW_ACCESS where AD_WINDOW_ACCESS.AD_Window_ID = w.AD_Window_ID and AD_WINDOW_ACCESS.AD_ROLE_ID = ? ) "		
		...
		try
		{
			...
			if(targetTableName.equalsIgnoreCase("C_BPartner"))
			{
				if (!baseLanguage)
				{
					pstmt.setString (index++, Env.getAD_Language(Env.getCtx()));pstmt.setInt(index++, m_context.getAD_Role_ID()); 
					pstmt.setString (index++, Env.getAD_Language(Env.getCtx()));pstmt.setInt(index++, m_context.getAD_Role_ID()); 
					pstmt.setString (index++, Env.getAD_Language(Env.getCtx()));pstmt.setInt(index++, m_context.getAD_Role_ID()); 
				}
			}
			...
			while (rs.next())
			{
				...
				if(targetTableName.equalsIgnoreCase("C_BPartner"))
				{
					if(PO_Window_ID==0 && zoom_Window_ID==0)
					{
						zoom_Window_ID=rs.getInt(1);
						zoom_WindowName = rs.getString(2);
						PO_Window_ID=rs.getInt(1);
					}
				}
		...
		if (targetTableName.equalsIgnoreCase(X_C_BankStatement.Table_Name))//Redmine 62561
		{	
			StringBuffer sqlBody= new StringBuffer("");
			for (int i=0; i< windowList.size();i++)
			{	
				if(i!=0)
				sqlBody.append(" UNION "); 
				sqlBody.append("(Select count(*),"+windowList.get(i).AD_Window_ID+" as zoom_Window_ID,'"+windowList.get(i).windowName+"' as zoom_WindowName");						
				sqlBody.append(" FROM " +targetTableName + " WHERE ");
				sqlBody.append(targetWhereClause);
				if (windowList.get(i).whereClause!=null)
				sqlBody.append(" AND ("+windowList.get(i).whereClause+")");
				sqlBody.append(")");
				
			}
			sql1=sqlBody.toString();
		}
		else                                                    //#62561
			...
			while (rs.next())
			{
				if (columns.size() > 0)
				{
					...
					for(int i=0; i<columns.size();i++)
					{
						...
						//Redmine #20017: nullpointexception si la valeur de la colonne est null
					...
					//Anomalie #35553 : Verify that the window entered on AD_Table.AD_Window_ID can be used to show record infos
					boolean zoomListFilled = false;
					for (int i=0; i<windowList.size(); i++)
					{
						if((windowList.get(i).AD_Window_ID == zoom_Window_ID || windowList.get(i).AD_Window_ID == PO_Window_ID) 
								&& evaluateWhereClause(columnValues,windowList.get(i).whereClause)){
				...
				{
					if (targetTableName.equalsIgnoreCase(X_C_BankStatement.Table_Name))//#62561
					{	
						zoom_Window_ID=rs.getInt(2); 
						zoom_WindowName=rs.getString(3);
					}	                                                     //#62561
