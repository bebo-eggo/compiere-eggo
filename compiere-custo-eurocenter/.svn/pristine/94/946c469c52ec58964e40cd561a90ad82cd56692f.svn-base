...
import org.compiere.vos.ListBoxVOExt;
 ...
 *
 *  @author Jorg Janke
 *  @version $Id: Login.java 8759 2010-05-12 23:46:40Z nnayak $
 */
public class Login
{
	/**
	...
	public static boolean isJavaOK (boolean isClient)
	{
		...
		if (jVersion.startsWith("1.6") || jVersion.startsWith("1.8"))
			return true;
		//  Warning
		boolean ok = false;
	...
	public KeyNamePair[] getRoles (ListBoxVOExt listBoxVo,String app_user )
	{
		return getRoles (listBoxVo, app_user, null, false, true);
	}
	...
	//ZCOM999 38665-39021
	public KeyNamePair[] getRoles (ListBoxVOExt listBoxVo,  String app_user, String app_pwd, int W_Store_ID)
	{
		KeyNamePair[] roles = null;
		if (W_Store_ID >= 0)
		{
		...
		if (roles == null)	//	Regular
			roles = getRoles (listBoxVo, app_user, app_pwd, false, false);
		return roles;
	}   //  login
	//ZCOM999 38665-39021
	...
	private KeyNamePair[] getRoles (String app_user, String app_pwd, boolean force, boolean ignore_pwd )
	{
		...
		if (system.isLDAP() && !system.get_ValueAsBoolean("Enabled3rdPartyAuthentication"))
		{
			authenticated = system.isLDAP(app_user, app_pwd);
			if (authenticated)
			{
				logingUser = getLoginUserWRoleAccess(app_user,app_pwd,system.isLDAP() );//ZCOM999 38665-39021
				app_pwd = null;
			}
		}
		...
		if(!authenticated)
			getLoginUserWRoleAccess(app_user,app_pwd,false );
		
		if(logingUser!=null && logingUser.get_ValueAsBoolean("IsLDAPOnlyLogin") && authenticated)
		{
		...
		if (app_pwd != null && logingUser!=null &&  !logingUser.get_ValueAsBoolean("IsOnlyLDAPLogin") && !authenticated)
	...
	private KeyNamePair[] getRoles (ListBoxVOExt listBoxVo, String app_user, String app_pwd, boolean force, boolean ignore_pwd )
	{
		log.info("User=" + app_user);
		long start = System.currentTimeMillis();
		if (app_user == null)
		...
		if(!authenticated)
			logingUser = getLoginUserWRoleAccess(app_user,app_pwd,false);//ZCOM999 38665-39021
		
		if(logingUser!=null && logingUser.get_ValueAsBoolean("IsLDAPOnlyLogin") && !authenticated)
		{
			log.saveError("UserPwdError", app_user, false);
			checkFailedLoginAttempt(listBoxVo, logingUser);//ZCOM999 38665-39021
			return null;
		}
		...
		resetFailedLoginAttempt(logingUser);//ZCOM999 38665-39021
	...
	//ZCOM999 38665-39021
//	private MUser getLoginUser(String userName)
//	{
//		String sql = "SELECT AD_User_ID FROM AD_User WHERE Name = ?";
//		int userID = DB.getSQLValue(null, sql, userName);
//		
//		if(userID <= 0)
//			return null;
//		
//		MUser logingUser = MUser.get(m_ctx, userID);
//		return logingUser;
//	}
	
	private MUser getLoginUserWRoleAccess(String userName,String app_pwd, boolean isLDAP)
	{
		
		String sql = "SELECT AD_User_ID FROM AD_User WHERE Name = ? and Exists (Select 1 from AD_USER_ROLES where AD_USER_ID = AD_User.AD_User_ID and IsActive='Y') and IsActive='Y'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		MUser logingUser = null;
		try
		{
			...
			while (rs.next())		//	no record found
			{
				logingUser = MUser.get(m_ctx, rs.getInt(1));
				logingUser.setPassword(app_pwd);
				logingUser.save();
			}
		}catch (SQLException ex) {
			log.log(Level.SEVERE, sql.toString(), ex);
	...
	private MUser checkFailedLoginAttempt(ListBoxVOExt listBoxVo, MUser logingUser)
	{
		
		if(logingUser != null && (!logingUser.getName().toUpperCase().equals("SUPERUSER") || isSuperUserIncluded()))
		{
			int maxAuthorizedAttempst = getMaxAuthorizedAttempts();
			if(maxAuthorizedAttempst > 0)
			{
				int userFailedAttempts = logingUser.get_ValueAsInt("Z_FailedLoginAttempts");
				if(++userFailedAttempts > maxAuthorizedAttempst)
				{
					listBoxVo.setHasError(true);
					listBoxVo.setMsgError(Msg.getMsg(m_ctx, GlobalMessage.NB_FAILED_LOGIN_ATTEMPTS_ERROR_MSG));
					log.saveError(GlobalMessage.NB_FAILED_LOGIN_ATTEMPTS_ERROR_MSG, logingUser.getName(), false);
					logingUser.set_Value("Z_FailedLoginAttempts", userFailedAttempts);
					logingUser.setIsActive(false);
				}else
				{
					logingUser.set_Value("Z_FailedLoginAttempts", userFailedAttempts);
				}
				logingUser.save(null);
			}
		}
	...
	private void resetFailedLoginAttempt(MUser logingUser)
	{
		if(logingUser != null)
		{
			logingUser.set_Value("Z_FailedLoginAttempts", 0);	
			logingUser.save(null);
		}
	}
	
	private boolean isSuperUserIncluded()
	{
		boolean retValue = false;	
		String valueMessage = Msg.getMsg(Locale.US.getLanguage(), GlobalMessage.NB_FAILED_LOGIN_ATTEMPTS_SUPER_USER_IMPACTED);
		retValue = (valueMessage != null && valueMessage.equals("Y"));	
		return retValue;
	}

	private int getMaxAuthorizedAttempts() {
		
		int retValue = -1;
		try {
			String valueMessage = Msg.getMsg(Locale.US.getLanguage(), GlobalMessage.NB_FAILED_LOGIN_ATTEMPTS);
			if(valueMessage != null && !valueMessage.equals(""))
			{
				Integer maxAttempts = Integer.valueOf(valueMessage);
				retValue = maxAttempts;
			}
	...
	public KeyNamePair[] getClients (KeyNamePair role)
	{
		...
		try
		{
			...
			//  Role Info
			MSystem system = MSystem.get(m_ctx);
			m_ctx.setContext("#Enabled3rdParty", system.get_ValueAsString("Enabled3rdPartyAuthentication")==null?"N":system.get_ValueAsString("Enabled3rdPartyAuthentication"));
	...
	public String validateLogin (KeyNamePair org)
	{
		...
		log.info(info);
		//MERGE
		//PosCtx.setConstants(m_ctx, null);
		//EggoCtx.setConstants(m_ctx, AD_Client_ID); //MERGE
		//MERGE
		return null;
	}	//	validateLogin
		...
		KeyNamePair warehouse, java.sql.Timestamp timestamp, String printerName)
	{
		...
		try
		{
			...
			{
				Object[][] results = QueryUtil.executeQuery(sql, queryParams.params.toArray());
		...
		// ZCOM999 : Permettre plus de colonnes
		int nbCols = 4;
		String s = Msg.getMsg("en_US", "Z_AUDAXIS_WEB_COLS", true).trim();
		try {
			nbCols = 2 * Integer.valueOf(s);
		}
		catch (Throwable t)
		{
		}
		m_ctx.setContext("#Z_AUDAXIS_WEB_COLS", nbCols);
		// ZCOM999 : Permettre plus de colonnes

		//set Version
		m_ctx.setContext("##VersionSmile", Msg.getMsg("fr_FR", Version.VersionSmile, true).trim());
		m_ctx.setContext("##VersionPOS",  Msg.getMsg("fr_FR", Version.VersionPOS, true).trim());
		return retValue;
	}	//	loadPreferences
	...
	public boolean batchLogin(java.sql.Timestamp loginDate, String printerName)
	{
		...
		KeyNamePair whPP = null; //ZCOM999
		
		if (whs == null || whs.length == 0)
		{
			log.severe("No Warehouses for Org: " + org);
//ZCOM999	return false;
		}
		else { //ZCOM999
		//ZCOM999 KeyNamePair whPP = null;
	...
	**/
	...
	public KeyNamePair[] getRoles(String app_user) {
		return this.getRoles(app_user, (String) null, false, true);
	}
	
	public String getOrgWarehouseByRoleUser(String User, String Role, String tenant)
	{
		String ret = null;
		String sql = "Select ar.Name, ao.NAme, NVL(mw.Name,'NULL') from AD_USER_ROLES aur "+
				"inner join AD_Client ac on ac.AD_CLIENT_ID = aur.AD_Client_ID and ac.Name = ? "+
				"inner join AD_User au on au.AD_User_ID = aur.AD_User_ID and au.Name = ? "+
				"inner join AD_role ar on ar.AD_Role_ID = aur.AD_Role_ID and ar.name = ? "+
				"left join AD_ROLE_ORGACCESS aro on aro.AD_ROLE_ID = ar.AD_ROle_ID and aro.ISActive='Y' "+
				"left join AD_Org ao on ao.AD_Org_ID = aro.AD_Org_ID "+
				"left join AD_ORGINFO aoi on ao.AD_Org_ID = aoi.AD_Org_ID "+
				"left join M_Warehouse mw on aoi.M_WAREHOUSE_ID = mw.M_WAREHOUSE_ID "+
				"where aur.ISActive='Y'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = DB.prepareStatement(sql, (Trx) null);
