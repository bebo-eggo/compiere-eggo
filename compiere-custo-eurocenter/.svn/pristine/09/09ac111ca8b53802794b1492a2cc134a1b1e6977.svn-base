...
public final class ImpFormat
{
	...
	public static final String FORMATTYPE_SEMICOLON = "S";
	public static final String FORMATTYPE_PIPE = "P";
	...
	public void setFormatType(String newFormatType)
	{
		if (newFormatType.equals(FORMATTYPE_FIXED) || newFormatType.equals(FORMATTYPE_COMMA)
			...
			|| newFormatType.equals(FORMATTYPE_SEMICOLON) || newFormatType.equals(FORMATTYPE_PIPE))
	...
	public String[] parseLine (String line, boolean withLabel, boolean trace, boolean ignoreEmpty)
	{
		...
		for (int i = 0; i < m_rows.size(); i++)
		{
			...
			if (row.isConstant())
				info = "Constant";
			else if (row.getColumnName().equalsIgnoreCase("Attributs")){
				info = parseSpecial(line, "S", row.getStartNo());
			}
			...
			//	Interpret Data
			if (row.getColumnName().equalsIgnoreCase("Attributs"))
				entry.append(info);	
			else
	...
	private String parseFlexFormat (String line, String formatType, int fieldNo)
	{
		...
		if (formatType.equals(FORMATTYPE_COMMA))
			...
			delimiter = '\t';
		else if (formatType.equalsIgnoreCase(FORMATTYPE_SEMICOLON))
			delimiter = ';';
		else if (formatType.equalsIgnoreCase(FORMATTYPE_PIPE))
			delimiter = '|';
		/*ZCOM731*/
		else
	...
	//	log.config( "ImpFormat.updateDB - listSize=" + nodes.length);
		...
		//  Update Cmd
		int no = DB.executeUpdate(sql.toString(), new Object[]{UpdatedBy, ID}, trx);
		if (no != 1)
		{
	...
	private String parseSpecial(String line, String formatType, int fieldNo){
		final char QUOTE = '"';
		//  check input
		char delimiter = ' ';
		/*ZCOM731*/
		if (formatType.equalsIgnoreCase("S"))
			delimiter = ';';
