 ...
 * 
 * @author Jorg Janke
 * @version $Id: UITab.java 8751 2010-05-12 16:49:31Z nnayak $
 */
public class UITab extends UITabVO {
	...
	// ZCOM872	public static final int MAX_QUERY_RESULT_SIZE = 50000; // ZCOM872
	public static int MAX_QUERY_RESULT_SIZE = getMAX_QUERY_RESULT_SIZE(); // ZCOM872

	//ZCOM872
	private static int getMAX_QUERY_RESULT_SIZE()
	{
		int max = 50000;
		String msg = Msg.getMsg(Language.AD_Language_en_US, "AUDAXIS_WEB_MAXROWS", true);
		try {
			max = Integer.parseInt(msg);
		} catch (Throwable t) {
//			log.log(Level.WARNING, "E", t);
		}
		return max;
	}
	//ZCOM872
	...
	//Redmine 30745
	static int[] tabs = PO.getAllIDs(X_AD_Tab.Table_Name, " Value IN ('InvoiceValidator','InvoiceValidatorLine')", null);
	...
	protected void initialize(ArrayList<UIField> fields,
		...
		if (fields == null)
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				//Redmin 30745
				if (exist && !field.getColumnName().equals("DocAction")){
					field.setIsReadOnly(true);
					field.setIsAlwaysUpdateable(false);
				}
	...
	public Object[][] executeQuery(QueryVO queryVO,
		...
		try {
			results = QueryUtil.executeQuery(sql1, whereClause.params.toArray(), 0, (MAX_QUERY_RESULT_SIZE + 1), null);
		...
		if (queryVO.restrictions != null && queryVO.restrictions.size() > 0) {
			...
			for (int i = 0; i < queryVO.restrictions.size(); i++) {
				...
				if (checkWhereClause(vo.DirectWhereClause)) {
					...
					//ZCOM999 SC781
					restriction.setColumnDisplyType(vo.DisplayType);
					//ZCOM777
					String columnSql = getColumnSql(getTableName(), vo.ColumnName);
					restriction.setColumnSql(columnSql);
					//ZCOM777
	...
	private ArrayList<String> getUpdatedFieldsByOthers(PO po, String[] cachedRow) {
		...
		for (int i = 0; i < size; i++) {
			...
			// #5797
			//ZCOM999 if (!v.equals(po.get_ValueAsString(colName)))
			//ZCOM999 Start : Sauver l'ecran des utilisateurs en web donne une erreur à cause du mot de passe encrypter
			String poValue = po.get_ValueAsString(colName) == null ? "" : po.get_ValueAsString(colName);
			
			if((field.isEncryptedField() || field.isEncryptedColumn())) {
				String v_new = v;
				String poValue_new = poValue;
				if (SecureEngine.isEncrypted(v))
					v_new = SecureEngine.decrypt(v);
				if (SecureEngine.isEncrypted(poValue))
					poValue_new = SecureEngine.decrypt(poValue);
				if (!v_new.equals(poValue_new))
					conflictedFields.add(colName);
			}
			//Anomalie #37610 COMPTA: Erreur paiement manuel partiel
			else if (!isEqual(po, colName, v))
				//#37610
			//ZCOM999 END
				conflictedFields.add(colName);
		}
	...
	public ChangeVO saveRow(CContext ctx, int windowNo, boolean newRecord,
		...
		//ZCOM666 En cas d'exception dans le save() (Triggers) 
		//ZCOM666 il faut qu'on fasse quelque chose avec la transaction courante
		//ZCOM666 START
		boolean ok = true;
		try {
			ok = po.save();
	...
	public ChangeVO refreshRow(CContext ctx, int windowNo) {
		...
		if (po == null) {
			...
			c.rowData = new String[getColumnNames().length]; //ZCOM
	...
	private ChangeVO processCallout(CContext ctx, int windowNo, UIField field,
		...
		// External Callout
		if (!GwtMode.isGwtMode(ctx)){//Redmine 22558: ne pas appeler les callout GUI et WEBUI ZCOM999
	...
	private void processCalloutExternal(ChangeVO change, PO po, int windowNo,
		...
		try {
			...
			/*ZCOM #21931 Ne pas montrer aux utilisateurs les conneries des développeurs
			 * que ceux ci se démerdent dans les log
	...
	{		
		...
		// if the values are different, the record is assumed to be changed.
			...
			try
			{
				...
				//ZCOM999 si le parent est en multi ID le SQL est faux throw new DBException(e);
				log.log(Level.SEVERE, "E", e); //ZCOM999
				dbProcessedS = "N"; //ZCOM999
			}
			finally {
				DB.closeResultSet(rs);
				DB.closeStatement(pstmt);
	...
	/**
	 * ZCOM777 Traitement des ColumnSql dans les Query
	 * @param tableName
	 * @param columnName
	 * @return
	 */
	private String getColumnSql(String tableName, String columnName) {
		...
		ResultSet rs = null;
		try {
			ps = DB.prepareStatement("SELECT ColumnSql FROM AD_Column c INNER JOIN AD_Table t ON c.AD_Table_ID = t.AD_Table_ID WHERE t.TableName=? and c.ColumnName=?", null);
	...
	//#37610
    public boolean isEqual(final PO po, final String columnName, final String compare)
    {
            final Object value = po.get_Value(columnName);
            
            if( value instanceof String ) {
                    return ((String) value).equals(compare);     
            }
            else if (value == null) {
            ...
            else if (value instanceof Timestamp)
            {
                    ...
                    return String.valueOf(time).equals(compare);       
            }
            else if(value instanceof Boolean) {
                    if(((Boolean)value).booleanValue())
                            return "Y".equals(compare);
                    else
                            return "N".equals(compare);    
            }
            else if(value instanceof BigDecimal){
                    BigDecimal comp = new BigDecimal(compare);    
                    return ((BigDecimal)value).compareTo(comp) == 0; 
            }
                    return value.toString().equals(compare);      
    }        // #37610
