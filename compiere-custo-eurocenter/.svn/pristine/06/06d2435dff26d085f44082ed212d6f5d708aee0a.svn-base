 ...
 */
public final class MDefLocatorLookup extends Lookup implements Serializable {
    /** Logger for class MLocatorLookup */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MDefLocatorLookup.class);
	/**
	 * 
	 */
	 ...
	 *            window no
	 */
	public MDefLocatorLookup(Ctx ctx, int WindowNo) {
	...
	/** Only Def. Loacator */
	private Boolean m_only_DefLoc = false;

	private Boolean m_only_procure = false;
	 ...
	 * @return def Loc
	 */
	public boolean isOnly_DefLoc() {
		return m_only_DefLoc;
	} // getOnly_Product_IDef_l
	 ...
	 *            Product
	 */
	public void setOnly_DefLoc(boolean only_DefLoc) {
		m_only_DefLoc = only_DefLoc;
	} // setOnly_Product_ID

	public boolean isOnly_Procure() {
		return m_only_procure;
	} // getOnly_Product_IDef_l
	 ...
	 *            Product
	 */
	public void setOnly_Procure(boolean only_procure) {
		m_only_procure = only_procure;
	} // setOnly_Product_ID
	...
	private final Callable<LinkedHashMap<Integer, KeyNamePair>> m_loader = new Callable<LinkedHashMap<Integer, KeyNamePair>>() {
		...
		public LinkedHashMap<Integer, KeyNamePair> call() throws Exception {
			...
			boolean only_DefLoc = isOnly_DefLoc();  
			boolean only_Procure = isOnly_Procure();  
			...
			if (only_DefLoc && !only_Procure) 
				sql.append(" AND IsDefault='Y' "); // Default Locator
			if (only_DefLoc && only_Procure) 
				sql.append(" AND (IsDefault='Y' OR PriorityNo = 93 )"); 
			...
			try {
				...
				while (rs.next()) {
					...
					// row returned
					if (loc.isDefault() || rows == 1) {
						m_defaultID = Integer.toString(M_Locator_ID);
					}
					if (m_procureID.length() == 0
							&& (loc.getPriorityNo() == 93)) {
						m_procureID = Integer.toString(M_Locator_ID);
					}
