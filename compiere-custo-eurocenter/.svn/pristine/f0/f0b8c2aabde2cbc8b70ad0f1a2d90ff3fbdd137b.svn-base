 ...
 * 
 * @author Jorg Janke
 * @version $Id: MRole.java 8755 2010-05-12 18:30:14Z nnayak $
 */
public final class MRole extends X_AD_Role {
	...
	public static MRole get(Ctx ctx, int AD_Role_ID, int AD_User_ID,
		...
		if (gwtServerID > 0) {
			...
			s_log.config("ServerID=" + gwtServerID + ": " + role.toString());
			return role;
		}
	...
	public String updateAccessRecords() {
		...
		int winDel = DB.executeUpdate(
				"DELETE FROM AD_Window_Access" + whereDel, get_Trx());
		int win = DB.executeUpdate(sqlWindow + roleAccessLevelWin, get_Trx());
		int procDel = DB.executeUpdate("DELETE FROM AD_Process_Access"
				+ whereDel, get_Trx());
		int proc = DB.executeUpdate(sqlProcess + roleAccessLevel, get_Trx());
		int formDel = DB.executeUpdate("DELETE FROM AD_Form_Access" + whereDel,
				get_Trx());
		int form = DB.executeUpdate(sqlForm + roleAccessLevel, get_Trx());
		int infoWindowDel = DB.executeUpdate("DELETE FROM AD_InfoWindow_Access" + whereDel,
				get_Trx());
		int infoWindow = DB.executeUpdate(sqlInfoWindow + roleAccessLevel, get_Trx());
		int wfDel = DB.executeUpdate("DELETE FROM AD_Workflow_Access"
				+ whereDel, get_Trx());
	...
	private void loadOrgAccessUser(ArrayList<OrgAccess> list) {
		...
		//EC Add All Org
		MClient cli = new MClient(getCtx(), getCtx().getAD_Client_ID(), get_Trx());
		int orgID[] = PO.getAllIDs("AD_Org", " AD_Client_ID = " + cli.getAD_Client_ID(), get_Trx());
		for (int i = 0; i < orgID.length; i++) {
			loadOrgAccessAdd(list, new OrgAccess(cli.getAD_Client_ID(), orgID[i]
					, false));
		}
		//EC
	...
	//EC Start 
	public boolean isUsersAccessAllow(int orgID, int userID){
		if (isUseUserOrgAccess())
			for (OrgAccess element : m_orgAccess)
				if (element.AD_Org_ID == orgID && element.readOnly) return false;
		return true;
	}	
	// EC End
	...
	public boolean canUpdate(int AD_Client_ID, int AD_Org_ID, int AD_Table_ID,
		...
		// Org Access
		retValue = isROOrgAccess(AD_Org_ID);
	...
	//EC
	private HashMap<Integer, Integer> m_accessOrg = null;
	public boolean isROOrgAccess(int OrgRecord)
	{
		if (m_accessOrg == null ){
			// Load access for One User
			m_accessOrg = new HashMap<Integer, Integer>();
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
				pstmt = DB.prepareStatement ("select ad_org_id from ad_user_orgAccess WHERE IsActive='Y' and  ad_user_id = ? and isReadOnly = 'N' and AD_Client_ID = ?", null);
				pstmt.setInt (1, getCtx().getAD_User_ID());
				pstmt.setInt (2, getCtx().getAD_Client_ID());
				rs = pstmt.executeQuery();
				while (rs.next()){
					m_accessOrg.put(rs.getInt(1), rs.getInt(1));
				} 
			}
		...
		return ( m_accessOrg.get(OrgRecord) != null   || m_accessOrg.size() == 0 || getCtx().getAD_Client_ID() == 0   ) ;
	}
//EC	
