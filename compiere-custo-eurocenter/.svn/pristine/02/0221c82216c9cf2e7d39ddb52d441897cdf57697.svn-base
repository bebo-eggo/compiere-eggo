...
public class ColumnSync extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		int no = 0;
		// SYNCHRO DB MAGASIN
		boolean isSync = table.get_ValueAsBoolean("IsDownSynchronized");
				...
				if(modified)
				{
					...
					try
					{
						...
						if(createfk)
						{
							...
							// SYNCHRO DB MAGASIN
							if(isSync){
								syncDBWarehouse(sql.toString(),dbs);
							}
						} //if createfk
	...
	private void syncDBWarehouse(String sql,MSyncDatabase[] dbs) throws SQLException{
		for(MSyncDatabase db : dbs){
			Trx trx = Trx.getAutoCommitTrx();
			CallableStatement cstmt = trx.getConnection().prepareCall("call dbms_utility.exec_ddl_statement@"+db.getDatabaseLinkName()+"('"+sql+"')");
			try {
				int no = cstmt.executeUpdate();
				addLog (0, null, new BigDecimal(no),"exec dbms_utility.exec_ddl_statement@"+db.getDatabaseLinkName()+"('"+sql+"')");
