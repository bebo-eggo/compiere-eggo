 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MTable.java 8755 2010-05-12 18:30:14Z nnayak $
 */
public class MTable extends X_AD_Table
implements Comparable<MTable> //ZCOM999
{
	...
	static String[] getPackages(Ctx ctx)
	{
		...
		list.add("compiere.model");		//	default ZCOM999
	...
	public ArrayList<String> getColumnNames ()
	{
		...
		for (int i = 0; i < m_columns.length; i++) {
			ret.add(m_columns[i].getColumnName().toUpperCase());
		} 
		return ret;
	}
...
//			TODO check return null;
	...
	/**
	 * ZCOM999 Pour ADreferenceGenerate
	 */
	@Override
	public int compareTo(MTable arg0) {
		return getTableName().compareTo(arg0.get_TableName());
	}
	...
	/** ZCOM999*/
	protected TreeMap<MTable, TreeMap<String, MTable>> internalLinkedTables = new TreeMap<MTable, TreeMap<String, MTable>>();
	/** ZCOM999*/
	protected TreeMap<MTable, TreeMap<String, MTable>> externalLinkedTables = new TreeMap<MTable, TreeMap<String, MTable>>();
	
	/**
	 * ZCOM999
	 * Example : C_BPartner for C_Order
	 * 
	 * @return a Map of Property/M_Table link (Bill_BPartner_ID/C_BPartner, etc...)
	 */
	public TreeMap<String, MTable> getInternalLinkedTables(){
		TreeMap<String, MTable> ret = internalLinkedTables.get(this);
		if (DEBUG)
			ret = null;
		if (ret == null){
			TreeMap<String, MTable> tables = new TreeMap<String, MTable>();
			MColumn[] cols = getColumns(true);
			for (int i = 0; i < cols.length; i++) {
				int tableId = -1;
				MColumn col = cols[i];
				// Table Direct / Search
				if (col.getAD_Reference_ID() == 19
						|| (col.getAD_Reference_ID() == 30 && col.getAD_Reference_Value_ID() == 0)) { 
					String tableName = col.getColumnName().substring(1 + col.getName().indexOf("_"), col.getColumnName().length() - 3);
					tableId = PO.get_Table_ID(tableName);
					s_log.finest(col.getColumnName()+ " - Table Direct/Search");
				}
				else if (col.getAD_Reference_ID() > 16
						&& col.getAD_Reference_ID() != 20 
						&& col.getAD_Reference_ID() != 24
						&& col.getAD_Reference_ID() != 28
						&& col.getAD_Reference_ID() != 29
						&& col.getAD_Reference_ID() != 34
						&& col.getAD_Reference_ID() != 37) {
					// Le reste : table, search, ...
					if (col.getAD_Reference_Value_ID() > 0) {
						X_AD_Reference ref = new X_AD_Reference(getCtx(), col.getAD_Reference_Value_ID(), null);
						if (ref.getValidationType().equalsIgnoreCase(X_AD_Reference.VALIDATIONTYPE_TableValidation)){
							tableId = DB.getSQLValue(null, "select ad_table_id from ad_ref_table where ad_reference_id = ?",ref.getAD_Reference_ID());
							s_log.finest(col.getColumnName()+ " - Table/Search with REFERENCE");
						}
						else if (ref.getValidationType().equalsIgnoreCase(X_AD_Reference.VALIDATIONTYPE_ListValidation)){
							// List Validation
							s_log.config("Not Implemented : "+col.getColumnName() + " " +col.getAD_Element_ID());
						}
					}
					// Ceci marche pour les C_Location_ID par exemple (Reference = Location)
					else if (col.getColumnName().endsWith("_ID")){
						String tableName = col.getColumnName().substring(1 + col.getName().indexOf("_"), col.getColumnName().length() - 3);
						tableId = PO.get_Table_ID(tableName);
						s_log.finest(col.getColumnName()+ " - Table/Search !!!!!!!!!!!!!!!");
					}
					// Special types : Account, Attributes, Locators, ...
					else {
						s_log.config("Not Implemented : "+col.getColumnName() + " " +col.getAD_Element_ID());
					}
				}
				if (tableId > 0){
					MTable t = MTableUtil.getZ (getCtx(), tableId);
					tables.put(col.getColumnName(), t);
				}
			}
			ret = tables;
			internalLinkedTables.put(this, ret);
		}
		return ret;
	}
	...
	/**
	 * ZCOM999
	 * Example : C_OrderLine for C_Order
	 * 
	 * @return a Map of Property/M_Table link (listC_OrderLine/C_OrderLine, etc...)
	 */
	public TreeMap<String, MTable> getExternalLinkedTables(){
		TreeMap<String, MTable> ret = externalLinkedTables.get(this);
		if (DEBUG)
			ret = null;
		if (ret == null){
			int tableId = PO.get_Table_ID(getTableName());
			TreeMap<String, MTable> tables = new TreeMap<String, MTable>();
			// 1. Prendre toutes les tables ayant en column l'id de la table courante
			// Sur cet ID il faut que le ref_value_id soit = 0
			PreparedStatement ps = null;
			ResultSet rs = null;
			try{
				StringBuffer sql = new StringBuffer("select * from ad_column where upper(columnname) = upper(?) and ad_table_id <> ?");
				ps = DB.prepareStatement(sql.toString(), null);
				ps.setString(1, getTableName()+"_ID");
				ps.setInt(2, tableId);
				rs = ps.executeQuery();
				while (rs.next()){
					MColumn col = new MColumn(Env.getCtx(), rs, null);
					// Table Direct && Search
					if (col.getAD_Reference_ID() == 19
						|| (col.getAD_Reference_ID() == 30 && col.getAD_Reference_Value_ID() == 0)){
						MTable extTable = MTableUtil.getZ (getCtx(), col.getAD_Table_ID());
						String name = "list" + extTable.getTableName();
						tables.put(name, extTable);
						s_log.finest(name+ " External Link for "+getTableName());
					}
					
				}
			} catch (SQLException e){
				s_log.log(Level.SEVERE, "E", e);
			} finally {
				UtilExtended.closeCursor(ps, rs);
			}
			
			// 2. Prendre toutes les AD_reference sur cette table
			// Prendre toutes les columns ayant cette reference
			// Prendre la table correspondante
			// Reste à faire...
			
			ret = tables;
			externalLinkedTables.put(this, ret);
		}
		return ret;
	}
