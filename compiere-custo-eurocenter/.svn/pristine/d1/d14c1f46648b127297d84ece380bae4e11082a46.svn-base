...
public class MPInstancePara extends X_AD_PInstance_Para
{
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
    	...
    	MPInstance pi = new MPInstance(getCtx(), getAD_PInstance_ID(), get_Trx());
    	MProcess ps = new MProcess(getCtx(), pi.getAD_Process_ID(), get_Trx());
    	/*
    	if(ps.get_ValueAsBoolean("isEventLog"))
		{
			//1. Verifier si CM_Chat existe pour AD_Table_ID et Record_ID.
		}
		*/
	...
	protected boolean beforeSave(boolean newRecord) {
    	...
    	if(getP_Date()!=null || getP_Date_To()!=null)
    	{
    		MPInstance pi = new MPInstance(getCtx(), getAD_PInstance_ID(), get_Trx());
        	MProcess ps = new MProcess(getCtx(), pi.getAD_Process_ID(), get_Trx());
    		MProcessPara[] paras = ps.getParameters();
    		for (int i = 0; i < paras.length; i++) {
    			if(paras[i].getColumnName().equals(getParameterName()))
    			{
    				if(paras[i].getAD_Reference_ID() == 15)
    				{
    					if(getP_Date()!=null)
    					{
    						Calendar cal = Calendar.getInstance(); // locale-specific
    						cal.setTime(getP_Date());
    						cal.set(Calendar.HOUR_OF_DAY, 0);
    						cal.set(Calendar.MINUTE, 0);
    						cal.set(Calendar.SECOND, 0);
    						cal.set(Calendar.MILLISECOND, 0);
    						long time = cal.getTimeInMillis();
    						setP_Date(new Timestamp(time));
    					}
    					if(getP_Date_To()!=null)
    					{
    						Calendar cal = Calendar.getInstance(); // locale-specific
    						cal.setTime(getP_Date_To());
    						cal.set(Calendar.HOUR_OF_DAY, 0);
    						cal.set(Calendar.MINUTE, 0);
    						cal.set(Calendar.SECOND, 0);
    						cal.set(Calendar.MILLISECOND, 0);
    						long time = cal.getTimeInMillis();
    						setP_Date_To(new Timestamp(time));
    					}
    				}
    			}
    		}
