...
public class SavChangeStatusWindowImpl extends WindowImpl implements ASyncProcess 
{
	...
	public void unlockUI(ProcessInfo pi) {
 	...
 	public SavChangeStatusWindowImpl(int windowNO, Ctx serverCtx, WindowCtx windowCtx, UWindowID uid) {
		...
		/*+" AND ic.Sav_Status <> 'ATR'"
		+" AND ic.Sav_Status <> 'CLI'"*/
		+" AND il.qtyentered < 0"
		+" AND ic.AD_Client_ID=?", 
	...
	private void generate (ChangeVO change, Ctx ctx, WindowCtx windowCtx, int mode)
	{
		...
		if((windowCtx.get("SAV_STATUS_TARGET") != null) && !"".equals(windowCtx.get("SAV_STATUS_TARGET"))){
			for (int i = 0; i < order_ID.length; i++) {
				...
				params2.add(serverCtx.getAD_User_ID());
				params2.add("Y");
