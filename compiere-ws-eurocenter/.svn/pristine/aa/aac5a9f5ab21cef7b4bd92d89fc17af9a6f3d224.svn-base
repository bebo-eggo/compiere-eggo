...
public class setOrderWs extends BaseWs<ISetOrderService>{
	...
	@RequestMapping(value="/getOAFilesSize")
	@ResponseBody
	public long oaFilesSize(@RequestParam("C_Order_ID") int order_ID) {
		return service.oaFilesSize(order_ID);
	}
	
	@RequestMapping(value="/setExtraDatas", method = RequestMethod.POST)
	@ResponseBody
	public SetOrderResult setExtraDatas(@RequestBody OrderLinesDetails datas){
		return service.setExtraDatas(datas);
	}
}
