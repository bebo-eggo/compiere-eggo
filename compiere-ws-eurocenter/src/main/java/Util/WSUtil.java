...
public class WSUtil {
	...
	public static CopyOnWriteArrayList<Integer> getOrdersByPoRef(Ctx ctx,String PoRef, String IsSotrx, Trx trx)
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		CopyOnWriteArrayList<Integer> OrderIdList = new CopyOnWriteArrayList<Integer>();
		try {
			ps = DB.prepareStatement("SELECT C_Order_ID FROM C_Order WHERE AD_CLient_ID=? and upper(POREFERENCE) = upper(?) and IsSotrx = ? "
					+ "and ((DocStatus in ('CO') and C_Order.IsSotrx = 'N') OR (DocStatus in ('IP','CO') and C_Order.IsSotrx = 'Y') ) order by C_Order_ID DESC", trx);
			ps.setInt(1, ctx.getAD_Client_ID());
	...
	{
		String sql = "SELECT * FROM AD_Org WHERE AD_Client_ID in (?,0) and Value = ?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
	...
	public static HashMap<String,String> getFileTypePP(int AD_Client_ID, Trx trx)
	{
		HashMap<String,String> ret = new HashMap<String, String>();
		
		String sql = "Select Z_FILETYPE, XX_IsPP from XX_PARAMFILETYPE where AD_Client_ID = ?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{	
	...
	public static MTypeEcheance getTypeEchByValue(Ctx ctx, int AD_Client_ID, String typeValue)
	{
		String sql = "SELECT Z_TypeEcheance_ID FROM Z_TypeEcheance WHERE AD_Client_ID= ? and Value = ? ";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			rs = pstmt.executeQuery ();
			if (rs.next ())
				return new MTypeEcheance(ctx, rs.getInt(1), (Trx) null) ;
			else
				return null;
		}
		catch (Exception e) {
